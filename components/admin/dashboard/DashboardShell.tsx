...
function OperationalShortcuts() {
  ...
  return (
    <section className="mb-8">
      <div className="mb-4"><h2 className="text-xl font-black text-slate-950">Operational workspaces</h2><p className="mt-1 text-sm font-medium text-slate-700">Use these when you want a manual record view. The Admin AI remains the primary control surface.</p></div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{shortcuts.map((item) => <Link key={item.href} href={item.href} className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md"><div className="flex items-center justify-between gap-3"><Activity className="h-5 w-5 text-blue-700" /><ArrowRight className="h-4 w-4 text-slate-400 transition-transform group-hover:translate-x-1" /></div><h3 className="mt-4 font-black text-slate-950">{item.title}</h3><p className="mt-2 text-sm font-medium leading-5 text-slate-700">{item.description}</p></Link>)}</div>
    </section>
  );
}
...
export function AdminDashboardContent({ data, canAccessDevStudio = false }: { data: AdminDashboardData; canAccessDevStudio?: boolean }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] min-w-0 px-3 pb-16 pt-4 sm:px-5 lg:px-6">
      <section className="mb-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="p-6 sm:p-8"><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-2 rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-black uppercase tracking-wider text-emerald-900"><ShieldCheck className="h-4 w-4" /> Secure admin session</span><span className="rounded-full bg-blue-100 px-3 py-1.5 text-xs font-black text-blue-900">Live Supabase data</span></div><h1 className="mt-5 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl"><AdminGreeting name={firstName} /></h1><p className="mt-2 text-sm font-bold text-slate-600">{today}</p><p className="mt-4 max-w-3xl text-base font-medium leading-7 text-slate-700">This dashboard is an operational view of current application, enrollment, program, payment, compliance, and system-health data.</p></div></section>
