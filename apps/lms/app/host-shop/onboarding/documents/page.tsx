...
export default async function HostShopDocumentsPage({
  ...
  const allRequiredAccepted =
    board.requiredDocumentCount > 0 && board.acceptedDocumentCount === board.requiredDocumentCount;
      ...
      <div className="mx-auto max-w-5xl">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
          ...
          <p className="mt-2 max-w-3xl text-slate-700">
            Upload the required records for {partner.dba || partner.name}. Files are stored in the
            private partner-document bucket and remain pending until reviewed by Elevate.
          </p>
          <p className="mt-4 text-sm font-bold text-slate-700">
            {board.acceptedDocumentCount} of {board.requiredDocumentCount} required documents
            accepted
          </p>
          {isPlatformAdmin ? (
        ...
        {errorMessage ? (
          <div
            role="alert"
            className="mt-5 rounded-xl border border-red-300 bg-red-50 p-4 font-bold text-red-950"
          >
            {errorMessage}
          </div>
        ) : null}
                  ...
                  <div>
                    ...
                    {requirement.document?.file_name ? (
                      <p className="mt-2 text-xs font-semibold text-slate-600">
                        Latest file: {requirement.document.file_name}
                      </p>
                    ) : null}
                    {requirement.document?.rejection_reason ? (
                      <p className="mt-2 text-sm font-bold text-red-800">
                        Review note: {requirement.document.rejection_reason}
                      </p>
                    ) : null}
                  </div>
                  {statusBadge(status)}
          ...
          {allRequiredAccepted && partner.mou_signed && partner.onboarding_completed ? (
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="font-black text-green-800">
                  Host Shop onboarding requirements are complete.
                </p>
                <p className="mt-1 text-sm text-slate-700">
                  Return to the operational dashboard to manage apprentices and OJT hours.
                </p>
              </div>
              <Link
                href="/host-shop/dashboard"
                className="rounded-xl bg-green-700 px-5 py-3 text-center font-black text-white hover:bg-green-800"
              >
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm font-semibold text-slate-700">
                Pending documents stay visible here until Elevate accepts or rejects them.
              </p>
              <Link
                href="/host-shop/dashboard"
                className="rounded-xl border border-slate-300 px-5 py-3 text-center font-bold hover:bg-slate-50"
              >
                Return to dashboard
              </Link>
            </div>
