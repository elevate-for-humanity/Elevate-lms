  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex min-h-11 min-w-11 items-center justify-center gap-1.5 rounded-md p-2 transition-colors hover:bg-slate-100"
  ...
  return (
    <>
      <div className="admin-header-shell sticky top-0 z-50 border-b border-slate-200 bg-white text-slate-900 shadow-sm">
        <div className="mx-auto max-w-7xl px-3 sm:px-6">
          <div className="flex min-h-16 items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-3">
              <Link href="/dashboard" className="flex min-w-0 items-center gap-2">
                ...
                <div className="hidden min-w-0 sm:block">
                  ...
                  <span className="mt-0.5 flex items-center gap-1 text-[10px] font-bold text-emerald-700">
            ...
            <div className="flex shrink-0 items-center gap-0.5">
              {searchOpen ? (
                <form onSubmit={handleSearch} className="flex items-center gap-1 rounded-md border border-slate-200 bg-slate-50 px-1 py-1 sm:gap-2 sm:px-2">
                  <input
                    ref={searchInputRef}
                    type="search"
                    value={searchQuery}
                    ...
                    className="w-24 bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-500 sm:w-32 lg:w-48"
                  />
                  <button type="submit" className="inline-flex min-h-11 min-w-11 items-center justify-center rounded hover:bg-slate-100" aria-label="Submit search"><Search className="h-4 w-4" /></button>
                  <button type="button" onClick={() => setSearchOpen(false)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded hover:bg-slate-100" aria-label="Close search"><X className="h-4 w-4" /></button>
                </form>
              ) : (
                <button onClick={() => setSearchOpen(true)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md p-2 transition-colors hover:bg-slate-100" title="Search" aria-label="Search students"><Search className="h-4 w-4" /></button>
              )}

              <LanguageSwitcher />

              <Link href="/notifications" className="relative inline-flex min-h-11 min-w-11 items-center justify-center rounded-md p-2 transition-colors hover:bg-slate-100" aria-label="Notifications">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex min-h-11 min-w-11 items-center justify-center gap-1.5 rounded-md p-1.5 transition-colors hover:bg-slate-100"
                  aria-label="Open admin account menu"
                  aria-expanded={profileOpen}
                >
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-orange-500 to-red-600"><User className="h-3.5 w-3.5 text-white" /></div>
              ...
              <button
                ...
                className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md p-2 transition-colors hover:bg-slate-100 xl:hidden"
        ...
        <div className={`overflow-hidden border-t border-slate-200 transition-[max-height] duration-200 xl:hidden ${navExpanded ? 'visible max-h-96 pointer-events-auto' : 'invisible max-h-0 pointer-events-none'}`}>
          <div className="px-3 py-3 sm:px-4">
            <p className="mb-2 text-xs font-semibold text-slate-600">Choose a workspace:</p>
