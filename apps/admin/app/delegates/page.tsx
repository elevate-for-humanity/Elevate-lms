...
export const metadata: Metadata = {
  alternates: { canonical: 'https://admin.elevateforhumanity.org/delegates' },
...
export default async function DelegatesPage() {
  ...
  const delegateUserIds = [
    ...new Set((rawDelegates ?? []).map((d: any) => d.user_id).filter(Boolean)),
  ];
  const { data: delegateProfiles } = delegateUserIds.length
    ...
    : { data: [] };
  const delegateProfileMap = Object.fromEntries(
    (delegateProfiles ?? []).map((p: any) => [p.id, p]),
  );
  const delegates = (rawDelegates ?? []).map((d: any) => ({
    ...d,
    profiles: delegateProfileMap[d.user_id] ?? null,
  }));
            ...
            {delegates && delegates.length > 0 ? (
              delegates.map((delegate: any) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-brand-blue-100 rounded-full flex items-center justify-center">
                      <span className="text-brand-blue-600 font-medium">
                        {(delegate.profiles?.full_name || 'U')[0]}
                      </span>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    <span className="px-2 py-1 bg-brand-blue-100 text-brand-blue-800 rounded-full text-xs">
