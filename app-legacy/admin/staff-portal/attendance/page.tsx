...
export default async function StaffAttendancePage() {
  ...
  const { data: profile } = await supabase
    ...
    .eq('id', user.id)
    .maybeSingle();
  if (!profile || !['staff', 'admin', 'staff'].includes(profile.role))
  ...
  try {
    ...
    const orgId = staffUser?.staff_id;

    // Fetch attendance sessions — scoped to this staff's hosted sessions
    ...
    // Also pull from staff_attendance for weekly records
  ...
  return (
    <div>
      ...
      <section className="relative h-[160px] sm:h-[220px] md:h-[280px] overflow-hidden rounded-xl mb-6 -mx-4 sm:-mx-6 lg:-mx-8">
        ...
        <Image
          src="/images/pages/staff-page-2.webp"
      ...
      <div className="mb-6">
        <Breadcrumbs
          items={[{ label: 'Staff', href: '/staff/attendance' }, { label: 'Attendance' }]}
        />
      </div>
