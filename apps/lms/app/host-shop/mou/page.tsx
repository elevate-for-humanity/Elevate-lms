...
export default async function HostShopMouPage({
  ...
  const program = requested && (configured.length === 0 || configured.includes(requested))
    ? requested
    : configured[0] || 'barber';
    ...
    <main className="min-h-screen bg-slate-50 py-8">
      <div className="mx-auto max-w-5xl space-y-6 px-4 sm:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm font-extrabold uppercase tracking-[0.12em] text-brand-red-700">Host Shop Partnership</p>
            <h1 className="mt-1 text-3xl font-black text-slate-950">Memorandum of Understanding</h1>
            <p className="mt-2 text-slate-600">{partner.name || 'Host Shop'} · {program.replace(/\b\w/g, (letter: string) => letter.toUpperCase())}</p>
          </div>
          <Link href="/host-shop/dashboard/board" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-800 hover:bg-slate-50">Back to board</Link>
        </div>
        ...
        {configured.length > 1 && (
          <div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-4">
            {configured.map((item) => (
              <Link
                key={item}
                href={`/host-shop/mou?program=${encodeURIComponent(item)}`}
                className={`rounded-full px-3 py-1.5 text-sm font-bold ${item === program ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-800'}`}
              >
                {item.replace(/\b\w/g, (letter: string) => letter.toUpperCase())}
              </Link>
            ))}
          </div>
        )}
