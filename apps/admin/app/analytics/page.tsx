...
export default async function AnalyticsPage() {
  ...
  const [
    ...
    db.from('page_views').select('*', { count: 'exact', head: true }).gte('created_at', start30d),
    db.from('program_enrollments').select('program_id, programs:program_id(title)').not('program_id', 'is', null).limit(500),
  ]);
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="bg-white border-b border-slate-200 px-6 py-5">
        ...
        <p className="text-sm text-slate-500 mt-1">Website traffic, platform performance, learner outcomes, and operational metrics</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-4">
          {METRICS.map((m) => {
            const Icon = m.icon;
            return (
              <Link key={m.label} href={m.href} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-all group">
                <div className={`w-9 h-9 rounded-xl ${m.bg} flex items-center justify-center mb-3`}><Icon className={`w-4 h-4 ${m.color}`} /></div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/programs" className="text-xs text-brand-blue-600 hover:underline font-medium">View all →</Link>
          </div>
          {ranked.length === 0 ? (
              ...
              {ranked.map((p) => (
                <div key={p.title} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-brand-blue-500 rounded-full transition-all" style={{ width: `${Math.round((p.count / maxCount) * 100)}%` }} />
                  </div>
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {DEEP_LINKS.map((l) => {
              const Icon = l.icon;
              return (
                <Link key={l.href} href={l.href} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-all flex items-start gap-4 group">
                  <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0"><Icon className="w-4 h-4 text-slate-600" /></div>
