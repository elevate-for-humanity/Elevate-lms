...
function hasRequiredBoundaryTypes(
  conditions: z.infer<
    typeof boundaryConditionSchema
  >[],
): boolean {
  const types = new Set(
...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: 'Invalid CFD project query.',
  ...
  let actor: Awaited<
    ReturnType<typeof resolveAdminOrganization>
  >;


  try {
    actor = await resolveAdminOrganization(
      auth,
      parsedQuery.data.org_id,
    );
  } catch (error) {
    return safeError(
...
export async function POST(request: NextRequest) {
  ...
  if (
    !hasRequiredBoundaryTypes(
      parsedBody.data.configuration.boundaryConditions,
    )
  ) {
    return safeError(
      'CFD configuration requires at least one inlet, outlet and wall boundary.',
      400,
    );
  }
  ...
  try {
    ...
    if (projectInput.input_media_asset_ids.length > 0) {
      const { data: assets, error: assetsError } = await db
        ...
        .in(
          'id',
          projectInput.input_media_asset_ids,
        );
      ...
      if (assetsError) {
        return safeInternalError(
          assetsError,
          'Failed to validate CFD input assets',
        );
      }
      ...
      if (
        (assets?.length ?? 0) !==
        projectInput.input_media_asset_ids.length
      ) {
        return safeError(
          'One or more CFD input assets are unavailable.',
          400,
        );
      }
    }
    ...
    await writePlatformAuditEvent({
      ...
      metadata: {
        solver: data.solver,
        analysisType:
          projectInput.configuration.analysisType,
      },
    });
