...
const PROGRAM_IMAGES: Record<string, string> = {
  cna: '/images/pages/cna-nursing-real.webp',
  qma: '/images/pages/programs-cna-hero.webp',
...
export default async function ProgramsPage() {
...

  {
    const { data } = await db
      .from('programs')
      .select(
        'slug,title,short_description,description,category,duration,credential_type,wioa_eligible',
      )
      .eq('is_active', true)
      .eq('published', true)
      .neq('status', 'archived')
      ...
      programs = data
        ...
        .map((p) => {
          ...
          if (desc && !/[.!?]$/.test(desc.trim())) {
            const l = Math.max(desc.lastIndexOf('.'), desc.lastIndexOf('!'), desc.lastIndexOf('?'));
            desc = l > 20 ? desc.slice(0, l + 1) : null;
          }
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="relative h-64 sm:h-80 w-full overflow-hidden">
        ...
        <Image
          ...
          alt="{PLATFORM_DEFAULTS.orgName} programs"
          fill
        ...
        <div className="relative z-10 flex h-full flex-col justify-center px-6 sm:px-12 max-w-6xl mx-auto">
          ...
          <p className="mt-3 text-slate-200 text-sm sm:text-base max-w-xl">
            {programs.length} credential-bearing programs · 4–12 weeks · WIOA &amp; WRG funding
            available
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              href="/orientation/schedule"
              className="inline-flex items-center gap-2 rounded-lg bg-brand-red-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-red-700 transition-colors"
            >
              Schedule Free Orientation
            </Link>
            <Link
              href="/programs/catalog"
              className="inline-flex items-center gap-2 rounded-lg bg-white/10 border border-white/30 px-5 py-2.5 text-sm font-semibold text-slate-900 hover:bg-white/20 transition-colors"
            >
              Search Full Catalog
            </Link>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 overflow-x-auto">
          <ul className="flex gap-1 py-2 whitespace-nowrap">
            <li>
              <a
                href="#top"
                className="inline-block px-4 py-1.5 rounded-full text-sm font-medium text-slate-600 hover:bg-slate-100 transition-colors"
              >
                All ({programs.length})
              </a>
            </li>
      ...
      <div className="bg-brand-green-50 border-b border-brand-green-100">
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-brand-green-800">
          ...
          <Link
            ...
            className="ml-auto font-semibold underline hover:text-brand-green-900 whitespace-nowrap"
          >
            Check eligibility →
          </Link>
        </div>
                ...
                {list.map((p) => (
                  ...
                  >
                    <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-100 flex-shrink-0">
                      ...
                      />
                      {p.funding_eligible && (
                        <span className="absolute top-2 left-2 bg-brand-green-600 text-white text-xs font-bold px-2 py-0.5 rounded-full shadow">
                          WIOA Eligible
                        </span>
                      )}
                    </div>
                    <div className="flex flex-col flex-1 p-4">
                      <h3 className="font-semibold text-slate-900 text-sm leading-snug group-hover:text-brand-red-600 transition-colors">
                        {p.title}
                      </h3>
                      {p.description && (
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-400">
                        ...
                        {p.credential && (
                          <span className="flex items-center gap-1">
                            <Award className="w-3 h-3" aria-label="award" />
                            {p.credential}
                          </span>
                        )}
                        {p.funding_eligible && !p.duration && !p.credential && (
      ...
      <section className="py-12 border-t border-slate-100 bg-slate-50">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="text-slate-500 text-sm mb-6">
            Google and Microsoft certificates available through Coursera and LinkedIn Learning.
            Elevate advisors can help you access funding and enroll.
          </p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <Link
                ...
                className="bg-white rounded-xl border border-slate-200 p-5 hover:border-brand-green-400 hover:shadow-sm transition-all group"
              >
                <p className="font-bold text-slate-900 text-sm group-hover:text-brand-green-700 leading-snug">
                  {p.title}
                </p>
      ...
      <section className="bg-slate-900 text-white py-16">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="mt-3 text-slate-300 text-sm sm:text-base">
            Schedule a free orientation. We&apos;ll match you to the right program, check your
            funding eligibility, and get you enrolled.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Link
              href="/orientation/schedule"
              className="rounded-lg bg-brand-red-600 px-8 py-3 font-semibold text-white hover:bg-brand-red-700 transition-colors"
            >
              Schedule Free Orientation
            </Link>
            <Link
              href="/contact"
              className="rounded-lg border border-white/30 px-8 py-3 font-semibold text-slate-900 hover:bg-white/10 transition-colors"
            >
              Talk to an Advisor
            </Link>
          </div>
