...
import { OpenHostShopPortalButton } from './OpenHostShopPortalButton';
...
export async function HostShopPortalLauncher() {
    ...
    return (
      <section className="mb-8 rounded-3xl border border-rose-200 bg-rose-50 p-6 text-rose-950">
        ...
        <p className="mt-2 text-sm font-medium">
          The shop directory could not be loaded. No portal access was changed.
        </p>
      </section>
    );
  }
    ...
    return (
      ...
      >
        <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
          <h3 className="min-w-0 break-words text-lg font-black leading-tight text-slate-950 [overflow-wrap:anywhere]">
            {shop.name || 'Unnamed shop'}
          </h3>
          <span
            className={`w-fit max-w-full rounded-full px-2.5 py-1 text-xs font-black ${canOpen ? 'bg-emerald-100 text-emerald-900' : 'bg-amber-100 text-amber-900'}`}
          >
            {status}
          </span>
        </div>
  ...
  return (
    <section
      ...
      aria-labelledby="host-shop-launcher-title"
    >
      <div className="border-b border-slate-100 p-6 sm:p-8">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 id="host-shop-launcher-title" className="text-xl font-black text-slate-950">
              Open a Host Shop portal
            </h2>
            <p className="mt-1 max-w-3xl text-sm font-medium leading-6 text-slate-700">
              Open any connected shop in the live Host Shop workspace. The selected shop is
              remembered for one hour and does not change ownership, permissions, or compliance
              status.
            </p>
          </div>
        </div>
      ...
      {!shops.length ? (
        <p className="p-6 text-sm font-medium text-slate-700 sm:p-8">
          No Host Shop records are available.
        </p>
      ) : (
      ...
      {testShops.length ? (
        <details className="border-t border-slate-200 p-4 sm:px-8">
          <summary className="cursor-pointer text-sm font-black text-slate-700">
            Test & audit shops ({testShops.length})
          </summary>
          <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {testShops.map(renderShop)}
          </div>
        </details>
      ) : null}
    </section>
