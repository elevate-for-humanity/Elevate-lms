...
export async function GET(request: NextRequest) {
  ...
  const scope = request.nextUrl.searchParams.get('scope') ?? request.nextUrl.searchParams.get('category') ?? undefined;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const content = String(body.content ?? body.value ?? '').trim();
    if (!key || !content) return safeError('key and value are required', 400);

    const scope = String(body.scope ?? body.category ?? 'platform');
    const db = await requireAdminClient();
    const row = {
      scope,
      key,
    ...
    return jsonOk({ memory: { ...data, category: data.scope, value: data.content } }, { status: 201 });
  } catch (err) {
