...
const GPU_DEPLOYMENT_PLAN =
  process.env.NORTHFLANK_GPU_DEPLOYMENT_PLAN || `nf-gpu-${GPU_TYPE}-${GPU_COUNT}g`;
...
const ACCEPTANCE_TIMEOUT_MS = Number(
  process.env.GPU_PROVISION_ACCEPTANCE_TIMEOUT_MS || 30 * 60 * 1000,
);
...
async function preflight() {
  ...
  if (region !== GPU_REGION)
    throw new Error(
      `GPU project ${GPU_PROJECT_ID} is in ${region || 'unknown'}, expected ${GPU_REGION}`,
    );
  ...
  if (Number(gpu.memoryInfo?.sizeInMiB || 0) < 24 * 1024)
    throw new Error(`${GPU_TYPE} exposes less than 24GB VRAM`);
  if (Array.isArray(gpu.countOptions) && !gpu.countOptions.includes(GPU_COUNT))
    throw new Error(`${GPU_TYPE} does not support gpuCount=${GPU_COUNT}`);

  const plans = arrayFrom(await nfFetch<R>('/plans'), 'plans');
  if (
    !plans.some(
      (plan) => plan.id === BUILD_PLAN && Array.isArray(plan.type) && plan.type.includes('build'),
    )
  ) {
  ...
  if (GPU_DEPLOYMENT_EPHEMERAL_MB !== 256000) {
    throw new Error(
      `Northflank L4 deployment ephemeral storage must be exactly 256000MB; got ${GPU_DEPLOYMENT_EPHEMERAL_MB}`,
    );
  }
...
async function ensureVolume(): Promise<string> {
  const volumes = arrayFrom(
    await nfFetch<R>(projectApiPath(GPU_PROJECT_ID, '/volumes')),
    'volumes',
  );
...
async function scaleToZero(): Promise<void> {
  if (!(await serviceExists())) return;
  await nfFetch(projectApiPath(GPU_PROJECT_ID, `/services/combined/${SERVICE_ID}`), {
    method: 'PATCH',
    body: JSON.stringify({ deployment: { instances: 0 } }),
  });
  log('GPU worker returned to zero-instance idle state');
}
...
async function ensureService(volumeId: string) {
  ...
  try {
    await nfFetch(
      projectApiPath(
        GPU_PROJECT_ID,
        exists ? `/services/combined/${SERVICE_ID}` : '/services/combined',
      ),
      {
    ...
    if (message.includes('402') || /minimum \$50 credit/i.test(message)) {
      throw new Error(
        'Northflank GPU billing is locked: purchase at least $50 of Northflank account credits, then rerun this workflow.',
      );
    }
    throw error;
  }
  if (exists) {
    ...
    for (const volume of volumes) {
      ...
      const detailResponse = await nfFetch<R>(
        projectApiPath(GPU_PROJECT_ID, `/volumes/${volume.id}`),
      );
...
async function logBuildFailure(buildId: string, build: R): Promise<void> {
  ...
  try {
    ...
    const entries = arrayFrom(
      await nfFetch<R[]>(
        projectApiPath(GPU_PROJECT_ID, `/services/${SERVICE_ID}/build-logs?${query}`),
      ),
    );
    const diagnosticPattern =
      /error|failed|failure|denied|not found|no space|unauthorized|manifest|timeout|timed out|tls|certificate|rate limit|\b429\b|\b5\d\d\b/i;
    ...
    log(
      'Unable to retrieve Northflank build logs',
...
async function waitForExactBuild({ buildId, sha }: ExactBuild): Promise<void> {
  ...
  while (Date.now() < deadline) {
    const response = await nfFetch<R>(
      projectApiPath(GPU_PROJECT_ID, `/services/${SERVICE_ID}/build/${buildId}`),
    );
    const build = response.data || response;
    const status = String(build.status || 'unknown').toUpperCase();
    if (status !== previous) {
      log(`Exact build ${buildId} status ${status}`);
      previous = status;
    }
    if (failed.has(status)) {
      await logBuildFailure(buildId, build);
      throw new Error(
        `GPU build ${buildId} failed: ${status}${build.message ? `: ${build.message}` : ''}`,
      );
    }
    if (status === 'SUCCESS') {
      if (!build.sha || String(build.sha).toLowerCase() !== sha.toLowerCase()) {
        throw new Error(
          `GPU build SHA mismatch: expected ${sha}, received ${build.sha || 'missing'}`,
        );
      }
      return;
    }
...
function deploymentStatus(service: R) {
  return (
    service.status?.deployment?.status || service.deployment?.status || service.deploymentStatus
  );
}
...
function collectStrings(value: unknown, output: string[] = []): string[] {
  if (typeof value === 'string') output.push(value);
    ...
    Object.values(value as R).forEach((item) => collectStrings(item, output));
  return output;
}

function discoverPublicUrl(service: R): string | null {
  for (const value of collectStrings(service)) {
    if (/^https?:\/\//i.test(value) && !value.includes('github.com'))
...
async function waitForPublicUrl(): Promise<string> {
  for (let attempt = 0; attempt < 60; attempt++) {
    const url = discoverPublicUrl(
      await nfFetch<R>(projectApiPath(GPU_PROJECT_ID, `/services/${SERVICE_ID}`)),
    );
    if (url) return url;
    await sleep(10_000);
  }
...
async function wireSecrets(publicUrl: string, workerSecret: string) {
  await upsertSecretGroup(GPU_PROJECT_ID, 'elevate-gpu-worker-env', SERVICE_ID, {
...
async function acceptanceGenerate(publicUrl: string, secret: string) {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        `Wan acceptance generation failed (${response.status}): ${(await response.text()).slice(-4000)}`,
      );
    const generated = (await response.json()) as R;
    if (!generated.jobId || !generated.assetPath)
      throw new Error('Wan acceptance returned no asset');
    ...
    if (!contentType.startsWith('video/mp4') || bytes.length < 50_000)
      throw new Error(`Invalid acceptance MP4 (${contentType}, ${bytes.length} bytes)`);
    log('Real Wan MP4 acceptance passed', { bytes: bytes.length });
    await fetch(`${publicUrl}${generated.assetPath}`, {
      method: 'DELETE',
      headers: { authorization: `Bearer ${secret}` },
    }).catch(() => undefined);
  } finally {
...
async function main() {
  ...
  try {
    ...
    await Promise.all([
      restart(GPU_PROJECT_ID, SERVICE_ID),
      restart(WEB_PROJECT_ID, ADMIN_SERVICE_ID),
    ]);
    await waitForReady(publicUrl, workerSecret);
    if (process.env.GPU_RUN_ACCEPTANCE === 'true') {
      ...
      log('GPU readiness passed; paid render acceptance was not requested');
    }
  } finally {
    await scaleToZero();
  }
