    ...
    if (isAdmin && !isDashboard) {
      return (
        <CanonicalLearnerWorkspaceLayout>
          <AdminModulePreview pathname={pathname} previewing={subject.previewing} />
        </CanonicalLearnerWorkspaceLayout>
      );
    }
  }
...
function AdminModulePreview({ pathname, previewing }: { pathname: string; previewing: boolean }) {
  const moduleName =
    pathname.split('/').filter(Boolean).pop()?.replace(/-/g, ' ') || 'student module';
  return (
    <main className="mx-auto max-w-5xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-slate-700">
        {previewing
          ? 'The learner dashboard preview is read-only. Detailed records and actions stay in secured Admin management so an administrator cannot accidentally act as the learner.'
          : 'This Student Portal module is operational. No learner identity or record is attached to the administrator session.'}
      </p>
