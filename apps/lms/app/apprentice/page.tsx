...
export default async function ApprenticePortalPage() {
  ...
  if (isNeutralAdminPreview) {
    return <main className="mx-auto max-w-5xl space-y-6 pb-10"><section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><p className="text-xs font-black uppercase tracking-[0.16em] text-brand-red-700">Administrator portal preview</p><h1 className="mt-2 text-3xl font-black text-slate-950">Apprentice PWA</h1><p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-slate-700">This neutral preview confirms that the Apprentice PWA is available without attaching an administrator to a learner record. Select a learner from secured Admin student management to open an audited, read-only dashboard.</p><div className="mt-6 flex flex-wrap gap-3"><a href="https://admin.elevateforhumanity.org/students" className="inline-flex min-h-11 items-center justify-center rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Select a learner in Admin</a><a href="https://admin.elevateforhumanity.org/dashboard" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-black text-slate-950">Return to Admin dashboard</a></div></section><section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{['Orientation','Work Hours','Timeclock','Competencies','Documents','Billing & Payments','RTI','Handbook','State Board'].map((label) => <article key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><ShieldCheck className="h-5 w-5 text-brand-red-700"/><h2 className="mt-3 font-black text-slate-950">{label}</h2><p className="mt-1 text-sm text-slate-600">Available after an authorized learner is selected.</p></article>)}</section></main>;
  }
  ...
  const [{ data: profile }, { data: docs }, certsRes, { data: documentRequirements }, { data: handbookAcceptance }, { data: cosmetologyBilling }, { data: theorySchedule }] = await Promise.all([
    ...
    (db as any).from('apprenticeship_theory_schedules')
      .select('id,course_id,timezone,days_of_week,start_time,end_time,weekly_target_minutes,weekly_max_minutes,active')
      .eq('user_id', subject.userId).eq('program_slug', programSlug).eq('active', true).limit(1).maybeSingle(),
  ]);
  ...
  return <main className="space-y-7 pb-10">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5" aria-label="Apprentice progress"><Metric label="Registered competencies" value={`${progress.competencies.completed} / ${progress.competencies.required}`} detail={`${progress.competencies.percent}% verified competency progress`} icon={CheckCircle2} /><Metric label="Verified RTI hours" value={`${progress.rti.verifiedHours.toFixed(2)} / ${progress.rti.requiredHours}`} detail={`${progress.rti.percent}% verified RTI · ${progress.rti.pendingEntries} pending entries`} icon={BookOpen} /><Metric label="Approved weekly OJL" value={progress.ojl.approvedHours.toLocaleString()} detail={`${progress.ojl.pendingEntries} pending work entries · approved supervised hours only`} icon={Clock3} /><Metric label="Verified transfer hours" value={progress.ojl.transferHoursVerified ? progress.ojl.transferHours.toLocaleString() : 'Pending'} detail={progress.ojl.transferHoursVerified ? `${progress.ojl.totalCreditedHours.toLocaleString()} total credited OJL hours` : 'Transcript evidence and reviewer approval required'} icon={ShieldCheck} /><Metric label="Certificates / documents" value={String(certsRes.count ?? 0)} detail={`${verifiedDocs} of ${totalDocs} documents verified`} icon={Award} /></section>
