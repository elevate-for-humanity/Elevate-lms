...
import { ExternalLink, Film, ShieldCheck, Upload } from 'lucide-react';
...
export default async function PurchasedMediaPluginPage({
  ...
  const uploadHref = courseId
    ? `/videos/upload?courseId=${encodeURIComponent(courseId)}`
    : '/videos/upload';
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <header className="rounded-2xl border border-cyan-800 bg-gradient-to-br from-cyan-950 to-slate-950 p-6">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
            Open your licensed Envato library, download the correct scene, then upload it to the selected canonical course.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <Link href={uploadHref} className="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-4 py-3 font-black text-slate-950 hover:bg-cyan-400">
              <Upload className="h-4 w-4" /> Upload to course
            </Link>
        ...
        <section className="rounded-2xl border border-emerald-800 bg-emerald-950/40 p-5">
          <div className="flex items-start gap-3">
            <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-300" />
            <div>
              <h2 className="font-black text-emerald-100">Licensed upload workflow</h2>
              <p className="mt-1 text-sm text-emerald-100/80">
                Download only media covered by your Envato license. Keep the license certificate with the source asset record, then use Upload to course to attach the downloaded file.
              </p>
            </div>
          </div>
        </section>
