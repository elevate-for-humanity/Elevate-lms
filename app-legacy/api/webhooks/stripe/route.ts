...
async function flagCertRows(
  ...
  if (updateErr) {
    ...
    logger.info(
      `[webhook] Flagged ${certIds.length} certificate(s) as funding-refunded: ${certNumbers.join(', ')}`,
    );
  }
}
...
async function _POST(request: NextRequest) {
  ...
  try {
    ...
    logger.error(
      '[webhook] hydrateProcessEnv failed — continuing with process.env as-is',
      hydrateErr as Error,
    );
  }
  ...
  if (!supabase) {
    logger.error(
      '[webhook] Supabase admin client not initialized — SUPABASE_SERVICE_ROLE_KEY missing',
    );
  ...
  try {
    ...
    try {
      ...
      if (isDestructive) {
        logger.error(
          `[webhook] FAIL-CLOSED: Skipping destructive event ${event.type} (${event.id}) — cannot verify idempotency`,
        );
        Sentry.captureException(idempotencyErr, {
          tags: {
            subsystem: 'stripe_webhook',
            failure: 'idempotency_fail_closed',
    ...
    try {
      ...
      if (insertError?.code === '42703') {
        ...
        const fallback = await supabase.from('stripe_webhook_events').insert({
          stripe_event_id: event.id,
          event_type: event.type,
          status: 'processing',
        });
    ...
    if (isDestructive && !eventRecorded) {
      logger.error(
        `[webhook] FAIL-CLOSED: Cannot record destructive event ${event.type} (${event.id}) — skipping to prevent duplicate mutations`,
      );
      Sentry.captureException(
        new Error(`Fail-closed: could not record destructive webhook ${event.type}`),
        {
          tags: {
            subsystem: 'stripe_webhook',
            failure: 'record_fail_closed',
            event_type: event.type,
          },
        },
      );
      ...
      switch (event.type) {
        ...
        case 'payment_intent.payment_failed': {
          ...
          if (enrollmentId) {
            ...
            }).then(undefined, (err) => logger.error('[Stripe] Failed to insert payment_failed admin alert', normalizeError(err, 'Failed to insert payment_failed admin alert')));
            ...
            }).catch((err) => logger.error('[Stripe] Failed to send payment_failed admin email', normalizeError(err, 'Failed to send payment_failed admin email')));
            ...
            if (customerEmail) {
              ...
              }).catch((err) => logger.error('[Stripe] Failed to send payment_failed customer email', normalizeError(err, 'Failed to send payment_failed customer email'), { customerEmail }));
            }
          }
          break;
        }
        ...
        case 'invoice.payment_failed': {
          ...
          // Admin alert for all invoice payment failures
          {
            ...
            }).then(undefined, (err) => logger.error('[Stripe] Failed to insert invoice_payment_failed admin alert', normalizeError(err, 'Failed to insert invoice_payment_failed admin alert')));
            ...
            }).catch((err) => logger.error('[Stripe] Failed to send invoice_payment_failed admin email', normalizeError(err, 'Failed to send invoice_payment_failed admin email')));
            ...
            if (customerEmail) {
              ...
              }).catch((err) => logger.error('[Stripe] Failed to send invoice_payment_failed customer email', normalizeError(err, 'Failed to send invoice_payment_failed customer email'), { customerEmail }));
            }
          }
