...
type ProgramFilter = 'all' | 'published' | 'unpublished';
...
export function ProgramsTable({
  ...
  const [filterStatus, setFilterStatus] = useState<ProgramFilter>('published');
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border">
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider">Modules</th>
      ...
      <div className="px-6 py-4 border-t bg-slate-50">
        <p className="text-sm text-black">Showing <span className="font-medium">{filteredPrograms.length}</span> of <span className="font-medium">{programs.length}</span> programs</p>
      </div>
    </div>
  );
}
