...
export const AI_TOOL_REGISTRY: Record<string, AIToolDefinition> = {
  ...
  'wioa.followups.search': {
    name: 'wioa.followups.search',
    description: 'Find WIOA participant follow-ups that are due, overdue, incomplete, or match a requested follow-up type such as 30-day.',
    mode: 'read',
    risk: 'medium',
    requiresApproval: false,
    requiredRoles: ['admin', 'super_admin', 'staff'],
    ...
    inputSchema: {
      type: 'object',
      properties: {
        type: { type: 'string', description: 'Optional follow-up type or phrase, e.g. 30-day.' },
        status: { type: 'string' },
        overdueOnly: { type: 'boolean' },
        dueBefore: { type: 'string', description: 'ISO date or datetime cutoff.' },
        limit: { type: 'integer', minimum: 1, maximum: 100 },
      },
      additionalProperties: false,
    },
  ...
  'wioa.performance.summary': {
    name: 'wioa.performance.summary',
    description: 'Return non-PII WIOA performance metrics for a reporting period so an AI can generate an audit-ready narrative.',
    mode: 'read',
    risk: 'medium',
    requiresApproval: false,
    requiredRoles: ['admin', 'super_admin', 'staff'],
