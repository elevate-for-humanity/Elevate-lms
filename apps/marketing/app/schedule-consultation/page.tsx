...
const MEETING_TYPES = [
  ...
  },
  {
    ...
    duration: '30 min',
    desc: 'Discuss programs, eligibility, and next steps',
  },
  {
    value: 'funding',
    label: 'Financial Aid & Funding Review',
    duration: '30 min',
    desc: 'WIOA, Job Ready Indy, grants, and payment options',
  },
  {
    value: 'info',
    label: 'Program Information Session',
    duration: '20 min',
    desc: 'Learn about a specific training program',
  },
  {
    value: 'career',
    label: 'Career Advising',
    duration: '30 min',
    desc: 'Explore career paths and program fit',
  },
];
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-white border-b border-slate-200 py-6 px-6">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-slate-600 text-sm mt-1">
            Meet with our enrollment team to discuss your training goals.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2 flex-1">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= s ? 'bg-brand-red-600 text-white' : 'bg-slate-200 text-slate-500'}`}
              >
                {s}
              </div>
              <span
                className={`text-sm hidden sm:inline ${step >= s ? 'text-slate-900 font-medium' : 'text-slate-400'}`}
              >
                {s === 1 ? 'Meeting Type' : s === 2 ? 'Date & Time' : 'Your Info'}
              </span>
              {s < 3 && (
                <div className={`flex-1 h-0.5 ${step > s ? 'bg-brand-red-600' : 'bg-slate-200'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        {error && (
          <div className="bg-brand-red-50 border border-brand-red-200 text-brand-red-700 rounded-lg p-3 mb-4 text-sm">
            {error}
          </div>
        )}
              ...
              {MEETING_TYPES.map((type) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-slate-500 text-sm flex-shrink-0 ml-4">
                      {type.duration}
                    </span>
                  </div>
                </button>
          ...
          <div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4">
              <div className="text-sm text-slate-500">
                <strong>{MEETING_TYPES.find((m) => m.value === meetingType)?.label}</strong> ·{' '}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-300 rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-brand-red-500 focus:border-brand-red-500"
