            ...
            {nextMilestone && (
              <div className="bg-brand-red-500/20 border border-brand-red-500/30 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-brand-red-300 text-sm font-medium">Next Milestone</span>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-brand-red-500/30 rounded-xl flex items-center justify-center">
                    {(() => {
                      const Icon = ICON_MAP[nextMilestone.icon];
                      return <Icon className="w-7 h-7 text-brand-red-400" />;
                    })()}
                  </div>
                  <div className="flex-1">
                    <h3 className="text-white font-bold">{nextMilestone.title}</h3>
                    <p className="text-brand-red-200 text-sm">{nextMilestone.description}</p>
                    <div className="mt-2">
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-brand-red-300">{totalHours} / {nextMilestone.hoursRequired} hours</span>
                        <span className="text-brand-red-300">{Math.round((totalHours / nextMilestone.hoursRequired) * 100)}%</span>
                      </div>
