...
export default async function HostShopDashboardView() {
  ...
  try {
    ...
    if (isPlatformAdmin && error instanceof Error && error.message === 'HOST_SHOP_ADMIN_PARTNER_REQUIRED') {
      ...
      return <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6"><section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="relative h-52 w-full"><Image src="/images/pages/admin-partners-hero.webp" alt="Host Shop apprenticeship workspace" fill priority className="object-cover" sizes="100vw" /></div><div className="p-7 sm:p-9"><p className="text-xs font-black uppercase tracking-[0.14em] text-brand-blue-700">Admin portal access</p><h1 className="mt-2 text-3xl font-black text-slate-950">Choose an active verified Host Shop</h1><form action={selectAdminPartner} className="mt-6 flex flex-col gap-3 sm:flex-row"><select name="partnerId" required className="min-h-12 flex-1 rounded-xl border border-slate-400 bg-white px-4 py-3 font-semibold"><option value="">Select Host Shop</option>{partners.map((partner: any) => <option key={partner.id} value={partner.id}>{partner.name} {partner.city ? `— ${partner.city}, ${partner.state || ''}` : ''}</option>)}</select><button className="min-h-12 rounded-xl bg-brand-blue-700 px-6 py-3 font-black text-white">Open Host Shop portal</button></form></div></section></main>;
    }
  ...
  const toolCards: PortalCard[] = [
    { href: '/host-shop/orientation', title: 'Required orientation', detail: 'Review recruiting, payroll, WIOA, geofencing, work approval, competencies, transfer-credit rules, RAPIDS, and compliance.', image: '/images/pages/orientation-page-2.webp' },
    { href: '/host-shop/dashboard/match-requests', title: 'Match requests', detail: 'Review apprentice placement requests for your shop.', image: '/hero-images/employer-new-hero.webp' },
    { href: '/host-shop/dashboard/attendance/record', title: 'Record attendance', detail: 'Record attendance against the active apprentice placement.', image: '/images/pages/pathways-page-7.webp' },
    { href: '/host-shop/dashboard/wages', title: 'Wage compliance', detail: 'Verify actual wages against the registered occupation and employer-specific RAPIDS schedule.', image: '/images/pages/bookkeeping-ledger.webp' },
    ...
    { href: '/host-shop/dashboard/profile', title: 'Shop profile', detail: 'Maintain the approved shop profile, logo, flyer, and operating information.', image: '/images/programs/efh-barber-hero.webp' },
  ];
