  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-black uppercase tracking-[0.14em] text-brand-blue-700">{board.partner?.name || 'Host Shop'}</p><h1 className="mt-2 text-3xl font-black text-slate-950">Work Hours & Evidence</h1><p className="mt-2 text-slate-600">Approved work time is retained as supervised employment/OJL evidence. {competencyBased ? 'For Barber, Appendix A completion progress is competency-based and is shown separately.' : 'Time-based progress is calculated only where the registered program uses an hour target.'}</p></div>
