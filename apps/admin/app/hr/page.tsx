...
export default async function HRPage() {
  ...
  const [
    ...
    supabase
      .from('employees')
      .select('id, first_name, last_name, department, role, status, created_at', { count: 'exact' })
      .order('created_at', { ascending: false })
      .limit(8),
    supabase
      .from('leave_requests')
      .select('id, employee_id, leave_type, start_date, end_date, status', { count: 'exact' })
      .eq('status', 'pending')
      .limit(5),
    supabase
      .from('time_off_requests')
      .select('id, employee_id, start_date, end_date, status', { count: 'exact' })
      .eq('status', 'pending')
      .limit(5),
    supabase
      .from('payroll_runs')
    ...
    supabase
      ...
      .select('id, employee_id, review_date, rating, status')
    ...
    },
    {
      ...
      value:
        recentPayroll && recentPayroll.length > 0
          ? `$${Number((recentPayroll[0] as any).total_amount || 0).toLocaleString()}`
          : '$0',
      sub: 'last run',
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-700">
                          {emp.department || 'Unassigned'} · {emp.role || '—'}
                        </p>
                      </div>
                    </div>
                    <span
          ...
          <div className="bg-white rounded-xl shadow-sm border">
            ...
            <div className="divide-y">
              {recentPayroll && recentPayroll.length > 0 ? (
                recentPayroll.map((run: any) => (
                  <div key={run.id} className="px-5 py-3 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-slate-900">
                        {run.period_start ? new Date(run.period_start).toLocaleDateString() : '—'} –{' '}
                        {run.period_end ? new Date(run.period_end).toLocaleDateString() : '—'}
                      </p>
                      <p className="text-xs text-slate-700 mt-0.5">
                        ${Number(run.total_amount || 0).toLocaleString()} total
                      </p>
                    </div>
                    <span
