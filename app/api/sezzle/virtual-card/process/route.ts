...
async function _POST(request: NextRequest) {
  try {
    ...
    if (!sezzle.isConfigured()) {
      return NextResponse.json({ error: 'Sezzle is temporarily unavailable.' }, { status: 503 });
    }
    ...
    if (!cardToken && !cardData) {
      return NextResponse.json({ error: 'Missing card token or card data' }, { status: 400 });
    }

    if (!referenceId || !amountInCents) {
      return NextResponse.json({ error: 'Missing referenceId or amountInCents' }, { status: 400 });
    }
