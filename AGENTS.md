...
- **Build disk (`ERR_PNPM_ENOSPC`):** Set build ephemeral storage to **32768 MB (32 GB)** minimum via `pnpm tsx scripts/northflank/configure-services.ts --execute` (or `patch-ephemeral-storage.ts --execute`). PATCH must target `/services/combined/{id}` (plain `/services/{id}` returns 405). Northflank only allows 16384, 32768, 65536, … — there is no 40960. **65536** may require a higher project build-resource quota on your Northflank plan. If the UI still shows **0 MB**, refresh the build settings page after a new build starts — the list API often omits storage even when PATCH succeeded.
- **pnpm in Docker:** Use a **BuildKit cache mount** at `/pnpm/store` (not `/app/.pnpm-store` in the image layer). Never use `--package-import-method=copy`. **Do not run `pnpm fetch`**. Flow: copy workspace manifests → `pnpm install --frozen-lockfile` (with cache mount) + `pnpm store prune` → `COPY . .` → `rm -rf /app/.pnpm-store` → `next build`. See `docs/audits/northflank-pnpm-fetch-enospc-audit.md`.
...
- **Admin (Northflank):** service `elevate-admin`, Dockerfile `Dockerfile.northflank-admin`, branch `main`, readiness path `/api/ping`. Same pnpm install flow as LMS (no separate `pnpm fetch`, no `--package-import-method=copy`). Docker **HEALTHCHECK** `start-period=120s` on `/api/ping`; **`configure-services.ts`** sets startup + readiness probes.
- **Health checks (LMS + Admin):** Northflank `startupProbe` + `readinessProbe` on `http://127.0.0.1:8080/api/ping` — apply with `pnpm tsx scripts/northflank/configure-services.ts --execute` (also runs on every `deploy-lms` / `deploy-admin` workflow). Verify: `pnpm tsx scripts/northflank/verify-health-checks.ts`. Docker image HEALTHCHECK matches the same path (120s start period). After `next build`, **`node scripts/prune-admin-standalone.mjs`**. Lesson video/TTS output → **Supabase `course-videos`** (`lib/video/upload-lesson-media.ts`), not `public/generated/`. Runtime image copies slim `public/` subtrees + `remotion-src/` only. Northflank secret group `elevate-production-env`: canonical **`NORTHFLANK_API_TOKEN`** only (no `NORTHFLANK_API_KEY` / `NF_API_TOKEN` duplicates); set **`DEVSTUDIO_DEVCONTAINER_MODE=github-only`**. Sync: `pnpm tsx scripts/northflank/sync-env.ts --execute`. Build timeout vs health: `docs/audits/northflank-build-timeout-health-audit.md`.
...
- **Org SaaS plans (Solo/Business/Professional):** `lib/store/platform-pricing.ts`, `/store/plans`, checkout `POST /api/store/platform-checkout`, blueprint `docs/platform-saas-blueprint.md`.
- **SaaS licensing (Phase 1):** DB tables in `20260702000017_saas_subscription_foundation.sql`; `requireFeature(organizationId, code)` in `lib/platform/organization-features.ts`; admin `/admin/billing/*`; customer `/account/plan`, `/account/addons`.
