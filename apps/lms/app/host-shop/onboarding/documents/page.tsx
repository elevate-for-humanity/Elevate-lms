...
function statusBadge(status: string) {
  if (status === 'accepted') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 text-xs font-black text-green-800">
  ...
  if (status === 'pending') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-900">
        <Hourglass className="h-3.5 w-3.5" /> In review
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-3 py-1 text-xs font-black text-red-800">
      <AlertCircle className="h-3.5 w-3.5" /> {status === 'rejected' ? 'Rejected' : 'Required'}
    </span>
  );
}

export default async function HostShopDocumentsPage({
  searchParams,
}: {
  searchParams: Promise<{ error?: string; uploaded?: string; profile?: string }>;
}) {
      ...
      <div className="mx-auto max-w-5xl">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
          ...
          <p className="mt-2 max-w-3xl text-slate-700">
            Upload the required records for {partner.dba || partner.name}. Files are stored in the private partner-document bucket and remain pending until reviewed by Elevate.
          </p>
          <p className="mt-4 text-sm font-bold text-slate-700">
            {board.acceptedDocumentCount} of {board.requiredDocumentCount} required documents accepted
          </p>
          {isPlatformAdmin ? (
            <p className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm font-bold text-blue-950">
              Admin tenant view: uploads and replacements apply only to the selected Host Shop.
            </p>
          ) : null}
        </div>

        {params.profile === 'saved' ? (
          <div className="mt-5 rounded-xl border border-green-300 bg-green-50 p-4 font-bold text-green-950">
            Onboarding profile saved. Complete the required document uploads below.
          </div>
        ) : null}
        {params.uploaded ? (
          <div className="mt-5 rounded-xl border border-blue-300 bg-blue-50 p-4 font-bold text-blue-950">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-700">
                      {requirement.description || 'Required host-site compliance document.'}
                    </p>
                    {requirement.document?.file_name ? (
                      <p className="mt-2 text-xs font-semibold text-slate-600">Latest file: {requirement.document.file_name}</p>
                    ) : null}
                    {requirement.document?.rejection_reason ? (
                      <p className="mt-2 text-sm font-bold text-red-800">Review note: {requirement.document.rejection_reason}</p>
                    ) : null}
                  </div>
                  {statusBadge(status)}
                </div>
                  ...
                  <form action={uploadHostShopDocument} className="mt-5 grid gap-4 border-t border-slate-200 pt-5 sm:grid-cols-[1fr_auto] sm:items-end">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <label className="font-bold">
                        File *
                        <input type="file" name="file" required accept=".pdf,image/jpeg,image/png,image/webp" className="mt-2 block w-full rounded-xl border border-slate-300 bg-slate-50 p-3 text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-slate-950 file:px-3 file:py-2 file:font-bold file:text-white" />
                      </label>
                      {requirement.requires_expiration ? (
                        <label className="font-bold">
                          Expiration date *
                          <input type="date" name="expirationDate" required className="mt-2 w-full rounded-xl border border-slate-400 px-4 py-3 font-medium" />
                        </label>
                      ) : null}
                    </div>
                    <button type="submit" className="min-h-11 rounded-xl bg-blue-700 px-5 py-3 font-black text-white hover:bg-blue-800">
                      Upload
                    </button>
                  </form>
                ) : null}
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6">
          {allRequiredAccepted && partner.mou_signed && partner.onboarding_completed ? (
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="font-black text-green-800">Host Shop onboarding requirements are complete.</p>
                <p className="mt-1 text-sm text-slate-700">Return to the operational board to manage apprentices and OJT hours.</p>
              </div>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link href="/host-shop/dashboard/board" className="rounded-xl border border-slate-300 px-5 py-3 text-center font-bold hover:bg-slate-50">
                Return to dashboard
              </Link>
            </div>
          )}
