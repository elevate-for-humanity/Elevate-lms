...
export async function GET(request: NextRequest) {
  return capabilityHealthResponse(request, async () => {
    ...
    try {
      ...
      directoryMessage = directoryTablesPassed
        ? 'Team and training-partner tables are accessible.'
        : 'A website directory table query failed.';
    ...
    return buildCapabilityHealth('content', [
      ...
      },
      {
        ...
        required: true,
        message: 'At least one AI content provider must be configured.',
      },
      {
        name: 'ai-memory',
        passed: memoryPassed,
        required: true,
        message: memoryPassed ? 'AI memory is accessible.' : 'AI memory is unavailable.',
      },
      {
        name: 'social-campaigns',
        passed: campaignsPassed,
        required: false,
        message: campaignsPassed
          ? 'Social campaigns are accessible.'
          : 'Social campaigns are unavailable.',
      },
      {
        name: 'email-provider',
        passed: Boolean(process.env.RESEND_API_KEY || process.env.SENDGRID_API_KEY),
        required: false,
        message: 'Email provider configuration checked.',
      },
    ]);
  });
}
