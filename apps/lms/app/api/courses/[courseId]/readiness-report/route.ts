...
export async function GET(
  ...
  const [
    ...
    db
      .from('course_lessons')
      .select('id,title,domain_key,learning_objectives,content_json,content')
      .eq('course_id', courseId),
    db
      .from('interaction_progress')
      .select('lesson_id,score,completed,attempts,weak_objectives,updated_at')
      .eq('learner_id', user.id)
      .eq('course_id', courseId),
    db
      .from('practice_attempts')
      .select('attempt_number,score,domain_scores,completed_at,status,time_spent_seconds')
      .eq('user_id', user.id)
      .eq('course_id', courseId)
      .eq('status', 'completed')
  ...
  const domainThresholds = resolveDomainMasteryThresholds(lessons ?? []);
  ...
  for (const row of interactions ?? []) {
    ...
    if (!(row as any).completed || score < threshold) {
  ...
  if (latestPractice?.domain_scores && typeof latestPractice.domain_scores === 'object') {
    for (const [domain, raw] of Object.entries(
  ...
  const suggestedMasteryThreshold = domainThresholds.size
    ? Math.max(...domainThresholds.values())
    : 80;
  ...
  const interactionAverage = interactionScores.length
    ? interactionScores.reduce((sum: number, value: number) => sum + value, 0) /
      interactionScores.length
    : 0;
  const practiceScore = Number(latestPractice?.score ?? 0);
  const overallScore = clamp(
    Math.round(
      latestPractice ? interactionAverage * 0.4 + practiceScore * 0.6 : interactionAverage,
    ),
  );

  const weakDomains = Object.entries(domainBreakdown)
    .filter(([domain, score]) => Number(score) < (domainThresholds.get(domain) ?? 80))
    .map(([domain]) => domain);
  ...
  if ((attempts ?? []).length === 0)
    recommendations.push('Complete a full practice assessment to strengthen readiness evidence.');
  if (weakDomains.length)
    recommendations.push(`Focused review required in: ${weakDomains.join(', ')}.`);
  if (weakObjectives.size)
    recommendations.push(
      `Review ${weakObjectives.size} missed objective${weakObjectives.size === 1 ? '' : 's'} before the next attempt.`,
    );
  if (!recommendations.length)
    recommendations.push(
      'Continue spaced review and complete another practice assessment before certification testing.',
    );

  const confidenceLevel =
    (attempts ?? []).length >= 2 && domainValues.length >= 2 ? 'moderate' : 'low';
  const report = {
    user_id: user.id,
    course_id: courseId,
  ...
  if (saveError)
    return NextResponse.json({ error: 'Unable to persist readiness report' }, { status: 500 });
