    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="space-y-6">
              <div>
                ...
                <input type="text" required value={formData.topic} onChange={(e) => setFormData({ ...formData, topic: e.target.value })} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-red-500 focus:border-transparent" placeholder="e.g., HVAC Fundamentals, Medical Billing Basics, CDL Test Preparation" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-900 mb-2">Course Description</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-red-500 focus:border-transparent" placeholder="Describe what students will learn and achieve..." />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-900 mb-2">Target Audience</label>
                <input type="text" value={formData.targetAudience} onChange={(e) => setFormData({ ...formData, targetAudience: e.target.value })} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-red-500 focus:border-transparent" placeholder="e.g., Career changers, Entry-level workers, Experienced professionals" />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select value={formData.difficulty} onChange={(e) => setFormData({ ...formData, difficulty: e.target.value })} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-red-500 focus:border-transparent">
                ...
                <div>
                  ...
                  <input type="number" min={1} max={100} value={formData.duration} onChange={(e) => setFormData({ ...formData, duration: parseInt(e.target.value) || 10 })} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-red-500 focus:border-transparent" />
                </div>
                ...
                <div>
                  ...
                  <input type="number" min={1} max={20} value={formData.moduleCount} onChange={(e) => setFormData({ ...formData, moduleCount: parseInt(e.target.value) || 5 })} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-red-500 focus:border-transparent" />
                </div>
              </div>

              <div className="flex flex-wrap gap-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={formData.includeQuizzes} onChange={(e) => setFormData({ ...formData, includeQuizzes: e.target.checked })} className="w-5 h-5 rounded border-gray-300 text-brand-red-600 focus:ring-brand-red-500" />
                  <HelpCircle className="w-4 h-4 text-gray-500" />
                  <span className="text-gray-700">Include Quizzes</span>
                </label>

                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={formData.includeAssignments} onChange={(e) => setFormData({ ...formData, includeAssignments: e.target.checked })} className="w-5 h-5 rounded border-gray-300 text-brand-red-600 focus:ring-brand-red-500" />
                  <FileText className="w-4 h-4 text-gray-500" />
                  <span className="text-gray-700">Include Assignments</span>
                </label>
              </div>

              <button type="submit" disabled={loading || !formData.topic} className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-brand-red-600 text-white rounded-lg font-semibold hover:bg-brand-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                {loading ? (
                  <>
        ...
        <div className="mt-8 bg-brand-red-50 border border-brand-red-200 rounded-xl p-6">
          <h3 className="font-semibold text-brand-red-900 mb-2 flex items-center gap-2">
            <GraduationCap className="w-5 h-5" />
            What Gets Generated
          </h3>
          <ul className="text-brand-red-800 space-y-1 text-sm">
