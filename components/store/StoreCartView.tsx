    ...
    try {
      ...
      if (individual) {
        if (cart.items.length !== 1)
          throw new Error('App subscriptions must be checked out separately from store products.');
        ...
        if (!response.ok || !data.checkoutUrl)
          throw new Error(data.error || 'Could not start app checkout');
        clearCart();
      ...
      const response = await fetch('/api/store/cart-checkout', {
        ...
        body: JSON.stringify({
          ...
          checkoutAttemptId: checkoutAttempt.current,
        }),
      });
      if (response.status === 401) {
        redirectToLogin();
      ...
      {!cart.items.length ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
          ...
          <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            <Link
              href="/store"
              className="rounded-xl bg-brand-red-600 px-6 py-3 font-bold text-white"
            >
              Browse Store
            </Link>
            <Link
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
            ...
            {cart.items.map((item) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <h2 className="font-black text-slate-950">{item.product.name}</h2>
          ...
          <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-6 lg:sticky lg:top-24">
            ...
            <div className="mt-5 flex justify-between text-slate-700">
            ...
            <button
              ...
              className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-brand-red-600 py-3.5 font-black text-white hover:bg-brand-red-700 disabled:opacity-60"
            >
              {checkingOut ? (
              ...
              {checkingOut ? 'Opening secure checkout…' : 'Proceed to Checkout'}
            </button>
