    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm">
                Approved atomic facts — the only values inserted into generated documents
              </p>
            </div>
          </div>
          <Link
        ...
        {error && (
          <div className="mb-5 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
            ...
            <p className="text-sm text-amber-800">
              Facts Vault tables not yet applied. Run migrations 20260527000005 in Supabase
              Dashboard.
            </p>
          </div>
        )}
          ...
          {!facts || facts.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-slate-500 mb-4">
                Add approved facts to use as merge fields in generated documents.
              </p>
              <Link
                href="/submissions/facts"
                className="inline-flex items-center gap-2 px-4 py-2 bg-brand-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-brand-blue-700 transition"
                ...
                {(facts as any[]).map((fact) => (
                  <tr key={fact.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_STYLES[fact.status] ?? 'bg-slate-100 text-slate-600'}`}
                      >
                        {fact.status}
                      </span>
                    </td>
        ...
        <p className="text-xs text-slate-400 mt-4">
          Only <strong>approved</strong> facts are inserted into generated documents. Draft and
          pending facts are visible here but blocked from packet generation.
        </p>
      </div>
    </div>
  );
