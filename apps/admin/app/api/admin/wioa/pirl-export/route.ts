...
export async function POST(request: NextRequest) {
  ...
  if (createErr || !job) {
    return safeDbError(createErr ?? new Error('Insert failed'), 'Failed to create export job');
  }
  ...
  try {
    ...
    const reportJson = JSON.parse(await fs.readFile(result.reportFilePath, 'utf8'));
