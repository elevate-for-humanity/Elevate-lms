...
function password() {
  return `Qa!${randomBytes(18).toString('base64url')}9x`;
}
...
async function provision() {
  ...
  let state = {
    ...
    qaProgramId: null,
    holderId: null,
    holderProgramId: null,
  };

  try {
    const learner = await createQaUser('learner', 'student', '[QA E2E] Learner', anchor.tenant_id);
    state.learner = learner;

    const programHolder = await createQaUser(
      'program-holder',
      'program_holder',
      '[QA E2E] Program Holder',
      anchor.tenant_id,
    );
    state.programHolder = programHolder;

    const qaProgram = await must(
      db.from('programs').insert({
        slug: `${marker}-program`,
        title: '[QA E2E] Isolated Program Holder Program',
        name: '[QA E2E] Isolated Program Holder Program',
        category: 'qa',
        description: 'Disposable isolated program used only for responsive production QA.',
        is_active: false,
        published: false,
        status: 'draft',
        review_status: 'draft',
      ...
      'create isolated QA program',
    );
    state.qaProgramId = qaProgram.id;

    const holder = await must(
      db.from('program_holders').insert({
        user_id: programHolder.id,
        organization_name: '[QA E2E] Isolated Program Holder',
        name: '[QA E2E] Program Holder',
        contact_name: '[QA E2E] Program Holder',
        contact_email: programHolder.email,
        status: 'approved',
        ...
        primary_program_id: qaProgram.id,
        teaches_multiple: false,
        features: { qa_e2e: true, qa_run_id: runId, isolated: true },
      }).select('id').single(),
      'create isolated QA program holder',
    );
    state.holderId = holder.id;

    await must(
      db.from('profiles').update({ program_holder_id: holder.id }).eq('id', programHolder.id),
      'link QA profile to program holder',
    );

    const holderProgram = await must(
      db.from('program_holder_programs').insert({
        program_holder_id: holder.id,
        program_id: qaProgram.id,
        program_slug: qaProgram.slug,
        role_in_program: 'owner',
        is_primary: true,
        status: 'active',
      }).select('id').single(),
      'associate isolated QA program',
    );
    state.holderProgramId = holderProgram.id;

    await writeFile(statePath, JSON.stringify(state, null, 2));

    if (process.env.GITHUB_ENV) {
      const lines = [
        `E2E_LEARNER_EMAIL=${learner.email}`,
        `E2E_LEARNER_PASSWORD=${learner.password}`,
        `E2E_PROGRAM_HOLDER_EMAIL=${programHolder.email}`,
        `E2E_PROGRAM_HOLDER_PASSWORD=${programHolder.password}`,
      ];
      await writeFile(process.env.GITHUB_ENV, `${lines.join('\n')}\n`, { flag: 'a' });
      console.log(`::add-mask::${learner.password}`);
      console.log(`::add-mask::${programHolder.password}`);
    }

    console.log(JSON.stringify({
      ok: true,
      learnerUserId: learner.id,
      programHolderUserId: programHolder.id,
      holderId: holder.id,
      qaProgramId: qaProgram.id,
    }));
  } catch (error) {
...
async function cleanup() {
  ...
  try {
    ...
    console.log('No Learner/Program Holder QA state file found; nothing to clean up.');
    return;
  }
