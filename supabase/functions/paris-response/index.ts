...
async function getApiKey(supabase: ReturnType<typeof createClient>): Promise<string> {
  ...
  const { data } = await supabase
    ...
    .single();
  if (!data?.value_enc) throw new Error('SendGrid API key not found');
  return data.value_enc;
}

function sendEmail(to: string, subject: string, html: string, apiKey: string): Promise<boolean> {
...
function parseIntent(text: string): 'interested' | 'not_interested' | 'question' | 'unknown' {
  ...
  if (/no[t\s]interested|not interested|i\s*don'?t\s*want|withdraw|cancel/i.test(lower))
    return 'not_interested';
  if (
    /interested|yes|let'?s\s*do\s*it|move\s*forward|enroll|start|begin|schedule|call\s*you|counting\s*me/i.test(
      lower,
    )
  )
    return 'interested';
  if (/\?|how|what|can\s*i|tell\s*me|explain|want\s*to\s*know|unsure|confused/i.test(lower))
    return 'question';
  return 'unknown';
}
...
function getSignupUrl(program: string): string {
  if (program === 'employer-partnership')
    return 'https://www.elevateforhumanity.org/partners/host-shop/apply';
  return `https://www.elevateforhumanity.org/apply?program=${encodeURIComponent(program)}`;
}

function buildInterestedEmail(app: Application): string {
  const programName = getProgramName(app.program_interest);
  const signupUrl = getSignupUrl(app.program_interest);
  return `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;"><div style="background:#f97316;color:white;padding:30px;text-align:center;border-radius:8px 8px 0 0;"><h1 style="margin:0;">You're Ready to Move Forward!</h1></div><div style="padding:30px;background:#f9fafb;border:1px solid #e5e7eb;border-top:none;"><p>Hi ${app.first_name},</p><p>Thank you for confirming your interest in <strong>${programName}</strong>.</p><p><a href="${signupUrl}" style="display:inline-block;background:#1d4ed8;color:#fff;padding:14px 22px;border-radius:8px;text-decoration:none;font-weight:bold;">Continue ${programName} signup</a></p><p>After completing the signup, finish your enrollment interview at <a href="https://www.elevateforhumanity.org/interview">elevateforhumanity.org/interview</a>.</p><p>If you are seeking WorkOne funding, contact your nearest WorkOne office. Funding is limited, eligibility is determined by WorkOne, and no funding is guaranteed.</p><p>Reply with your question or call <a href="tel:317-314-3757">317-314-3757</a>. Reference: <strong>${app.reference_number}</strong></p></div><div style="text-align:center;padding:20px;color:#6b7280;font-size:12px;"><p>PARIS | Elevate AI Advisor</p></div></div>`;
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!email) {
      ...
      for (const app of apps || []) {
        const success = await sendEmail(
          app.email,
          `Action Required: Your Enrollment Next Steps [Ref: ${app.reference_number}]`,
          buildFollowUpEmail(app),
          apiKey,
        );
        if (success) {
          await supabase
            .from('applications')
            .update({ status: 'pending_workone' })
            .eq('id', app.id);
          sent++;
        } else failed++;
        await new Promise((r) => setTimeout(r, 250));
      }
      return new Response(JSON.stringify({ sent, failed, total: apps?.length || 0 }), {
    ...
    switch (intent) {
      case 'not_interested':
        subject = `Application Update - Elevate for Humanity [Ref: ${app.reference_number}]`;
        html = buildNotInterestedEmail(app);
        newStatus = 'withdrawn';
        break;
      case 'interested':
        subject = `You're Ready to Move Forward! [Ref: ${app.reference_number}]`;
        html = buildInterestedEmail(app);
        newStatus = 'under_review';
        break;
      case 'question':
        subject = `Thanks for Your Question - Elevate for Humanity [Ref: ${app.reference_number}]`;
        html = buildQuestionEmail(app, text || reply_subject || 'Your question');
        newStatus = 'under_review';
        break;
      default:
        subject = `Thanks for Your Response - Elevate for Humanity [Ref: ${app.reference_number}]`;
        html = buildFollowUpEmail(app);
    }
    const success = await sendEmail(email, subject, html, apiKey);
    if (success) await supabase.from('applications').update({ status: newStatus }).eq('id', app.id);
    return new Response(JSON.stringify({ success, intent, email, subject, status: newStatus }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
