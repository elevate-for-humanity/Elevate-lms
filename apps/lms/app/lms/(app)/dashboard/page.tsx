...
export default async function StudentDashboard() {
  ...
  const [
    ...
    supabase
      ...
      .select('id, status, course_id, created_at, completed_at')
  ...
  const activeCourseEnrollment =
    courseEnrollments.find((row) => ['active', 'enrolled', 'in_progress'].includes(String(row.status))) ??
    courseEnrollments[0] ??
    null;
  ...
  const externalCourses = (externalCoursesRes.data ?? []).filter((course: any) =>
    enrolledProgramIds.has(course.program_id),
  );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6">
        {activeCourseEnrollment && activeCourseId ? (
          <section className="overflow-hidden rounded-3xl bg-slate-950 text-white">
            <div className="p-6 sm:p-8">
              ...
              {totalLessons > 0 && (
                <div className="mt-6 max-w-3xl">
                  <div className="mb-2 flex items-center justify-between text-xs font-bold">
                    <span className="text-slate-300">{completedLessons} of {totalLessons} verified lessons complete</span>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                ...
                <Link href={`/lms/courses/${activeCourseId}`} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/25 px-5 py-3 text-sm font-bold text-white hover:bg-white/10">
          ...
          <section className="rounded-3xl border border-blue-200 bg-blue-50 p-7 sm:p-9">
            <p className="text-xs font-black uppercase tracking-[0.15em] text-blue-800">Welcome, {firstName}</p>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Link href="/lms/apply/status" className="rounded-xl border border-blue-300 bg-white px-5 py-3 font-bold text-blue-950 hover:bg-blue-100">Application Status</Link>
            </div>
          </section>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            {programEnrollments.length > 0 && (
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                  <h2 className="flex items-center gap-2 font-black text-slate-950"><BookOpen className="h-4 w-4" /> My Programs</h2>
                  <Link href="/lms/courses" className="text-sm font-bold text-brand-blue-700 hover:underline">View courses</Link>
                </div>
                <div className="divide-y divide-slate-100">
                  {programEnrollments.slice(0, 6).map((enrollment: any) => (
                    <div key={enrollment.id} className="flex items-center justify-between gap-4 px-5 py-4">
                      <div>
                        <p className="font-bold text-slate-950">{enrollment.programs?.title ?? 'Program'}</p>
                        <p className="mt-1 text-xs font-semibold capitalize text-slate-600">Program status: {String(enrollment.status ?? 'recorded').replace(/_/g, ' ')}</p>
                      </div>
                      <GraduationCap className="h-5 w-5 shrink-0 text-brand-blue-700" />
                    </div>
                  ))}
                </div>
              </section>
            ...
            {externalCourses.length > 0 && (
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <div className="border-b border-slate-100 px-5 py-4">
                  ...
                  <p className="mt-1 text-xs text-slate-600">External course content stays on the provider platform; Elevate tracks your completion evidence.</p>
                </div>
                <div className="divide-y divide-slate-100">
                      ...
                      <div key={course.id} className="px-5 py-4">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-slate-600">{course.partner_name}{course.credential_type ? ` · ${course.credential_type}` : ''}</p>
                          </div>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          ...
                          {!approved && (
                            <Link href={`/lms/external-pathways/${course.id}/upload`} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-800 hover:bg-slate-50">
                              <Upload className="h-3.5 w-3.5" /> {uploaded ? 'Replace Certificate' : 'Upload Certificate'}
                            </Link>
                          )}
                          <span className={`inline-flex items-center rounded-lg px-3 py-2 text-xs font-bold ${approved ? 'bg-green-100 text-green-800' : uploaded ? 'bg-amber-100 text-amber-900' : 'bg-slate-100 text-slate-700'}`}>
                ...
                <div className="divide-y divide-slate-100">
                  {recentQuizAttempts.map((attempt: any) => (
                    <div key={attempt.id} className="flex items-center gap-4 px-5 py-4">
                      <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl font-black ${attempt.passed ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {Number(attempt.score ?? 0)}%
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-bold text-slate-950">{attempt.quizzes?.title ?? 'Practice Assessment'}</p>
                        <p className="mt-1 text-xs font-semibold text-slate-600">{attempt.passed ? 'Passed' : 'Not passed'}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                {[
                  ['/lms/courses', 'My Courses', BookOpen],
                  ['/lms/assignments', 'Assignments', Target],
                  ['/lms/certificates', 'Certificates', Award],
                  ['/lms/calendar', 'Schedule', Clock],
                  ['/lms/messages', 'Messages', MessageSquare],
                  ['/lms/support', 'Get Help', HelpCircle],
                ].map(([href, label, Icon]) => {
                  const ToolIcon = Icon as typeof BookOpen;
                  return (
                    <Link key={String(href)} href={String(href)} className="flex min-h-12 items-center gap-3 rounded-xl border border-slate-200 p-3 font-bold text-slate-800 hover:bg-slate-50">
                      <ToolIcon className="h-4 w-4 text-brand-blue-700" /> {String(label)}
                    </Link>
                  );
                })}
              </div>
          ...
          <aside className="space-y-5">
                ...
                <div className="mt-4 space-y-3">
                  {certifications.slice(0, 4).map((cert: any) => (
                    <div key={cert.id} className="rounded-xl bg-white p-3">
                      ...
                      {cert.issued_at ? <p className="mt-1 text-xs text-slate-600">{new Date(cert.issued_at).toLocaleDateString()}</p> : null}
                    </div>
                  ))}
                </div>
                <Link href="/lms/certificates" className="mt-4 inline-flex text-sm font-bold text-amber-900 hover:underline">View all certificates</Link>
              </section>
            )}
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
              ...
              <div className="p-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Recorded paid total</p>
                ...
                <div className="mt-4 space-y-2">
                  {recentPayments.slice(0, 3).map((payment: any) => (
                    <div key={payment.id} className="flex items-center justify-between gap-3 text-xs">
                      <span className="capitalize text-slate-600">{payment.status ?? 'recorded'}</span>
                      <span className="font-bold text-slate-900">${(Number(payment.amount ?? 0) / 100).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
                <Link href="/lms/payments" className="mt-4 inline-flex text-sm font-bold text-brand-blue-700 hover:underline">View payment history</Link>
              </div>
            </section>

            <section className="rounded-2xl bg-slate-950 p-5 text-white">
              <h2 className="font-black">Credential Pathway</h2>
              <div className="mt-4 space-y-3 text-sm">
...
function PathStep({ label, done }: { label: string; done: boolean }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={done ? 'text-green-300' : 'text-slate-300'}>{label}</span>
    </div>
  );
}
