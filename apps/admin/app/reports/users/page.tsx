...
export default async function UsersReportPage() {
  ...
  const roleColors: Record<string, { bg: string; text: string; icon: typeof Users }> = {
    ...
    super_admin: { bg: 'bg-brand-blue-100', text: 'text-brand-blue-700', icon: Shield },
  ...
  return (
    <div className="min-h-screen bg-white p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <Link href="/reports" className="inline-flex items-center gap-2 text-slate-700 hover:text-slate-900 mb-4">
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-brand-blue-100 rounded-lg"><Users className="w-5 h-5 text-brand-blue-600" /></div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-brand-green-100 rounded-lg"><UserPlus className="w-5 h-5 text-brand-green-600" /></div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-brand-blue-100 rounded-lg"><Shield className="w-5 h-5 text-brand-blue-600" /></div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-brand-orange-100 rounded-lg"><GraduationCap className="w-5 h-5 text-brand-orange-600" /></div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/students" className="text-sm text-brand-blue-600 hover:text-brand-blue-700">View All</Link>
            </div>
                      ...
                      return (
                        <tr key={user.id} className="border-b last:border-0">
                          <td className="py-3 px-2">
                            <p className="font-medium text-slate-900">{user.first_name || 'Unknown'} {user.last_name || ''}</p>
                            <p className="text-sm text-slate-700">{user.email}</p>
                          </td>
                          <td className="py-3 px-2">
                            <span className={`px-2 py-1 text-xs rounded-full ${roleStyle.bg} ${roleStyle.text}`}>{roleLabels[role] || role}</span>
                          </td>
                          <td className="py-3 px-2 text-sm text-slate-700">{new Date(user.created_at).toLocaleDateString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                {Object.entries(usersByRole).sort((a, b) => b[1] - a[1]).map(([role, count]) => {
                  ...
                  return (
                    <div key={role} className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${roleStyle.bg}`}><Icon className={`w-4 h-4 ${roleStyle.text}`} /></div>
                      <div className="flex-1">
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-2">
                          <div className={`h-2 rounded-full ${roleStyle.bg.replace('100', '500')}`} style={{ width: `${(count / (totalUsers || 1)) * 100}%` }} />
                        </div>
                      </div>
                    </div>
                  );
