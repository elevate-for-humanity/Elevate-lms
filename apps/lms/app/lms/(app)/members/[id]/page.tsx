...
export default async function MemberProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: member } = await supabase
    .from('profiles')
    .select('id,full_name,avatar_url,role,community_visible,community_allow_messages,community_allow_follow,community_show_role')
    .eq('id', id)
    .maybeSingle();

  const canView = !!member && (member.id === user.id || member.community_visible === true);
  let following = false;
  if (canView && member && member.id !== user.id && member.community_allow_follow) {
      ...
      {!canView || !member ? (
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1"><h1 className="text-3xl font-black text-slate-900">{member.full_name ?? 'Elevate Member'}</h1>{member.community_show_role && <p className="mt-1 capitalize text-slate-500">{member.role ?? 'student'}</p>}</div>
          </div>
          ...
          {member.id === user.id ? (
            ...
            <div className="mt-7 flex flex-wrap gap-3 border-t border-slate-100 pt-5">
              {member.community_allow_messages && <Link href={`/lms/messages?recipient=${member.id}`} className="inline-flex items-center gap-2 rounded-xl bg-brand-blue-600 px-5 py-3 font-bold text-white hover:bg-brand-blue-700"><MessageSquare className="h-4 w-4" />Message member</Link>}
