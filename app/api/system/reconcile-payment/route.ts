...
async function reconcileApplication(db: SupabaseClient, applicationId: string, actorId: string) {
  // 1. Load application
    ...
    try {
      ...
      if (match) {
        ...
        stripeSession = {
          ...
          created_at: new Date(match.created * 1000).toISOString(),
        };
        logger.info('[reconcile] Found Stripe session live, upserted to staging', {
          applicationId,
          sessionId: match.id,
        });
      }
