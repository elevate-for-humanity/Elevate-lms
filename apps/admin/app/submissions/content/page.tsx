...
export const metadata: Metadata = {
  robots: { index: false }, title: 'Content Library | Submissions OS | Admin' };
        ...
        {!blocks || blocks.length === 0 ? (
          ...
          <div className="space-y-3">
            {(blocks as any[]).map((block) => (
              <div
                key={block.id}
                className="bg-white rounded-xl border border-slate-200 p-5 hover:border-brand-blue-200 transition"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-semibold ${STATUS_CLS[block.status] ?? 'bg-slate-100 text-slate-600'}`}
                      >
                        {block.status}
                      </span>
                    </div>
