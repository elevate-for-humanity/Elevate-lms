...
export default function EnrollmentManagementClient({
  initialEnrollments,
  users,
  courses,
  cohorts,
  ...
  const handleDelete = async (enrollmentId: string) => {
    if (
      !confirm(
        'Are you sure you want to delete this enrollment? This will also remove all progress data.',
      )
    )
      return;
  ...
  const handleToggleAtRisk = async (enrollment: Enrollment) => {
    try {
      ...
      setEnrollments(
        enrollments.map((e) => (e.id === enrollment.id ? { ...e, at_risk: !e.at_risk } : e)),
      );
    } catch (err: any) {
      setError(err.message || 'An error occurred');
    }
  ...
  const markComplete = async (enrollment: Enrollment) => {
    try {
      ...
      setEnrollments(
        enrollments.map((e) =>
          e.id === enrollment.id ? { ...e, status: 'completed', progress: 100 } : e,
        ),
      );
    } catch (err: any) {
      setError(err.message || 'An error occurred');
  ...
  const handleApproveEnrollment = async (enrollment: Enrollment) => {
    try {
      ...
      setEnrollments(
        enrollments.map((e) => (e.id === enrollment.id ? { ...e, status: 'active' } : e)),
      );
      ...
      setEnrollments(
        enrollments.map((e) =>
          e.id === enrollment.id
            ? { ...e, status: 'active', approved_at: new Date().toISOString() }
            : e,
        ),
      );
    } catch (err: any) {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="px-6 py-3 bg-brand-blue-600 text-white rounded-lg hover:bg-brand-blue-700 font-medium flex items-center gap-2"
        >
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {enrollment.at_risk && (
                        <span className="px-2 py-1 rounded-full text-xs font-medium bg-brand-red-100 text-brand-red-700">
                          At Risk
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex justify-end gap-2">
                      {enrollment.status === 'pending_approval' && (
                        <button
                          ...
                          className="px-3 py-1 text-sm text-white bg-brand-green-600 rounded hover:bg-brand-green-700 font-medium"
                        >
                          Approve
                        </button>
                      )}
                      ...
                      {enrollment.status !== 'completed' &&
                        enrollment.status !== 'pending_approval' && (
                          <button
                            onClick={() => markComplete(enrollment)}
                            className="px-3 py-1 text-sm text-brand-green-600 border border-brand-green-200 rounded hover:bg-brand-green-50"
                          >
                            Complete
                          </button>
                        )}
                      ...
                      <button
                        ...
                        className="px-3 py-1 text-sm text-brand-red-600 border border-brand-red-200 rounded hover:bg-brand-red-50"
                      >
                        Delete
                      </button>
                    </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full mx-4">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="at_risk" className="text-sm text-slate-900">
                  Flag as at-risk
                </label>
              </div>
