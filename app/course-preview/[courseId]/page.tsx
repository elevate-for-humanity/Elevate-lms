...
const CREDENTIAL_MAP: Record<string, { credential: string; issuer: string; icon: typeof Award }> = {
  ...
  welding: {
    credential: 'AWS D1.1 Structural Welding',
    issuer: 'American Welding Society',
    icon: Wrench,
  },
  electrical: {
    credential: 'OSHA 10-Hour Construction Safety',
  ...
  cdl: {
    credential: 'Commercial Driver License (CDL) Class A',
  ...
  trades: {
    credential: 'OSHA 10-Hour + NCCER Core Curriculum',
    issuer: 'OSHA / NCCER',
    icon: Wrench,
  },
};

function getCredentialForCourse(
  course: any,
): { credential: string; issuer: string; icon: typeof Award } | null {
  if (!course) return null;
  const text =
    `${course.title || ''} ${course.course_name || ''} ${course.category || ''} ${course.certification_name || ''}`.toLowerCase();
...
function getCompetencies(course: any): string[] {
  ...
  const text =
    `${course.title || ''} ${course.course_name || ''} ${course.category || ''}`.toLowerCase();
...
export default function CoursePreviewPage() {
  ...
  const progressPct =
    lessons.length > 0 ? Math.round((viewedLessons.size / lessons.length) * 100) : 0;
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex items-center gap-4 text-xs text-slate-400">
                ...
                <span className="flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5" /> {totalHours} Hours
                </span>
                <span className="flex items-center gap-1">
                  <BarChart3 className="w-3.5 h-3.5" /> {course.difficulty || 'All Levels'}
                </span>
              </div>
          ...
          <div className="border-t border-slate-800 bg-white/80">
            <div className="max-w-7xl mx-auto px-6 py-2.5 flex items-center gap-3">
              ...
              <p className="text-xs text-slate-600">
                <span className="text-amber-400 font-semibold">Credential Alignment:</span> This
                course prepares you for{' '}
                <span className="text-white font-medium">{credential.credential}</span>{' '}
          ...
          <div className="p-4">
            ...
            <div className="mb-4 pb-4 border-b border-slate-800">
              <p className="text-xs text-slate-500 mb-1">
                {viewedLessons.size} of {lessons.length} viewed
              </p>
                  ...
                  >
                    <span
                      className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                        isViewed && !isActive
                          ? 'bg-brand-green-600/20 text-brand-green-400'
                          : isActive
                ...
                {activeIdx > 0 ? (
                  <button
                    onClick={() => {
                      ...
                      markViewed(lessons[activeIdx - 1].id);
                    }}
                ...
                <div className="mt-8 bg-white/50 rounded-xl border border-amber-500/20 p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/10">
                      <Award aria-label="award" className="w-5 h-5 text-amber-400" />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold">Competency Checklist</h3>
                      <p className="text-xs text-slate-500">
                        Skills demonstrated upon course completion
                      </p>
                    </div>
                  </div>
                  ...
                  {credential && (
                    <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-500">
                      Successful completion prepares learners for:{' '}
                      <span className="text-amber-400">{credential.credential}</span>
                    </div>
                  )}
                </div>
              )}
