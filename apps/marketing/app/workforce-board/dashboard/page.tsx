...
export default async function WorkforceBoardDashboard() {
  ...
  if (!platformWide && !scopeId) {
    return (
      <main className="w-full max-w-none p-6 sm:p-8">
        <section
          role="alert"
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm font-medium leading-6">
                This account has Workforce Board access but is not linked to an organization or
                tenant. Regional metrics are blocked rather than falling back to platform-wide
                participant data.
              </p>
            </div>
          </div>
  ...
  if (enrollmentResult.error)
    throw new Error(`WORKFORCE_BOARD_ENROLLMENTS_FAILED:${enrollmentResult.error.message}`);
  if (programResult.error)
    throw new Error(`WORKFORCE_BOARD_PROGRAMS_FAILED:${programResult.error.message}`);

  const enrollments = enrollmentResult.data ?? [];
  const programs = programResult.data ?? [];
  const learnerIds = [
    ...new Set(enrollments.map((row: any) => row.user_id).filter(Boolean)),
  ] as string[];
  const programIds = [
    ...new Set(enrollments.map((row: any) => row.program_id).filter(Boolean)),
  ] as string[];
  ...
  if (programRowsError)
    throw new Error(`WORKFORCE_BOARD_PROGRAM_DETAILS_FAILED:${programRowsError.message}`);
  if (credentialResult.error)
    throw new Error(`WORKFORCE_BOARD_CREDENTIALS_FAILED:${credentialResult.error.message}`);
  ...
  const completedEnrollments = enrollments.filter(
    (row: any) => enrollmentState(row) === 'completed',
  ).length;
  const activeEnrollments = enrollments.filter((row: any) =>
    ['active', 'enrolled', 'in_progress'].includes(enrollmentState(row)),
  ).length;
  const activePrograms = programs.filter((row: any) =>
    ['active', 'published'].includes(String(row.status || '').toLowerCase()),
  ).length;
  ...
  return (
    <main className="space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <Link
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-700 px-5 py-3 text-sm font-black text-white hover:bg-indigo-800"
          >
      ...
      <section
        ...
        aria-label="Workforce Board performance metrics"
      >
      ...
      <section className="grid gap-5 lg:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm font-medium text-slate-600">
            Programs inside the authorized reporting scope.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
          ...
          <p className="mt-1 text-sm font-medium text-slate-600">
            All scoped enrollment records, including completed and inactive records.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
          ...
          <p className="mt-2 text-lg font-black">
            {platformWide ? 'Platform-wide admin' : 'Organization scoped'}
          </p>
          <p className="mt-1 break-all text-xs font-medium text-slate-500">
            {platformWide ? 'Administrator override' : scopeId}
          </p>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-5 py-4">
          <h2 className="font-black text-slate-950">Recent enrollments</h2>
        </div>
        {recentEnrollments.length ? (
                ...
                >
                  <div>
                    <p className="font-bold text-slate-900">
                      {profileMap[enrollment.user_id]?.full_name || 'Participant'}
                    </p>
                  ...
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700">
                    {enrollmentState(enrollment) || 'unknown'}
                  </span>
                </div>
              );
            })}
          ...
          <p className="p-8 text-center text-sm font-medium text-slate-500">
            No enrollments exist inside this reporting scope.
          </p>
        )}
      </section>
    </main>
