...
export default async function HostShopSchedulePage() {
  ...
  const { data: theorySchedules, error: theoryError } = apprenticeIds.length
    ? await (db as any).from('apprenticeship_theory_schedules')
        .select('id,user_id,program_slug,course_id,timezone,days_of_week,start_time,end_time,starts_on,weekly_target_minutes,weekly_max_minutes,active')
        ...
        <div className="mt-5 grid gap-4">
          {(theorySchedules || []).map((schedule: any) => {
            ...
            return <article key={schedule.id} className="rounded-2xl border border-fuchsia-200 bg-white p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="text-lg font-black text-slate-950">{apprentice?.name || 'Apprentice'}</h3><p className="mt-1 text-sm font-semibold text-slate-700">{course?.title || syllabus?.rtiLabel || 'Assigned theory course'} · {completed}/{lessons.length} lessons complete</p>{schedule.starts_on ? <p className="mt-1 text-sm font-black text-fuchsia-950">Theory/RTI begins Monday, September 21, 2026.</p> : null}<p className="mt-1 text-sm text-slate-600">Next: {nextLesson?.title || (lessons.length ? 'Course complete' : 'Course publication pending')}</p></div>{syllabus?.syllabusPath ? <a href={syllabus.syllabusPath} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-fuchsia-300 px-4 py-2 text-sm font-black text-fuchsia-950">Open full syllabus</a> : null}</div>
