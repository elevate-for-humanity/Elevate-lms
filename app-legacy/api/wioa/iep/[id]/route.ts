...
async function _GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const _authCheck = await requireApiRole(['workforce_board', 'staff', 'admin', 'admin']);
...
async function _PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (error) {
      logger.error('[wioa/iep] DB mutation failed', normalizeError(error, 'DB mutation failed'), { code: error.code });
