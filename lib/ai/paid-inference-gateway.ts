...
import { runWithPaidInferenceContext } from './paid-inference-context';

export type PaidInferenceDecision =
  | 'approved'
  | 'approval_required'
  | 'budget_exceeded'
  | 'duplicate'
...
export async function reservePaidInference(
  db: SupabaseClient,
  input: {
    scopeKey: string;
  ...
  const { data, error } = await db.rpc(
    'reserve_paid_inference_v1' as never,
    {
  ...
  if (result.decision === 'duplicate' && result.request_id) {
    ...
    if (existingError) {
      throw new Error(
        `Paid inference authorization reconciliation failed: ${existingError.message}`,
      );
    }
    ...
    if (existingStatus === 'approval_required') {
      return { decision: 'approval_required', requestId: result.request_id };
    }
    if (
      ...
      return { decision: 'retry_exhausted', requestId: result.request_id };
    }
  }
...
export async function approvePaidInference(
  ...
  const { data, error } = await db.rpc(
    'approve_paid_inference_v1' as never,
    {
...
async function claimPaidInferenceDispatch(db: SupabaseClient, requestId: string): Promise<void> {
  const { data, error } = await db.rpc(
    'claim_paid_inference_dispatch_v1' as never,
    {
      p_request_id: requestId,
    } as never,
  );
...
async function finishPaidInference(
  ...
  const { data, error } = await db.rpc(
    'finish_paid_inference_v1' as never,
    {
      p_request_id: requestId,
      p_status: status,
  ...
  if (error || data !== true) {
    throw new Error(
      `Paid inference completion reconciliation failed${error ? `: ${error.message}` : ''}`,
    );
  }
}
...
export async function executePaidInference<T>(input: {
  ...
  try {
    const value = await runWithPaidInferenceContext(requestId, () => input.dispatch(requestId));
