...
const CourseTemplateGallery = dynamic(() => import('./CourseTemplateGallery'), { ssr: false });
...
export default function UnifiedCourseBuilder() {
  ...
  const [tab, setTab] = useState<Tab>(
    requestedTab && TAB_IDS.has(requestedTab) ? requestedTab : 'build',
  );
  ...
  const selectedCourse = useMemo(
    () => courses.find((course) => course.id === courseId) ?? workspace?.course ?? null,
    [courses, courseId, workspace?.course],
  );
  ...
  async function loadWorkspace(id = courseId) {
    ...
    try {
      const response = await fetch(
        `/api/admin/course-builder/course?courseId=${encodeURIComponent(id)}`,
        { cache: 'no-store' },
      );
      const data = await response.json();
  ...
  useEffect(() => {
    void Promise.all([loadCourses(requestedCourseId), loadPrograms()]);
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (courseId) void loadWorkspace(courseId);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
        ...
        <div className="mx-auto flex max-w-[1600px] flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">
              One authoring surface for manual editing, automatic AI generation, templates,
              document ingestion, pipeline builds, media, interactions, assessments, governance,
              and SCORM.
            </p>
          </div>
          <div className="flex min-w-0 items-center gap-2">
            <select
              ...
              className="min-h-10 min-w-[300px] rounded-lg border border-slate-700 bg-slate-950 px-3 text-sm text-white"
            >
            ...
            <button
              ...
              className="rounded-lg border border-slate-700 p-2 hover:bg-slate-800 disabled:opacity-50"
      ...
      {message && (
        <div className="mx-auto mt-4 max-w-[1600px] rounded-lg border border-amber-700 bg-amber-950/50 px-4 py-3 text-sm text-amber-200">
          {message}
        </div>
      )}
      ...
      <main className="mx-auto max-w-[1600px] p-4">
        {tab === 'build' &&
          (!courseId ? (
            <CreateCoursePanel onCreated={(id) => selectGeneratedCourse(id, 'build')} />
          ) : loading ? (
            <Loading />
          ) : workspace ? (
            <LiveCourseBuilder
              ...
              lmsBaseUrl="https://app.elevateforhumanity.org"
            />
          ) : (
            <CreateCoursePanel onCreated={(id) => selectGeneratedCourse(id, 'build')} />
          ))}
        ...
        {tab === 'pipeline' && (
          <CoursePipelineClient
            programs={programs}
            onCourseCreated={(id) => void selectGeneratedCourse(id, 'build')}
          />
        )}

        {tab === 'templates' && (
          <CourseTemplateGallery
            onCourseCreated={(id) => selectGeneratedCourse(id, 'build')}
          />
        )}
        ...
        {tab === 'blueprints' && (
          <BlueprintPanel
            ...
            onGenerated={(id) => selectGeneratedCourse(id, 'build')}
          />
        )}
        ...
        {tab === 'assessments' && (
          <AssessmentPanel workspace={workspace} onChanged={() => void loadWorkspace()} />
        )}

        {tab === 'compliance' &&
          (workspace ? (
            <CourseGovernancePanel
...
function CreateCoursePanel({ onCreated }: { onCreated: (id: string) => void | Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Unable to create course');
    } finally {
      setSaving(false);
    }
  }
...
function BlueprintPanel({
  ...
  async function generate() {
    if (!blueprintId || !programId) {
      setResult('Select a blueprint and link a program first.');
      return;
    }
    setRunning(true);
    setResult('');
    try {
      ...
      setResult(
        `Generated ${data.moduleCount ?? 0} modules and ${data.lessonCount ?? 0} lessons.`,
      );
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {blueprints.map((blueprint) => (
            ...
            >
              <div className="font-bold">{blueprint.title}</div>
              <div className="mt-1 text-xs text-slate-400">
                {blueprint.state ?? 'Multi-state'} · {blueprint.modules} modules ·{' '}
                {blueprint.lessons} lessons
              </div>
...
function AssessmentPanel({
  ...
  const assessments =
    workspace?.modules
      ...
      .filter((lesson: any) => ['quiz', 'checkpoint', 'exam'].includes(lesson.step_type)) ?? [];
  ...
  async function hydrate(lesson: any) {
    ...
    try {
      const response = await fetch('/api/admin/course-builder/hydrate', {
        ...
        body: JSON.stringify({
          lessonId: lesson.id,
          lessonType: lesson.step_type === 'exam' ? 'exam' : 'checkpoint',
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error || 'Assessment generation failed');
      }
      onChanged();
    } finally {
      setRunningId('');
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
      <h2 className="text-xl font-bold">Assessment studio</h2>
      <div className="mt-4 space-y-2">
        {assessments.length ? (
          assessments.map((lesson: any) => (
            <div
              key={lesson.id}
            ...
            >
              <div>
                ...
                <div className="text-xs text-slate-500">
                  {lesson.step_type} · pass {lesson.passing_score ?? 70}%
                </div>
              </div>
              <button
          ...
          <p className="text-sm text-slate-400">
            No quiz, checkpoint, or exam lessons found.
          </p>
        )}
      </div>
    </div>
