...
> = {
  admin: {
    ...
    color: 'bg-red-600',
    description: 'Site-wide metrics and management',
  },
  education: {
    label: 'Education',
    icon: <BookOpen className="w-4 h-4" />,
    color: 'bg-blue-600',
    description: 'Courses, progress, and learning',
  },
  trades: {
    label: 'Trades',
    icon: <Scissors className="w-4 h-4" />,
    color: 'bg-purple-600',
    description: 'Apprenticeship hours and shop management',
  },
  workforce: {
    label: 'Workforce',
  ...
  business: {
    ...
    color: 'bg-amber-600',
    description: 'Programs, providers, and employers',
  },
};
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {TAB_LINKS.admin.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="bg-white border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-blue-600 flex items-center justify-between"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        {data.taughtCourses.length > 0 && (
          <Stat
            ...
            value={data.taughtCourses.length}
            icon={<BookOpen className="w-4 h-4" />}
            color="bg-purple-600"
          />
        )}
        ...
        <div>
          ...
          <div className="space-y-2">
            {data.enrollments.map((e: any) => (
              <Link
                key={e.id}
                href={e.programs?.slug ? `/lms/courses/${e.id}` : '/lms/courses'}
                ...
                <div key={d} className="bg-white border border-slate-200 rounded-xl p-4">
                  <div className="flex justify-between mb-2">
                    ...
                    <p className="text-sm font-bold text-slate-900">
                      {logged.toLocaleString()} / {target.toLocaleString()}
                    </p>
                  </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-700">
                Type: {data.partner.partners?.partner_type ?? '—'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-700">
                MOU {data.partner.partners?.mou_signed ? 'Signed' : 'Pending'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-700">
                Onboarding {data.partner.partners?.onboarding_completed ? 'Complete' : 'Incomplete'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {data.pendingHours.map((h: any) => (
              <div
                key={h.id}
                className="flex items-center justify-between bg-white border border-amber-200 rounded-lg px-4 py-3"
              >
                <div>
                  <p className="text-sm font-medium text-slate-800">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`${activeMeta.color} text-white rounded-lg p-1.5`}>
              {activeMeta.icon}
            </div>
            <div>
              ...
              {[
                {
                  ...
                  roles: ['admin', 'super_admin', 'org_admin', 'staff'],
                },
                {
                ...
                },
                {
                  ...
                  roles: ['program_holder', 'delegate', 'admin', 'super_admin'],
                },
                {
                ...
                },
                {
                  ...
                  roles: ['case_manager', 'admin', 'super_admin'],
                },
                {
                ...
                .filter(
                  (item) => item.roles.includes(role) || ['admin', 'super_admin'].includes(role),
                )
                .map((item) => (
                  <Link
                    key={item.href}
