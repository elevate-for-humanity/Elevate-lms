...
export default async function StartTrialPage() {
  ...
  return (
    <div className="min-h-screen bg-brand-red-800 flex items-center justify-center p-4">
      <div className="max-w-lg w-full">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Layout className="w-10 h-10 text-brand-red-900" />
          </div>
          <h1 className="text-3xl font-black text-white mb-2">Website Builder</h1>
          <p className="text-brand-red-200">Professional Websites in Minutes</p>
        </div>

        <div className="bg-white rounded-2xl p-8 shadow-2xl">
          <div className="text-center mb-6">
            <div className="inline-flex items-center gap-2 bg-brand-red-100 text-brand-red-800 px-4 py-2 rounded-full text-sm font-bold mb-4">
          ...
          <ul className="space-y-3 mb-8">
            {features.map((feature, i) => (
              <li key={i} className="flex items-center gap-3">
                <Check className="w-5 h-5 text-brand-red-500 flex-shrink-0" />
          ...
          <form action={startTrial}>
            <button type="submit" className="w-full bg-brand-red-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-brand-red-700 transition">
              Start Free Trial
            </button>
          </form>

          <p className="text-center text-sm text-gray-500 mt-4">
            By starting a trial, you agree to our <Link href="/terms" className="text-brand-red-600 hover:underline">Terms</Link>
          </p>
        </div>
      </div>
    </div>
