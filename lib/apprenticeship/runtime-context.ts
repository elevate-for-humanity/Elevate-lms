...
export async function resolveApprenticeshipRuntimeContext(
  ...
  if (!input.enrollmentId) {
    enrollmentQuery = enrollmentQuery
      .in('status', ACTIVE_ENROLLMENT_STATES)
  ...
  const { data: placement, error: placementError } = await db
    ...
    .select(
      'id,student_id,program_slug,shop_id,supervisor_user_id,start_date,end_date,status,tenant_id',
    )
  ...
  if (placement?.shop_id) {
    const { data: shopRow, error: shopError } = await db
      .from('shops')
      .select(
        'id,name,address1,address2,city,state,zip,active,partner_id,latitude,longitude,owner_id',
      )
    ...
    if (shop?.partner_id) {
      const { data: partnerRow, error: partnerError } = await db
        .from('partners')
        .select(
          'id,name,dba,legal_name,shop_name,status,is_active,approval_status,verification_status,documents_verified,mou_signed,onboarding_completed,rapids_employer_number,rapids_registration_status,rapids_sponsor_registration_number,programs',
        )
...
export function assertOperationalPlacement(context: ApprenticeshipRuntimeContext) {
  if (!context.contract)
    throw new Error(`REGISTERED_PROGRAM_CONTRACT_MISSING:${context.programSlug}`);
  if (!context.placement)
    throw new Error(`ACTIVE_APPRENTICESHIP_PLACEMENT_REQUIRED:${context.enrollment.id}`);
  if (!context.shop || context.shop.active === false)
    throw new Error(`ACTIVE_HOST_SHOP_REQUIRED:${context.enrollment.id}`);
  if (!context.partner)
    throw new Error(`CANONICAL_HOST_SHOP_PARTNER_REQUIRED:${context.enrollment.id}`);
  ...
  if (
    context.partner.approval_status !== 'approved' ||
    context.partner.verification_status !== 'verified'
  ) {
