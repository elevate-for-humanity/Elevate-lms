...
for (const required of [
  ...
  if (!rootText.includes(required))
    failures.push(`${rootRoute}: missing canonical action/authority ${required}`);
...
if (!index.includes("export { courseFactory } from '../course-builder/orchestrator'"))
  failures.push(
    'lib/course-factory/index.ts must expose Course Builder facade instead of raw factory.ts',
  );
const orchestrator = readRequired('lib/course-builder/orchestrator.ts');
if (!orchestrator.includes("from '../course-factory/factory'"))
  failures.push('Course Builder orchestrator does not own private Course Factory execution');
if (!orchestrator.includes('saveCourseProgramConfiguration'))
  failures.push(
    'Course Builder orchestrator does not own course program configuration persistence',
  );
...
for (const capability of [
  ...
  if (!editService.includes(capability))
    failures.push(`Course Builder edit service is missing ${capability}`);
}
...
for (const capability of [
  ...
  if (!reviewService.includes(capability))
    failures.push(`Course Builder review service is missing ${capability}`);
}
const persistedPublish = readRequired('lib/course-builder/persisted-publish-service.ts');
for (const capability of [
  'runPersistedCourseProcurementHealthCheck',
  'repairPersistedCourseAcceptanceWithClient',
  'normalizeGeneratedCourseForGovernance',
  'publishPersistedCourse',
  'publishCourse',
  'review_status',
  'reviewed_by',
  'module_completion_rules',
]) {
  if (!persistedPublish.includes(capability))
    failures.push(`Persisted Course Builder publication service is missing ${capability}`);
}

const cloneService = readRequired('lib/course-builder/clone-service.ts');
for (const capability of [
  'cloneCanonicalCourse',
  ".from('courses')",
  ".from('course_modules')",
  ".from('course_lessons')",
  "review_status: 'draft'",
  ...
  if (!cloneService.includes(capability))
    failures.push(`Course Builder clone service is missing ${capability}`);
}
for (const rel of [
  ...
  if (!text.includes('cloneCanonicalCourse'))
    failures.push(`${rel}: clone route is not delegated to Course Builder clone service`);
  for (const forbidden of [
    ...
    if (text.includes(forbidden))
      failures.push(`${rel}: direct canonical clone write remains: ${forbidden}`);
  }
}
...
for (const rel of retiredMutationRoutes) {
  ...
  if (!text) continue;
  if (!/(RETIRED|COURSE_FACTORY_REQUIRED|COURSE_BUILDER_ROOT_REQUIRED)/.test(text))
    failures.push(`${rel}: historical course mutation endpoint is not explicitly retired`);
  if (/\bcourseFactory\s*\(/.test(text))
    failures.push(`${rel}: retired endpoint still invokes course generation`);
  if (
    /\.from\(['\"](?:courses|course_modules|course_lessons)['\"]\)[\s\S]{0,220}\.(?:insert|upsert|update|delete)\(/.test(
      text,
    )
  )
    failures.push(`${rel}: retired endpoint still writes canonical course tables`);
  if (/\bpublishCourse\s*\(/.test(text))
    failures.push(`${rel}: retired endpoint still publishes outside Course Builder root`);
}
...
for (const required of [
  ...
  "action: 'queue-media'",
  "result.status ?? (validateOnly ? 'validated' : 'queued')",
  'charged: result.charged ?? !validateOnly',
]) {
  if (!mediaCompat.includes(required))
    failures.push(`${mediaCompatRoute}: compatibility media route missing ${required}`);
}
for (const forbidden of [
  'processLesson',
  "from '@/lib/video/pipeline'",
  ".from('course_lessons').update",
  'OPENAI_API_KEY',
  'PEXELS_API_KEY',
]) {
  if (mediaCompat.includes(forbidden))
    failures.push(
      `${mediaCompatRoute}: duplicate media generation authority remains: ${forbidden}`,
    );
}
...
if (!lessonCompat.includes('/api/admin/course-builder'))
  failures.push(
    `${lessonCompatRoute}: compatibility lesson mutations do not cross Course Builder root`,
  );
for (const forbidden of [
  ".from('course_lessons').insert",
  ".from('course_lessons').delete",
  ".from('course_lessons').update",
]) {
  if (lessonCompat.includes(forbidden))
    failures.push(`${lessonCompatRoute}: direct canonical lesson mutation remains: ${forbidden}`);
}

const scormText = readRequired('apps/admin/app/api/admin/course-builder/scorm-link/route.ts');
if (!scormText.includes('RETIRED mutation'))
  failures.push('SCORM mutation route is not explicitly retired');
if (
  /export async function POST[\s\S]*?\.from\(['\"]scorm_packages['\"]\)[\s\S]{0,220}\.update\(/.test(
    scormText,
  )
)
...
for (const rel of [
  ...
  const writes = (table) =>
    new RegExp(
      `\\.from\\(['\"]${table}['\"]\\)[\\s\\S]{0,280}\\.(?:insert|upsert|update|delete)\\(`,
    ).test(text);
  if (writes('courses') && writes('course_modules') && writes('course_lessons'))
    failures.push(
      `${rel}: parallel complete course-package writer detected; complete packages must persist through Course Factory publisher or Course Builder clone service`,
    );
}
...
for (const rel of walk('apps')) {
  ...
  const text = readOptional(rel);
  if (text.includes('@/lib/course-factory/factory'))
    failures.push(`${rel}: runtime route imports private Course Factory engine`);
  if (/\bcourseFactory\s*\(/.test(text) && rel.includes('/api/')) {
    const studioFacadeAllowed =
      rel === studioChatRoute &&
      text.includes("await import('@/lib/course-factory')") &&
      !text.includes('@/lib/course-factory/factory');
    if (!studioFacadeAllowed)
      failures.push(`${rel}: independent HTTP complete-course caller detected; use ${rootRoute}`);
  }
  if (/\bpublishCourse\s*\(/.test(text) && rel.includes('/api/'))
    failures.push(`${rel}: independent HTTP publication caller detected; use ${rootRoute}`);
  if (
    /\.from\(['\"]courses['\"]\)[\s\S]{0,350}\.update\(\{[\s\S]{0,220}status\s*:\s*['\"]published['\"]/.test(
      text,
    )
  )
    failures.push(`${rel}: direct HTTP publication-state write detected; use ${rootRoute}`);
  if (
    /\.from\(['\"]courses['\"]\)[\s\S]{0,350}\.update\(\{[\s\S]{0,220}review_status\s*:/.test(text)
  )
    failures.push(`${rel}: direct HTTP course-review mutation detected; use ${rootRoute}`);
  if (
    /\.from\(['\"]course_lessons['\"]\)[\s\S]{0,350}\.update\(\{[\s\S]{0,220}approved\s*:/.test(
      text,
    )
  )
...
for (const rel of [...walk('apps'), ...walk('components'), ...walk('lib')]) {
  ...
  const text = readOptional(rel);
  for (const endpoint of forbiddenEndpointRefs)
    if (endpointReferenced(text, endpoint))
      failures.push(`${rel}: retired endpoint referenced: ${endpoint}`);
}
