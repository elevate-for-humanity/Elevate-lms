...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await signSalonHostShopMou(db, 'nail_technician', user.email, user.id, {
      salon_name,
      signer_name,
    ...
    if (err instanceof Error && err.message === 'NO_PARTNER') {
      return safeError('No nail technician host application found for this account', 404);
    }
