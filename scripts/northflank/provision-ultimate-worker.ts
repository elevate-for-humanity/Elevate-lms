...
const serviceId =
  process.env.NORTHFLANK_ULTIMATE_WORKER_SERVICE_ID || 'elevate-ultimate-worker';
...
const llmUrl = process.env.ELEVATE_LLM_URL;
const llmSecret = process.env.ELEVATE_LLM_SECRET;
if (!llmUrl || !llmSecret) throw new Error('ULTIMATE_LLM_CONFIG_REQUIRED');
...
const mediaServiceSecret =
  process.env.CRON_SECRET || process.env.ULTIMATE_MEDIA_SERVICE_SECRET;
if (!mediaServiceSecret) throw new Error('ULTIMATE_MEDIA_SERVICE_SECRET_REQUIRED');

const payload = {
  name: serviceId,
  description: 'Ultimate Course Builder durable production worker',
  ...
  buildSettings: {
    dockerfile: {
      ...
      dockerFilePath: '/Dockerfile.ultimate-worker',
      dockerWorkDir: '/',
  ...
  runtimeEnvironment: {
    ...
    ULTIMATE_WORKER_POLL_MS: '5000',
    ELEVATE_LLM_URL: llmUrl,
    ELEVATE_LLM_SECRET: llmSecret,
    ULTIMATE_MEDIA_SERVICE_URL: mediaServiceUrl,
    ULTIMATE_MEDIA_SERVICE_SECRET: mediaServiceSecret,
    ULTIMATE_NARRATION_TIMEOUT_MS: '300000',
    ULTIMATE_RENDER_TIMEOUT_MS: '1800000',
  },
};
...
if (!process.argv.includes('--execute')) {
  console.log(JSON.stringify({ serviceId, branch, projectId, mediaServiceUrl }, null, 2));
  process.exit(0);
}
