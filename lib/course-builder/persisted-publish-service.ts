...
export async function runPersistedCourseProcurementHealthCheckWithClient(
  ...
  const { data: course, error: courseError } = await supabase
    .from('courses')
    .select(
      'id,title,slug,description,status,review_status,reviewed_by,reviewed_at,generation_status,generation_progress,compliance_profile_key,governing_body,governing_standard_version,duration_hours,passing_score',
    )
    .eq('id', courseId)
    .maybeSingle();
  ...
  if (!course.duration_hours || Number(course.duration_hours) <= 0)
    blocking.push('course duration_hours is missing');
  const externalProfile = Boolean(
    course.compliance_profile_key && course.compliance_profile_key !== 'internal_basic',
  );
  if (externalProfile && !course.governing_body?.trim()) blocking.push('governing body is missing');
  if (externalProfile && !course.governing_standard_version?.trim())
    blocking.push('governing standard/test-plan version is missing');
  if (course.generation_status && !['completed', 'published'].includes(course.generation_status))
    blocking.push(
      `course generation is not complete (${course.generation_status}, ${course.generation_progress ?? 0}%)`,
    );
  ...
  for (const [mi, module] of (mods as any[]).entries()) {
    ...
    if (!module.domain_key?.trim())
      blocking.push(`module "${module.title || mi + 1}": standards/domain mapping missing`);
    if (!module.target_hours || Number(module.target_hours) <= 0)
      blocking.push(`module "${module.title || mi + 1}": target hours missing`);
    ...
    for (const lesson of lessons) {
      ...
      const objectives = asArray(lesson.learning_objectives),
        competencies = asArray(lesson.competency_checks),
      ...
      if (isPractical) {
        ...
        if (lesson.requires_instructor_signoff !== true)
          issues.push('authorized human sign-off missing for practical competency');
      }
...
export async function publishPersistedCourseWithClient(input: {
  ...
  await logAdminAudit({
    ...
    metadata: {
      label: input.label,
      lesson_count: (result as any)?.lessonCount,
