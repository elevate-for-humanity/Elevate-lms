    ...
    <div className="min-h-screen bg-slate-800">
      ...
      <div className="max-w-3xl mx-auto py-12 px-4">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-brand-red-600/20 text-brand-red-300 px-4 py-2 rounded-full text-sm font-medium mb-4">
          ...
          {[1, 2, 3, 4, 5].map((s) => (
            <div key={s} className="flex items-center">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                s < step ? 'bg-green-500 text-white' : s === step ? 'bg-brand-red-600 text-white' : 'bg-slate-700 text-slate-400'
              }`}>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.shopName} onChange={(e) => updateField('shopName', e.target.value)}
                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="ABC Barbershop LLC" />
                </div>
                <div>
                  ...
                  <input type="text" value={formData.dba} onChange={(e) => updateField('dba', e.target.value)}
                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="ABC Cuts" />
                </div>
              </div>
              <div>
                ...
                <input type="text" value={formData.fein} onChange={(e) => updateField('fein', e.target.value)}
                  className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="XX-XXXXXXX" />
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={formData.ownerName} onChange={(e) => updateField('ownerName', e.target.value)}
                      className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="John Smith" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" value={formData.email} onChange={(e) => updateField('email', e.target.value)}
                      className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="owner@shop.com" />
                  </div>
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="tel" value={formData.phone} onChange={(e) => updateField('phone', e.target.value)}
                    className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="Contact Us" />
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input type="text" value={formData.addressLine1} onChange={(e) => updateField('addressLine1', e.target.value)}
                  className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="123 Main Street" />
              </div>
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">City *</label>
                  <input type="text" value={formData.city} onChange={(e) => updateField('city', e.target.value)}
                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="Indianapolis" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={formData.zip} onChange={(e) => updateField('zip', e.target.value)}
                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-red-500" placeholder="46204" />
                </div>
              </div>
            </div>
          )}
              ...
              <div className="space-y-3">
                {AVAILABLE_PROGRAMS.map(program => (
                  <label key={program.id} className={`flex items-start gap-4 p-4 border-2 rounded-lg cursor-pointer ${
                    formData.programsRequested.includes(program.id) ? 'border-brand-red-500 bg-brand-red-50' : 'border-slate-200 hover:border-slate-300'
                  }`}>
                    <input type="checkbox" checked={formData.programsRequested.includes(program.id)}
                      onChange={() => toggleProgram(program.id)} className="mt-1 w-5 h-5 text-brand-red-600 rounded" />
                    <div>
          ...
          {step === 4 && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                <Upload className="w-5 h-5 text-brand-red-600" />
                Required Documents
              </h2>
                    ...
                    <div key={docType} className={`p-4 border-2 rounded-lg ${uploaded ? 'border-green-500 bg-green-50' : 'border-slate-200'}`}>
                      <div className="flex items-center justify-between">
                        ...
                        {uploaded ? (
                          ...
                          <label className="cursor-pointer bg-brand-red-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-red-700">
                            Upload
          ...
          {step === 5 && (
            <div className="space-y-6">
                ...
                <div><p className="text-sm text-slate-500">Programs</p>
                  <div className="flex flex-wrap gap-2 mt-1">
                    {formData.programsRequested.map(p => (
                      <span key={p} className="bg-brand-red-100 text-brand-red-700 px-3 py-1 rounded-full text-sm">{AVAILABLE_PROGRAMS.find(prog => prog.id === p)?.name}</span>
                    ))}
                  </div>
                </div>
              ...
              <label className="flex items-start gap-3 p-4 border-2 border-slate-200 rounded-lg cursor-pointer hover:border-brand-red-300">
                <input type="checkbox" checked={formData.agreedToTerms} onChange={(e) => updateField('agreedToTerms', e.target.checked)}
                  className="mt-1 w-5 h-5 text-brand-red-600 rounded" />
                <div className="text-sm text-slate-600">
                  I agree to the <Link href="/terms-of-service" className="text-brand-red-600 hover:underline">Terms of Service</Link> and <Link href="/privacy-policy" className="text-brand-red-600 hover:underline">Privacy Policy</Link>.
                </div>
              </label>
            </div>
          )}
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-slate-200">
            ...
            {step < 5 ? (
              ...
              }} className="flex items-center gap-2 px-6 py-3 bg-brand-red-600 text-white rounded-lg font-medium hover:bg-brand-red-700">
