-- A pause blocks new paid reservations, but must not invalidate a request that
-- an authorized administrator already approved. Resolve idempotent requests
-- and validated cache entries before enforcing the pause for new work.
CREATE OR REPLACE FUNCTION public.reserve_paid_inference_v1(p_scope_key text, p_tenant_id uuid, p_actor_id uuid, p_course_id uuid, p_lesson_id uuid, p_run_id uuid, p_job_id uuid, p_artifact_fingerprint text, p_idempotency_key text, p_provider text, p_model text, p_operation text, p_projected_cost_micros bigint)
