...
const BARBER = CONTRACT.standard;

export const HOST_SHOP_ORIENTATION_SLUG = 'host-shop-apprenticeship-orientation';
export const HOST_SHOP_ORIENTATION_VERSION = '2026.08.18-registered-contract-v2';
export const BARBER_APPRENTICE_APPLICATION_URL = 'https://www.elevateforhumanity.org/apply/student?program=barber-apprenticeship';
...
const wageRows = [`<li><strong>Entry baseline:</strong> $${BARBER.startingHourlyRate.toFixed(2)}/hour</li>`, ...BARBER.wageMilestones.map((m) => `<li><strong>After ${m.completedCompetencies} completed competencies:</strong> baseline $${m.hourlyRate.toFixed(2)}/hour</li>`)].join('');
const rtiRows = BARBER.relatedInstruction.map((item) => `<li>${item.title}: <strong>${item.hours} hours</strong></li>`).join('');
const competencyRows = BARBER.competencies.map((item, index) => `<li><strong>${index + 1}. ${item.category}</strong> — ${item.description}</li>`).join('');
...
export const HOST_SHOP_APPRENTICESHIP_ORIENTATION: CourseTemplate = {
  ...
  description: 'Required Host Shop operating orientation grounded in the canonical registered-program contract for the Barber occupation.',
  ...
  modules: [
    module('dol-registered-contract', '1. The Registered Program Contract Is the Source of Truth', 1, 'dol-registered-contract', `<h2>The approved registered program controls the apprenticeship operating model</h2><p>${CONTRACT.sponsor.sponsor} is the registered sponsor under <strong>${CONTRACT.sponsor.registrationNumber}</strong>. Barber is RAPIDS <strong>${BARBER.rapidsCode}</strong>, O*NET-SOC <strong>${BARBER.onetSocCode}</strong>, and <strong>${BARBER.approach}</strong>.</p><p>Completion is based on <strong>${CONTRACT.completion.competencyCount} approved competencies</strong> plus required RTI and sponsor requirements; there is no fabricated fixed OJL completion denominator.</p><p>The registered standard requires a <strong>${BARBER.apprenticeToMentorRatio} apprentice-to-mentor ratio</strong> and a <strong>${BARBER.probationaryHours}-hour probationary period</strong>.</p>`, ['Identify the registered program contract as the source of truth', 'Explain competency-based progression', 'Apply the approved mentor ratio and probationary period']),
    ...
    module('competency-progress', '3. Competency Progress & Host Shop Sign-Off', 3, 'dol-competencies', `<h2>${CONTRACT.completion.competencyCount} registered competencies</h2><p>The Host Shop must observe and document the apprentice performing the approved work processes. Attendance or time at the shop does not by itself complete a competency.</p><ol>${competencyRows}</ol><p>Use the Host Shop competency tracker to record demonstrated progress. A supervisor signs off only when the apprentice can safely and consistently perform the work to the expected standard. Geofenced time, service/practice records, supervisor observations, photos where permitted, and corrective notes can support the competency record.</p>`, ['Recognize all registered barber competencies', 'Distinguish attendance from demonstrated competency', 'Document competency evidence and supervisor verification']),
