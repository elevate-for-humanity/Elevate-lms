...
export default async function CertificationPage() {
  ...
  const [certsRes, attemptsRes, inProgressRes, availableRes] = await Promise.all([
    ...
    supabase
      ...
      .select(
        'id, credential_id, program_id, attempt_number, attempted_at, passed, credentials(name, abbreviation, issuing_authority)',
      )
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-slate-700 mt-1">
          Track your credential progress, exam status, and certificates
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-4">
                {attemptLifecycles.map(({ attempt, state, funding }) => {
                  ...
                  const isSponsoredPending =
                    funding.fundingSource !== 'self_pay' && funding.fundingStatus === 'pending';
                  const isSponsoredApproved =
                    funding.fundingSource !== 'self_pay' && funding.fundingStatus === 'approved';

                  const ctaHref = (() => {
                    if (state === 'payment_required' && !isSponsoredPending)
                      return `/lms/payments/checkout?attemptId=${attempt.id}`;
                    if (state === 'payment_approved' || state === 'eligible')
                      return '/lms/certification/schedule';
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-sm text-slate-700">
                            {attempt.credentials?.issuing_authority} · Attempt #
                            {attempt.attempt_number}
                          </p>
                        </div>
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${style.color}`}
                        >
                          {style.label}
                        </span>
                      </div>
                      ...
                      {state !== 'certificate_issued' && (
                        ...
                        >
                          ...
                          {ctaHref && !isSponsoredPending && (
                            <Link
                              href={ctaHref}
                              className="inline-flex items-center gap-1 text-sm font-semibold text-brand-blue-700 hover:text-brand-blue-900 flex-shrink-0"
                            >
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        {cert.course && (
                          <p className="text-sm text-slate-700">{cert.course.course_name}</p>
                        )}
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="space-y-3">
                {inProgress.map((item: any) => (
                  <div
                    key={item.id}
                    className="flex items-center gap-4 p-3 bg-brand-blue-50 rounded-lg"
                  >
                    <Clock className="w-5 h-5 text-brand-blue-600 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-sm truncate">
                        {item.course?.certification_name}
                      </h3>
                    ...
                    <Link
                      href={`/lms/courses/${item.course?.id}`}
                      className="text-brand-blue-600 text-sm font-medium flex-shrink-0"
                    >
                      Continue
                    </Link>
                  </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border p-6">
            ...
            <Link
              href="/lms/programs"
              className="block text-center text-brand-blue-600 text-sm font-medium mt-4 hover:underline"
            >
              View All Programs
            </Link>
          </div>
          ...
          <div className="bg-brand-blue-50 border border-brand-blue-200 rounded-xl p-6">
            ...
            <Link
              href="/verify"
              className="flex items-center gap-1 text-brand-blue-600 text-sm font-medium hover:underline"
            >
