...
async function getRelatedPosts(
  ...
  const staticRelated = STATIC_POSTS.filter(
          ...
          <div className="mt-12 pt-8 border-t border-slate-200">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {post.tags.map((tag: string) => (
                <span key={tag} className="px-3 py-2 bg-white text-slate-900 text-sm rounded-full">
                  {tag}
                </span>
              ))}
            </div>
              ...
              {relatedPosts.map((related: any) => (
                ...
                >
                  <div className="relative h-48 overflow-hidden">
                    <Image
                      src={related.image || '/images/pages/project-management.webp'}
                      alt={related.title}
                      fill
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-slate-700 text-sm mt-2 line-clamp-2">{related.excerpt}</p>
                  </div>
                </Link>
              ))}
            </div>
...
function formatContent(content: string): string {
  ...
  return content
    ...
    .replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" class="text-brand-blue-600 hover:underline">$1</a>',
    )
