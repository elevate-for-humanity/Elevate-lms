...
import { Users, Award, Clock, ClipboardList, ClipboardCheck } from 'lucide-react';
...
export default async function InstructorDashboard() {
  ...
  if (!isAdmin) {
    const { data: assignedPrograms } = await supabase
      ...
      .select('program_id')
      .eq('instructor_id', user.id);
    assignedProgramIds = (assignedPrograms || []).map((p: any) => p.program_id).filter(Boolean);
  }
        ...
        {!isAdmin && assignedProgramIds.length === 0 && (
          <div className="mb-8 rounded-xl border border-amber-300 bg-amber-50 p-5 text-amber-950">
            <p className="font-bold">
              No program assignments are linked to your instructor account.
            </p>
            <p className="mt-1 text-sm">
              Student records remain hidden until an administrator assigns you to a program.
            </p>
          </div>
        )}
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
              ...
              {students.length > 0 ? (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <span
                            className={`px-3 py-2 rounded-full text-xs font-medium ${student.status === 'active' ? 'bg-brand-green-100 text-brand-green-700' : student.status === 'completed' ? 'bg-brand-blue-100 text-brand-blue-700' : 'bg-white text-black'}`}
                          >
                            {student.status}
                          </span>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-black mb-4">No students assigned</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
              ...
              <div className="space-y-3">
                <Link
                  ...
                  className="flex items-center justify-between w-full px-4 py-3 bg-emerald-50 hover:bg-emerald-100 rounded-lg border border-emerald-200 transition"
                >
                  <div className="flex items-center gap-3">
                    <ClipboardCheck className="w-5 h-5 text-emerald-700 shrink-0" />
                    <div>
                      <p className="font-medium text-emerald-950">48-Hour Progress Forms</p>
                      <p className="text-xs text-emerald-800">
                        Speak or type training and hands-on progress
                      </p>
                    </div>
                  </div>
                </Link>
                ...
                >
                  ...
                  <p className="text-xs text-slate-500">View assigned students</p>
                </Link>
                <Link
                ...
                >
                  ...
                  <p className="text-xs text-slate-500">Browse assigned programs</p>
                </Link>
                <Link
