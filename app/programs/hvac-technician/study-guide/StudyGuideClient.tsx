...
const PHASE_OUT_TIMELINE = [
  ...
  {
    year: '2018',
    event: 'EPA 608 certification required to purchase ANY refrigerant (including HFCs)',
  },
  { year: '2020', event: 'R-22 production and import fully ended. Reclaimed R-22 only.' },
  {
    year: '2023+',
    event: 'AIM Act HFC phasedown begins. R-410A being replaced by A2L refrigerants.',
  },
  {
    year: '2025+',
    event: 'New residential AC systems shipping with R-454B (Opteon XL41) instead of R-410A.',
  },
];
...
const CERT_TYPES = [
  {
    type: 'Core',
    covers: 'Required for ALL types. Ozone, Clean Air Act, refrigerant safety, recovery basics.',
  },
  {
    type: 'Type I',
    covers:
      'Small appliances — factory-charged, 5 lbs or less. Window units, PTACs, vending machines.',
  },
  {
    type: 'Type II',
    covers: 'High-pressure systems — R-410A, R-22, R-134a. Residential and commercial AC.',
  },
  { type: 'Type III', covers: 'Low-pressure systems — R-11, R-123 centrifugal chillers.' },
  {
    type: 'Universal',
    covers: 'Passes all four sections. Work on any system type. Most valuable for technicians.',
  },
];

const MUST_KNOW_QA = [
  {
    q: 'Maximum fine for knowingly venting refrigerant?',
    a: '$44,539 per day per violation (Clean Air Act)',
  },
  {
    q: 'What does ODP stand for?',
    a: 'Ozone Depletion Potential. R-12 = 1.0 (reference). R-410A = 0.',
  },
  {
    q: 'What does GWP stand for?',
    a: 'Global Warming Potential. Measured relative to CO₂ (GWP=1).',
  },
  {
    q: 'Recovery cylinder max fill level?',
    a: '80% of water capacity. Never overfill — liquid expands with heat.',
  },
  {
    q: 'DOT hydrostatic retest interval?',
    a: 'Every 5 years. Expired = cannot be legally refilled.',
  },
  ...
  {
    q: 'Difference between recovery, recycling, reclamation?',
    a: 'Recovery = remove from system. Recycling = clean on-site for reuse. Reclamation = process to ARI 700 purity at certified facility.',
  },
  {
    q: 'What gas is NEVER used to pressure test a system?',
    a: 'Oxygen or compressed air — explosive with refrigerant oil. Use dry nitrogen only.',
  },
  {
    q: 'Target vacuum level before charging?',
    a: '500 microns or below. Verify with micron gauge, not manifold gauge.',
  },
  {
    q: 'What is a de minimis release?',
    a: 'Small unavoidable release when connecting/disconnecting hoses. The only legal release.',
  },
  ...
  {
    q: 'What does A2L mean?',
    a: 'Low toxicity (A), mildly flammable (2L). Burning velocity ≤10 cm/s.',
  },
  ...
  {
    q: 'What is a purge unit on a low-pressure chiller?',
    a: 'Removes air and non-condensables that leak INTO the system (low-pressure systems operate below atmospheric).',
  },
  {
    q: 'Can recovered refrigerant be returned to the same system without reclamation?',
    a: "Yes — same owner's equipment only. Different owner requires reclamation to ARI 700.",
  },
];
...
function SectionHeader({ number, title, color }: { number: string; title: string; color: string }) {
  return (
    <div
      className={`flex items-center gap-3 mb-4 pb-3 border-b-2 ${color} print:break-before-page`}
    >
      <span
        className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 ${color.replace('border-', 'bg-')}`}
      >
        {number}
      </span>
  ...
  return (
    <div className="space-y-2 print:space-y-3">
      {MUST_KNOW_QA.map((item, i) => (
        <div
          key={i}
          className="border border-slate-200 rounded-lg overflow-hidden print:border-slate-300"
        >
          <button
          ...
          <div className={`px-4 pb-3 ${open === i ? 'block' : 'hidden'} print:block`}>
            <p className="hidden print:block font-semibold text-slate-900 text-sm pt-3">
              {i + 1}. {item.q}
            </p>
...
function StudyGuideContent() {
  ...
  const buildTextContent = () => {
    ...
    text += '\nRECOVERY LEVELS\n' + '-'.repeat(40) + '\n';
    RECOVERY_LEVELS.forEach((r) => {
      text += `${r.system}: ${r.level} (${r.note})\n`;
    });

    text += '\nLEAK REPAIR TRIGGER RATES\n' + '-'.repeat(40) + '\n';
    LEAK_RATES.forEach((r) => {
      text += `${r.type}: ${r.rate} — ${r.days}\n`;
    });

    text += '\nPHASE-OUT TIMELINE\n' + '-'.repeat(40) + '\n';
    PHASE_OUT_TIMELINE.forEach((p) => {
      text += `${p.year}: ${p.event}\n`;
    });
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 print:px-0 print:py-0 space-y-10">
        ...
        <div className="text-center py-8 print:py-4">
          ...
          <p className="text-xs text-black mt-1">
            ETPL Program #10004322 · WIOA-Approved · Free to Enrolled Students
          </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white text-left">
                  ...
                  <th className="px-3 py-2 font-bold text-slate-700 rounded-r-lg">
                    What It Covers
                  </th>
                </tr>
              </thead>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 print:shadow-none print:border-0 print:rounded-none">
              ...
              <thead>
                <tr className="bg-white text-left">
                  {[
                    ...
                    <th key={h} className="px-2 py-2 font-bold text-slate-700">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {REFRIGERANT_TABLE.map((r) => (
                  <tr
                    key={r.name}
                    className={
                      r.safety === 'A2L' ? 'bg-amber-50' : r.odp !== '0' ? 'bg-red-50' : ''
                    }
                  >
                    ...
                    <td
                      className={`px-2 py-2 font-bold ${r.safety === 'A2L' ? 'text-amber-700' : r.safety === 'B1' ? 'text-red-700' : 'text-brand-green-700'}`}
                    >
                      {r.safety}
                    </td>
                  </tr>
          ...
          <p className="text-xs text-black mt-3">
            🟡 Yellow = A2L (mildly flammable — requires A2L-rated equipment) · 🔴 Red = ODP &gt; 0
            (ozone depleting)
          </p>
        </div>
          ...
          <div className="space-y-3">
            {PHASE_OUT_TIMELINE.map((p) => (
              <div key={p.year} className="flex gap-4 items-start">
                <span className="flex-shrink-0 w-14 text-right font-black text-brand-blue-600 text-sm pt-0.5">
                  {p.year}
                </span>
