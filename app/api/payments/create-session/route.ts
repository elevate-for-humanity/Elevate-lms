  ...
  try {
    ...
    const commonConfig: Partial<Stripe.Checkout.SessionCreateParams> = {
      ...
      metadata: {
        ...
        program_name: program.title ?? program.title,
    ...
    if (paymentType === 'plan' && price >= 500) {
      ...
      sessionConfig = {
        ...
        line_items: [
          {
            price_data: {
              currency: 'usd',
              product_data: {
                name: `${program.title ?? program.title} - Payment Plan`,
                description: `4-month payment plan for ${program.title ?? program.title}. Total: $${price}`,
        ...
        subscription_data: {
          ...
          description: `${program.title ?? program.title} - 4 Monthly Payments`,
      ...
      sessionConfig = {
        ...
        line_items: [
          {
            price_data: {
              currency: 'usd',
              product_data: {
                name: program.title ?? program.title,
                description: `Enrollment in ${program.title ?? program.title} training program`,
                images: program.image_url ? [program.image_url] : undefined,
                metadata: {
                  program_id: programId,
                  payment_type: 'full',
        ...
        payment_intent_data: {
          ...
          description: `${program.title ?? program.title} - Full Payment`,
        },
      } as Stripe.Checkout.SessionCreateParams;
    }
