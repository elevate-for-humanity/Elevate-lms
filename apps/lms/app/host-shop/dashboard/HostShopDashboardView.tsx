...
export default async function HostShopDashboardView() {
  ...
  if (board.partner?.approval_status !== 'approved' || board.partner?.status !== 'active') redirect('/host-shop/onboarding');
  ...
  const progressValue = competencyBased ? `${board.tradeInfo.competencyCount ?? 0} competencies` : `${Number(board.tradeInfo.hours ?? 0).toLocaleString()}h`;
  const progressDetail = competencyBased ? `Registered competency sign-off · ${board.tradeInfo.rtiHours ?? 0} RTI hours` : 'Review required skills and complete supervisor verification.';
  ...
  const toolCards: PortalCard[] = [
    { href: '/host-shop/orientation', title: 'Required orientation', detail: 'Review the registered program, recruiting, payroll, WIOA, geofencing, work approval, competencies, transfer-credit rules, and compliance.', image: '/images/pages/training-classroom.webp' },
    ...
    { href: '/host-shop/dashboard/wages', title: 'Wage compliance', detail: 'Verify actual wages against the registered occupation and employer-specific RAPIDS schedule.', image: '/images/pages/business-meeting.webp' },
