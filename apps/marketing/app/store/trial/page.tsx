...
function TrialForm() {
  ...
  const purchaseInterval = searchParams.get('interval') === 'annual' ? 'annual' : 'monthly';
    ...
    return (
      <main className="min-h-[70vh] bg-slate-50 px-4 py-16">
        <div className="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2"><a href={`https://app.elevateforhumanity.org/login?redirect=${encodeURIComponent(purchaseIntent && purchasePlan ? `https://www.elevateforhumanity.org/store/plans?purchase=1&plan=${purchasePlan}&interval=${purchaseInterval}` : workspaceTarget)}`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-red-600 px-5 py-3 font-bold text-white hover:bg-brand-red-700">{purchaseIntent ? 'Sign in & continue to payment' : 'Open Login'} <ArrowRight className="h-4 w-4" /></a>{result.publicPreviewUrl ? <a href={result.publicPreviewUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-xl border border-slate-300 px-5 py-3 font-bold text-slate-800 hover:bg-slate-50">Open Public Preview</a> : <Link href="/store" className="inline-flex items-center justify-center rounded-xl border border-slate-300 px-5 py-3 font-bold text-slate-800 hover:bg-slate-50">Back to Store</Link>}</div>
        </div>
      </main>
    );
  }
      ...
      <section className="border-y border-slate-200 bg-slate-50 px-4 py-14">
        <div className="mx-auto grid max-w-5xl gap-10 lg:grid-cols-2">
          <div>
            ...
            <h1 className="mt-5 text-4xl font-black text-slate-950">{purchaseIntent ? `Set up your workspace, then purchase ${purchasePlan || 'your selected plan'}` : demoToken ? 'Keep what you just built' : recommendedCapabilities.length ? 'Start with your recommended setup' : 'Test the platform with your own workspace'}</h1>
          ...
          <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
            ...
            <p className="mt-3 text-center text-xs text-slate-500">{purchaseIntent ? 'Workspace setup comes first; secure Stripe payment is the next step.' : 'No credit card required for the 14-day trial.'}</p>
          </form>
        </div>
      </section>
    </main>
