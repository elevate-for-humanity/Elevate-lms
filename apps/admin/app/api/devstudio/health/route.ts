...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data } = await db.from('platform_secrets').select('key, value_enc').in('key', [
      'GROQ_API_KEY','GEMINI_API_KEY','OPENAI_API_KEY','ANTHROPIC_API_KEY','GITHUB_TOKEN',
    ]);
  ...
  return NextResponse.json({
    ...
    supabaseUrlPresent: !!(process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.SUPABASE_URL),
    supabaseServiceKeyPresent: !!process.env.SUPABASE_SERVICE_ROLE_KEY,
    nodeVersion: process.version,
    nextVersion,
    availableProviders: { groq: hasGroq, gemini: hasGemini, openai: hasOpenAI, anthropic: hasAnthropic },
    git: { endpoint: '/api/devstudio/git', remoteUrlPresent: !!(process.env.GITHUB_REMOTE_URL || process.env.GITHUB_REPO), tokenPresent: hasGitHub, pushScript: 'pnpm run git:push-main' },
    northflank: {
      ready: isNorthflankReady(),
      tokenPresent: !!(process.env.NORTHFLANK_API_TOKEN || process.env.NORTHFLANK_API_KEY || process.env.NF_API_TOKEN),
      projectIdPresent: !!getNorthflankProjectId(),
