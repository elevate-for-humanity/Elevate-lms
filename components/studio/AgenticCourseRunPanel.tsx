...
export function AgenticCourseRunPanel({
  ...
  const [goal, setGoal] = useState(
    `Finish ${courseTitle} as a complete learner-ready governed course, repair missing instructional content and assessments, generate required media, validate every required automated checklist, and publish through the canonical Course Builder as soon as all gates pass.`,
  );
  ...
  const terminal = Boolean(
    projectId && snapshot?.run && ['completed', 'failed', 'canceled'].includes(snapshot.run.status),
  );
  ...
  async function refresh(id = projectId) {
    ...
    const response = await fetch(
      `/api/admin/dev-studio/course-agent?projectId=${encodeURIComponent(id)}`,
      { cache: 'no-store' },
    );
  ...
  async function start(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !body.projectId)
        throw new Error(body.error || 'Unable to start Course Builder agent');
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-base font-bold text-slate-950">
            Visible build plan · persistent task graph
          </h3>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-slate-600">
            Course Builder AI completes the governed checklist and publishes automatically after
            every required validation gate passes. You can inspect and edit the course at any time.
          </p>
        </div>
        {projectId ? (
      ...
      {!projectId ? (
        <form onSubmit={start} className="mt-4 space-y-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-sm font-bold text-white hover:bg-violet-700 disabled:opacity-50"
          >
        ...
        <div className="mt-4 space-y-4">
          <div>
            <div className="mb-1 flex items-center justify-between text-xs font-semibold text-slate-600">
              <span>
                {snapshot?.run?.status ?? 'loading'} · {progress}% tasks complete
              </span>
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {(snapshot?.tasks ?? []).map((task) => (
              <div
                key={task.id}
                className={`rounded-xl border p-3 ${STATUS_STYLE[task.status] ?? STATUS_STYLE.queued}`}
              >
                ...
                {task.output?.summary ? (
                  <p className="mt-2 text-xs">{String(task.output.summary)}</p>
                ) : null}
                {Array.isArray(task.output?.blocking_issues) &&
                task.output.blocking_issues.length ? (
                  <div className="mt-2 space-y-1 text-xs">
                    {task.output.blocking_issues.slice(0, 4).map((issue: string) => (
          ...
          {snapshot?.media ? (
            <div className="grid grid-cols-2 gap-2 rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700 sm:grid-cols-4">
              <div className="flex items-center gap-1.5">
                <Video className="h-3.5 w-3.5" /> Lesson complete:{' '}
                <b>{snapshot.media.lessonComplete ?? 0}</b>
              </div>
              <div>
                Lesson rendering: <b>{snapshot.media.lessonRendering ?? 0}</b>
              </div>
              <div>
                Microclips complete: <b>{snapshot.media.microclipComplete ?? 0}</b>
              </div>
              <div>
                Failed media: <b>{snapshot.media.failed ?? 0}</b>
              </div>
            </div>
          ) : null}
          ...
          {snapshot?.course ? (
            <div className="rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700">
              <b>{String(snapshot.course.title ?? courseTitle)}</b> ·{' '}
              {String(snapshot.course.status ?? 'draft')} · review{' '}
              {String(snapshot.course.review_status ?? 'draft')} · generation{' '}
              {String(snapshot.course.generation_status ?? 'draft')}
            </div>
          ) : null}
          ...
          {active ? (
            <p className="text-[11px] text-slate-500">
              This run is persisted in Supabase and continues independently of this browser tab.
            </p>
          ) : null}
        </div>
      )}
