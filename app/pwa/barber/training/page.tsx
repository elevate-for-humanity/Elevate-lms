      ...
      <div className="flex items-start gap-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
          ...
          {module.locked ? (
            ...
            <BookOpen className="w-6 h-6 text-brand-red-400" />
          )}
        </div>
              ...
              <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full ${
                    progress === 100 ? 'bg-green-500' : 'bg-brand-red-500'
                  }`}
    ...
    <div className="flex items-center gap-4 bg-slate-800 rounded-xl p-4">
      ...
      {chapter.type === 'video' && !chapter.completed && chapter.contentUrl && (
        <a href={chapter.contentUrl} target="_blank" rel="noopener noreferrer">
          <Play className="w-5 h-5 text-brand-red-400" />
        </a>
      )}
    </div>
  );
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          ...
          <Link
            ...
            className="inline-block px-6 py-3 bg-brand-red-600 text-white rounded-xl font-medium"
          >
            Sign In
          </Link>
        </div>
      ...
      <div className="min-h-screen bg-slate-900 pb-20">
        <header className="bg-slate-800 px-4 pt-12 pb-6 safe-area-inset-top">
          <button 
            ...
            className="flex items-center gap-2 text-brand-red-400 mb-4"
          >
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-slate-800 border-t border-slate-700 px-6 py-3 safe-area-inset-bottom">
          <div className="flex justify-around">
            ...
            <Link href="/pwa/barber/training" className="flex flex-col items-center gap-1 text-brand-red-400">
              <BookOpen className="w-6 h-6" />
  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-20">
      <header className="bg-slate-700 px-4 pt-12 pb-6 safe-area-inset-top">
        ...
        <p className="text-brand-red-200 text-sm mb-1">{data.course.subtitle}</p>
        <p className="text-brand-red-300 text-xs">Powered by {data.course.partner}</p>
        
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 mt-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-brand-red-200 text-sm">Overall Progress</span>
      ...
      <div className="px-4 py-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-800 text-white rounded-xl pl-10 pr-4 py-3 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-red-500"
          />
        </div>
      </div>
