  ...
  try {
    ...
    switch (action) {
      case 'record_email':
        ...
        const emailSubmission = await recordEmailSubmission(applicationId, submittedBy, details);
      ...
      case 'record_portal':
        ...
        const portalSubmission = await recordPortalSubmission(applicationId, submittedBy, details);
      ...
      case 'update_status':
        ...
        await updateSubmissionStatus(body.submissionId, body.status, body.notes, body.performedBy);
        return NextResponse.json({ success: true, message: 'Status updated' });

      case 'add_timeline_event':
        if (!body.submissionId || !body.event) {
          return NextResponse.json({ error: 'submissionId and event required' }, { status: 400 });
        }
