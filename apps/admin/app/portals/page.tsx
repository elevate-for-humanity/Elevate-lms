...
import { HostShopPortalLauncher } from '@/components/admin/dashboard/HostShopPortalLauncher';
...
export default async function PortalSwitcherPage() {
  ...
  const entries = (
    Object.entries(PORTAL_MAP) as Array<[PortalKey, (typeof PORTAL_MAP)[PortalKey]]>
  ).filter(([key]) => key !== 'hostshop');
    ...
    <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6">
      <div className="mb-8">
        ...
        <p className="mt-3 max-w-3xl text-slate-600">
          Platform Admin can enter every canonical portal without being assigned a fake tenant,
          shop, employer, learner, or program-holder identity. Scoped portals retain their own
          data-boundary rules while exposing an admin oversight mode where required.
        </p>
      </div>
        ...
        {entries.map(([key, portal]) => {
          ...
          return (
            ...
            >
              ...
              <Link
                href={href}
                className="mt-5 inline-flex rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-black text-white hover:bg-slate-800"
              >
                Open portal
              </Link>
            </article>
