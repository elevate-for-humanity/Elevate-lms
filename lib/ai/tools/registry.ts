...
export const AI_TOOL_REGISTRY: readonly AIToolDefinition[] = [
  ...
  { name: 'organization.directory', description: 'Read approved public team roles and biographies from the Elevate organization directory.', method: 'GET', path: (input) => queryPath('/api/admin/dev-studio/org-directory', input, ['query']), classification: 'read', risk: 'low', scope: 'platform', allowedAgents: ['PARIS', 'ELLIE', 'LIZZY', 'ZORA'], allowedRoles: ADMIN_ROLES, requiredInput: ['query'], approvalRequired: false, idempotent: true, timeoutMs: 15_000, retryAttempts: 2, audit: true },
  { name: 'courses.generate', description: 'Create or resume one complete governed course through the Dev Studio agent and canonical Course Builder, including assessments, lesson media, program mapping, acceptance, and publication readiness.', method: 'POST', path: '/api/admin/dev-studio/course-agent', classification: 'write', risk: 'medium', scope: 'platform', allowedAgents: ['PARIS', 'LIZZY'], allowedRoles: PRIVILEGED_ADMIN_ROLES, requiredInput: ['goal'], approvalRequired: true, confirmationPhrase: 'CONFIRM BUILD COURSE', idempotent: true, timeoutMs: 120_000, retryAttempts: 1, audit: true },
  ...
  { name: 'video.generate', description: 'Plan and render a governed commercial through the canonical Media Studio pipeline and asset registry.', method: 'POST', path: '/api/admin/media-studio/commercial', classification: 'write', risk: 'medium', scope: 'platform', allowedAgents: ['PARIS', 'LIZZY'], allowedRoles: PRIVILEGED_ADMIN_ROLES, requiredInput: ['action','projectName','title','prompt','durationSeconds','aspectRatio'], approvalRequired: true, confirmationPhrase: 'CONFIRM RENDER COMMERCIAL', idempotent: false, timeoutMs: 120_000, retryAttempts: 1, audit: true },
