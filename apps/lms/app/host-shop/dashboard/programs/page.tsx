...
export const metadata = {
  ...
  description:
    'View the canonical occupation and registered-program status assigned to this Host Shop.',
...
export default async function HostShopProgramsPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Only programs assigned to this Host Shop are shown. The former all-program fallback has
            been removed.
          </p>
        </div>
        <Link
      ...
      <section
        className={`mt-6 rounded-3xl border p-7 ${configured ? 'border-green-200 bg-green-50' : 'border-amber-300 bg-amber-50'}`}
      >
        {configured ? (
          <CheckCircle2 className="h-8 w-8 text-green-700" />
        ) : (
          <AlertTriangle className="h-8 w-8 text-amber-800" />
        )}
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="mt-1 text-xl font-black text-slate-950">
            {board.programType || 'Not assigned'}
          </p>
        </section>
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
        <h2 className="text-xl font-black text-slate-950">
          Assigned occupation syllabi and Appendix A
        </h2>
        <div className="mt-4 grid gap-3">
          ...
          {!board.apprentices.length ? (
            <p className="rounded-xl bg-slate-50 p-4 text-sm font-medium text-slate-600">
              Syllabi and occupation standards appear automatically when an active apprentice
