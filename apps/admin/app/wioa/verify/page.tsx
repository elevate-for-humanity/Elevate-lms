...
export default async function WIOAVerifyPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  if (!id) {
    ...
    return (
      <main className="mx-auto max-w-5xl space-y-6 p-6">
        ...
        {!pending?.length ? <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center"><CheckCircle className="mx-auto h-8 w-8 text-emerald-600" /><p className="mt-3 font-bold text-emerald-900">No pending verifications.</p></div> : <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{pending.map((participant) => <Link key={participant.id} href={`/admin/wioa/verify?id=${participant.id}`} className="flex items-center gap-4 border-b border-slate-100 p-4 last:border-0 hover:bg-slate-50"><div className="rounded-full bg-amber-100 p-2"><Users className="h-5 w-5 text-amber-700" /></div><div className="min-w-0 flex-1"><p className="font-bold text-slate-900">{participant.first_name} {participant.last_name}</p><p className="truncate text-sm text-slate-500">{participant.email}</p></div><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800">{participant.eligibility_status}</span></Link>)}</div>}
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 p-6">
      <Link href="/admin/wioa/verify" className="inline-flex items-center gap-2 text-sm font-bold text-brand-blue-700"><ArrowLeft className="h-4 w-4" />Back to verification list</Link>
