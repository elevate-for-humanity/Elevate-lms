...
export default async function StudentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [enrollmentsRes, applicationsRes, progressRes, barberSubRes] = await Promise.all([
    db.from('program_enrollments').select('id,status,enrolled_at,payment_status,amount_paid_cents,program_id,program_slug,student_start_date,voucher_issued_date,voucher_paid_date,payout_due_date,payout_status,payout_paid_date,payout_notes').eq('user_id', id).order('enrolled_at', { ascending: false }),
