...
async function _POST(req: NextRequest) {
  ...
  if (!aiAvailable) {
    const fallback = `The ${program.title ?? program.title} program is available at ${PLATFORM_DEFAULTS.orgName}. Apply at ${PLATFORM_DEFAULTS.canonicalDomain}${program.applyUrl} or contact us for details.`;
  ...
  try {
    ...
    const answer =
      result.content ||
      `I can help with ${program.title ?? program.title} questions. Please try rephrasing.`;
