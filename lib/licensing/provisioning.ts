...
type ProvisioningStep = 'payment_received' | 'purchase_created' | 'tenant_created' | 'license_created' | 'admin_created' | 'email_sent' | 'completed' | 'failed' | 'rolled_back';
...
async function logProvisioningEvent(
  ...
  await supabase.from('provisioning_events').insert({ correlation_id: correlationId, step, status, tenant_id: tenantId || null, payment_intent_id: paymentIntentId || null, error: error || null, metadata: metadata || null, environment: ENVIRONMENT });
}
...
function generateTemporaryPassword(): string {
  ...
  let password = '';
  for (let i = 0; i < 16; i++) password += chars[randomBytes[i] % chars.length];
  return password;
}
...
export async function provisionLicense(ctx: ProvisioningContext): Promise<ProvisioningResult> {
  ...
  try {
    await logProvisioningEvent(correlationId, 'payment_received', 'completed', undefined, paymentIntentId, undefined, { email, product_id: productId, amount_cents: amountCents });
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .insert({ organization_name: orgName, contact_name: `${orgName} Admin`, contact_email: email, license_type: 'enterprise', product_slug: productId, stripe_payment_intent_id: paymentIntentId, stripe_checkout_session_id: sessionId, status: 'paid', amount_cents: amountCents, currency })
      .select('id')
      .maybeSingle();
    ...
    const { data: tenant, error: tenantError } = await supabase
      .from('tenants')
      .insert({ name: orgName, slug, license_status: 'active', stripe_customer_id: ctx.metadata?.stripe_customer_id, stripe_subscription_id: ctx.metadata?.stripe_subscription_id, settings: { product_id: productId, plan_id: ctx.metadata?.plan_id, license_type: ctx.metadata?.license_type, provisioned_at: new Date().toISOString(), correlation_id: correlationId } })
      .select('id')
      .maybeSingle();
    ...
    const { data: license, error: licenseError } = await supabase
      .from('licenses')
      .insert({ license_key: licenseKey, customer_email: email, tenant_id: tenantId, tier: 'enterprise', status: 'active', max_users: 100, max_deployments: 1, features: ctx.metadata?.features ? Object.keys(ctx.metadata.features) : [], metadata: { product_id: productId, correlation_id: correlationId } })
      .select('id')
      .maybeSingle();
    ...
    if (existingUser) {
      ...
      const { data: newUser, error: userError } = await supabase.auth.admin.createUser({ email, password: temporaryPassword, email_confirm: true, user_metadata: { tenant_id: tenantId, role: 'admin' } });
    ...
    try {
      const { data: magicLink } = await supabase.auth.admin.generateLink({ type: 'magiclink', email, options: { redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL}/auth/callback?redirect=${encodeURIComponent('/admin')}` } });
      const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/email/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-internal-secret': process.env.CRON_SECRET ?? '' },
        body: JSON.stringify({ to: email, subject: 'Your Elevate Platform License is Ready', template: 'license-provisioned', data: { organizationName: orgName, email, licenseKey, loginUrl: magicLink?.properties?.action_link || `${process.env.NEXT_PUBLIC_SITE_URL}/login`, adminUrl: `${process.env.NEXT_PUBLIC_SITE_URL}/admin`, temporaryPassword, supportEmail: 'info@elevateforhumanity.org' } }),
      });
      if (!response.ok) throw new Error(`Provisioning email endpoint returned ${response.status}`);
    ...
    await logAuditEvent({ action: 'LICENSE_PROVISIONED', actor_id: adminUserId || 'system:license_provisioning', target_type: 'license', target_id: licenseId, metadata: { correlation_id: correlationId, tenant_id: tenantId, product_id: productId, payment_intent_id: paymentIntentId, email, amount_cents: amountCents } });
    return { success: true, tenantId, licenseId, licenseKey, adminUserId };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'License provisioning failed';
    ...
    await logAuditEvent({ action: 'LICENSE_PROVISIONING_FAILED', actor_id: 'system:license_provisioning', target_type: 'license', target_id: correlationId, metadata: { correlation_id: correlationId, tenant_id: tenantId, payment_intent_id: paymentIntentId, email, error: errorMessage } });
...
export async function suspendLicense(tenantId: string, reason: string): Promise<void> {
  ...
  await logAuditEvent({ action: 'LICENSE_SUSPENDED', actor_id: 'system:license_enforcement', target_type: 'tenant', target_id: tenantId, metadata: { reason, correlation_id: correlationId } });
...
export async function reactivateLicense(tenantId: string): Promise<void> {
  ...
  await logAuditEvent({ action: 'LICENSE_REACTIVATED', actor_id: 'system:license_enforcement', target_type: 'tenant', target_id: tenantId, metadata: { correlation_id: correlationId } });
