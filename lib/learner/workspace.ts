...
export async function loadLearnerWorkspace(userId: string, role = 'student'): Promise<LearnerWorkspace> {
  ...
  const [profileResult, progressResult, requirementResult, agreementResult, binderResult] = await Promise.all([
    supabase.from('profiles').select('full_name,phone,onboarding_completed,orientation_completed').eq('id', userId).maybeSingle(),
  ...
  const orientationComplete = Boolean(profileResult.data?.orientation_completed || progress?.status === 'completed' || progress?.completed_at);
  ...
  const onboardingSteps: LearnerOnboardingStep[] = [
    ...
    { id: 'orientation', title: 'Complete learner orientation', description: 'Review how training, support, progress, and credentials work.', href: '/lms/orientation', status: orientationComplete ? 'complete' : 'missing', completedAt: progress?.completed_at ?? null, blocking: true },
