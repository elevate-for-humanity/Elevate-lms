...
export default function MicrocourseCart({ courses }: { courses: CatalogMicrocourse[] }) {
  ...
  const chosen = useMemo(
    () => courses.filter((course) => cart.includes(course.slug)),
    [cart, courses],
  );
  ...
  async function checkout(slugs: string[]) {
    ...
    setBusy(true);
    try {
      const cartKey = [...slugs].sort().join(',');
      const checkoutAttemptId = checkoutAttempts.current.get(cartKey) || crypto.randomUUID();
      checkoutAttempts.current.set(cartKey, checkoutAttemptId);
      const response = await fetch('/api/microcourses/checkout', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ slugs, checkoutAttemptId }),
      });
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_22rem]">
          ...
          return (
            ...
            >
              <p className="text-sm font-semibold text-blue-700">
                {course.microcourse_providers?.display_name}
              </p>
              ...
              <div className="mt-5 flex items-end justify-between gap-4">
                <div>
                  <p className="text-2xl font-black">
                    {isFree ? 'Free' : money(course.retail_price_cents, course.currency)}
                  </p>
                  <p className="text-xs text-slate-500">
                    {isFree
                      ? 'No payment or credit card required.'
                      : 'Secure card or bank payment through QuickBooks.'}
                  </p>
                </div>
                {isFree ? (
                  <a
                    href={course.provider_enrollment_url || '/login'}
      ...
      <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:sticky lg:top-24">
        <div className="flex items-center gap-2">
          <ShoppingCart />
          <h2 className="text-xl font-bold">Cart ({chosen.length})</h2>
        </div>
        {chosen.length === 0 ? (
          <p className="mt-4 text-slate-600">Add a microcourse to begin.</p>
        ) : (
          <ul className="mt-4 divide-y">
            {chosen.map((course) => (
              <li key={course.id} className="flex justify-between gap-4 py-3">
                <span>{course.title}</span>
                <strong>{money(course.retail_price_cents, course.currency)}</strong>
              </li>
            ))}
          </ul>
        )}
        <div className="mt-5 flex justify-between text-lg">
          <span>Total</span>
          <strong>{money(total, chosen[0]?.currency || 'usd')}</strong>
        </div>
        <button
        ...
        >
          {busy ? 'Opening QuickBooks…' : 'Checkout'}
        </button>
      </aside>
    </div>
  );
