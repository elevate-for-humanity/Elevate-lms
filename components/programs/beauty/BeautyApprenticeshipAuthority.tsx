  ...
  return {
    ...
    additionalProperty: [
      ...
      },
      {
        '@type': 'PropertyValue',
        name: 'Funding status',
        value: program.funding?.wioa_eligible && program.funding?.etpl_approved
    ...
    <section className="bg-white px-4 py-14 sm:py-16" aria-labelledby={`${program.slug}-authority-heading`}>
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-8 lg:grid-cols-[1.08fr_.92fr] lg:items-start">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.15em] text-rose-700">Program Authority & Host-Site Pathway</p>
            <h2 id={`${program.slug}-authority-heading`} className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">
            ...
            <p className="mt-4 max-w-3xl text-base font-medium leading-7 text-slate-700">
              Elevate separates the training model, funding status, host-site relationship, and licensing requirements so applicants and workforce partners can see exactly what applies to this track.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <ShieldCheck className="h-6 w-6 text-rose-700" />
                <div className="mt-3 text-sm font-black text-slate-950">{registered ? 'Registered Apprenticeship' : 'Apprenticeship Pathway'}</div>
                <p className="mt-1 text-xs font-medium leading-5 text-slate-600">{registered ? 'Listed in Elevate’s canonical RAPIDS program registry.' : 'Registration status must be confirmed before a DOL-specific claim is used.'}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <CircleDollarSign className="h-6 w-6 text-emerald-700" />
                <div className="mt-3 text-sm font-black text-slate-950">{wioaEtpl ? 'WIOA / ETPL Eligible' : 'Funding Review Required'}</div>
                <p className="mt-1 text-xs font-medium leading-5 text-slate-600">Funding is never represented as guaranteed without current program and participant authorization.</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <GraduationCap className="h-6 w-6 text-violet-700" />
                <div className="mt-3 text-sm font-black text-slate-950">{hours.toLocaleString()} Structured Hours</div>
                <p className="mt-1 text-xs font-medium leading-5 text-slate-600">Pulled from the canonical program hours breakdown shown on this page.</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm font-medium leading-6 text-slate-700">
                  Host-site availability changes by region. Applicants can select an active approved location when available or ask a preferred licensed business to apply to the Elevate Host Site network.
                </p>
              </div>
            </div>
            {locations.length > 0 ? (
              <div className="mt-4 space-y-2">
                {locations.slice(0, 8).map((location) => (
                  <div key={`${location.city}-${location.state}`} className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-bold text-slate-800">
                    <MapPin className="h-4 w-4 text-rose-700" /> {location.city}, {location.state}
                  </div>
                ))}
              </div>
            ) : (
              <div className="mt-4 rounded-xl bg-white p-3 text-sm font-semibold text-slate-700">Current openings are confirmed during placement review rather than advertised for cities where no active host record is on file.</div>
            )}
        ...
        <div className="mt-12">
          <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.14em] text-slate-500"><BadgeCheck className="h-5 w-5 text-rose-700" /> Frequently asked questions</div>
          <div className="mt-4 divide-y divide-slate-200 overflow-hidden rounded-2xl border border-slate-200 bg-white">
            {faqs.map((faq) => (
              <details key={faq.question} className="group p-5 open:bg-slate-50">
                <summary className="cursor-pointer list-none pr-6 text-base font-black text-slate-950">{faq.question}</summary>
