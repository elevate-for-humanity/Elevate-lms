...
{/* Avatar Guide */}
      ...
      {/* Hero */}
      <section className="relative bg-brand-red-700 text-white py-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <span className="inline-block bg-brand-red-600 text-white text-sm font-bold px-4 py-1 rounded-full mb-4">
                Accessibility
              </span>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="#pricing" className="inline-flex items-center gap-2 bg-white text-brand-red-700 px-6 py-3 rounded-lg font-bold hover:bg-brand-red-50 transition">
                  View Pricing
                  <ArrowRight className="w-5 h-5" />
                </Link>
                <Link href="/demo" className="inline-flex items-center gap-2 bg-brand-red-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-brand-red-500 transition border border-brand-red-500">
                  Request Demo
                </Link>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={item.label} className="bg-white/10 backdrop-blur rounded-xl p-4 text-center">
                  <item.icon className="w-8 h-8 mx-auto mb-2 text-brand-red-300" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {features.map((feature) => (
              <div key={feature.title} className="bg-white rounded-2xl overflow-hidden border-2 border-gray-200 hover:border-brand-red-500 hover:shadow-xl transition-all group">
                  ...
                  <ul className="space-y-2">
                    {feature.items.map((item) => (
                      <li key={item} className="flex items-center gap-2 text-sm text-black">
                        <Circle className="w-4 h-4 text-brand-red-600 flex-shrink-0" />
                        {item}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {[
              ...
              <div key={plan.name} className={`bg-white rounded-2xl p-8 border-2 ${plan.popular ? 'border-brand-red-500 shadow-xl' : 'border-gray-200'}`}>
      ...
      <section className="py-16 bg-brand-red-700 text-white">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-brand-red-100 mb-8">Schedule a demo to see our accessibility tools in action.</p>
          <Link href="/demo" className="inline-flex items-center gap-2 bg-white text-brand-red-700 px-8 py-4 rounded-lg font-bold hover:bg-brand-red-50 transition">
            Schedule Demo
          </Link>
        </div>
      </section>
