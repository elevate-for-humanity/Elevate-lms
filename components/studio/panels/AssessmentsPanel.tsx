...
export function AssessmentsPanel() {
  ...
  async function hydrate(lesson: (typeof state.lessons)[number]) {
    ...
    setError('');
    try {
      const moduleTitle = state.modules.find((module) => module.id === lesson.module_id)?.title ?? 'Course';
  ...
  return (
    <section className="p-6">
      ...
      <div className="mt-4 space-y-2">
        {assessments.length ? assessments.map((lesson) => (
          <div key={lesson.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3">
            <div><div className="font-semibold">{lesson.title}</div><div className="text-xs text-slate-500">{lesson.lesson_type} · pass {lesson.passing_score ?? 70}%</div></div>
            <button onClick={() => void hydrate(lesson)} disabled={runningId === lesson.id} className="rounded-lg bg-cyan-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">{runningId === lesson.id ? 'Generating…' : 'Generate / refresh questions'}</button>
          </div>
