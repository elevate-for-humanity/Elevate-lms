...
export default async function BillingPage() {
  ...
  const openStatuses = new Set(['draft', 'open', 'pending', 'unpaid', 'overdue', 'past_due']);
  const totalOwed = invoices
    .filter((i) => openStatuses.has(String(i.status).toLowerCase()))
              ...
              {invoices.map((inv) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {inv.payment_url && openStatuses.has(String(inv.status).toLowerCase()) ? (
                      <a
