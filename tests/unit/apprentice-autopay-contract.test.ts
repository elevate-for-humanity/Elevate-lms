...
describe('apprentice automatic-payment contract', () => {
  ...
  const reconciliation = read(
    'apps/admin/app/api/cron/reconcile-apprentice-payments/route.ts',
  );
  const scheduler = read('.github/workflows/cron-scheduler.yml');
  const legacy83 = read(
    'supabase/migrations/20260922111421_legacy_83_billing_preference.sql',
  );
  ...
  it('reconciles PayPal automatic collections into QuickBooks every week', () => {
    ...
    expect(scheduler).toContain(
      'POST "$ADMIN_URL/api/cron/reconcile-apprentice-payments"',
    );
  });
  ...
  it('keeps Legacy 83 monthly on the 15th and outside apprentice tuition', () => {
    ...
    expect(legacy83).toContain(
      'Keep separate from weekly apprentice tuition collection.',
    );
  });
});
