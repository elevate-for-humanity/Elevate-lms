...
export default async function FinancialAssurancePage() {
  ...
  const [{ data: records }, { data: summary }] = await Promise.all([
    ...
    db.from('v_admin_financial_assurance_summary').select('*').maybeSingle(),
  ]);
        ...
        <div className="flex items-center justify-between mt-4 mb-6">
          <div>
            ...
            <p className="text-slate-700 text-sm mt-1">
              Live records — bonds, insurance, letters of credit
            </p>
          </div>
          <Link
          ...
          <div className="px-4 py-3 border-b bg-slate-50 flex items-center justify-between">
            ...
            {s?.active_coverage_total > 0 && (
              <span className="text-sm text-slate-600">
                Active coverage:{' '}
                <strong>${Number(s.active_coverage_total).toLocaleString()}</strong>
              </span>
            )}
          </div>
                ...
                {(records ?? []).map((r: any) => (
                  <tr key={r.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-600">
                      {r.policy_or_reference_number ?? '—'}
                    </td>
                    <td className="px-4 py-3 text-slate-600">
                      {r.coverage_amount != null
                        ? `$${Number(r.coverage_amount).toLocaleString()}`
                        : '—'}
                    </td>
