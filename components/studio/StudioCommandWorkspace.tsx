...
const CloudBrowserWorkspace = dynamic(() => import('./CloudBrowserWorkspace'), {
  ssr: false,
  loading: () => <div className="flex h-full items-center justify-center bg-slate-950 text-sm text-slate-400">Connecting isolated browser…</div>,
});

type InspectionMode = 'preview' | 'browser';

export default function StudioCommandWorkspace({ workspaces }: { workspaces: Array<{ id: string; label: string; route: string }> }) {
    ...
    <div className="flex h-[100dvh] min-h-0 min-w-0 flex-col overflow-hidden bg-white lg:h-full">
      <header className="shrink-0 border-b border-slate-200 bg-slate-950 text-white">
        <div className="flex min-h-12 min-w-0 items-center gap-2 px-3">
          ...
          <button type="button" onClick={() => { setConversationKey((value) => value + 1); setMobileSurface('chat'); }} className="ml-auto inline-flex min-h-9 shrink-0 items-center gap-1.5 rounded-lg border border-white/20 px-3 text-xs font-bold hover:bg-white/10">
            <Plus className="h-4 w-4" aria-hidden="true" /> New task
          </button>
          <div className="hidden items-center gap-1 sm:flex" role="group" aria-label="Choose AI agent">
            {(['ELLIE', 'LIZZY', 'PARIS'] as const).map((agent) => (
              <button key={agent} type="button" aria-pressed={selectedAgent === agent} onClick={() => { setSelectedAgent(agent); setConversationKey((value) => value + 1); }} className={`rounded-lg px-3 py-2 text-xs font-black ${selectedAgent === agent ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-white/10'}`}>
                {agent[0] + agent.slice(1).toLowerCase()}
              </button>
            ))}
          </div>
        </div>
        <nav aria-label="Studio tools" className="scrollbar-hide flex min-w-0 items-center gap-1 overflow-x-auto border-t border-white/10 px-2 py-1.5">
          <button type="button" onClick={() => setMobileSurface('chat')} className="inline-flex min-h-9 shrink-0 items-center gap-2 rounded-lg bg-white/10 px-3 text-xs font-bold hover:bg-white/15">
            <MessageSquare className="h-4 w-4" aria-hidden="true" /> Chat
          </button>
          {workspaces.map((workspace) => (
            <Link key={workspace.id} href={workspace.route} className="inline-flex min-h-9 shrink-0 items-center rounded-lg px-3 text-xs font-semibold text-slate-300 hover:bg-white/10 hover:text-white">{workspace.label}</Link>
          ))}
        </nav>
      </header>

      <div className="flex min-h-0 min-w-0 flex-1">
        <section className={`${mobileSurface === 'chat' ? 'flex' : 'hidden'} min-h-0 min-w-0 flex-1 flex-col border-r border-slate-200 lg:flex lg:basis-[42%]`}>
        ...
        <section className={`${mobileSurface === 'tool' ? 'flex' : 'hidden'} min-h-0 min-w-0 flex-1 flex-col bg-slate-950 lg:flex lg:basis-[58%]`} aria-label="Studio workspace">
          <header className="flex min-h-12 shrink-0 items-center gap-2 border-b border-slate-800 bg-slate-900 px-3 text-white">
            <span className="mr-auto text-xs font-black">Active workspace</span>
            <button type="button" onClick={() => setMode('preview')} className={`inline-flex items-center gap-1 rounded-md px-3 py-2 text-xs font-bold ${mode === 'preview' ? 'bg-cyan-500 text-slate-950' : 'text-slate-300 hover:bg-slate-800'}`}><Eye className="h-4 w-4" aria-hidden="true" /> Preview</button>
