...
import {
  ensureProgramHolderPayoutAccount,
  syncProgramHolderPayoutAccount,
...
const appUrl = () =>
  (process.env.NEXT_PUBLIC_APP_URL || 'https://app.elevateforhumanity.org').replace(/\/$/, '');
async function holderContext() {
...
export async function GET() {
  ...
  const [readiness, payout] = await Promise.all([
  ...
  try {
    return NextResponse.json({
      ...(await syncProgramHolderPayoutAccount(ctx, data.stripe_account_id)),
      onboardingReady: readiness.ready,
      missingRequirements: readiness.missing,
    });
  } catch {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!stripe)
      return NextResponse.json(
        { error: 'Stripe payout processing is unavailable.' },
        { status: 503 },
      );
    if (action === 'dashboard') {
      ...
      if (!ready.transfersEnabled || !ready.payoutsEnabled)
        return NextResponse.json(
          { error: 'Complete payout verification before accessing funds.' },
          { status: 409 },
        );
      ...
      stripe.v2.core.accountLinks.create({
        ...
        use_case: {
          ...
          account_onboarding: {
            ...
            refresh_url: `${appUrl()}/program-holder/payouts?onboarding=refresh`,
            return_url: `${appUrl()}/program-holder/payouts?onboarding=returned`,
          },
        },
      }),
    );
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Unable to start payout onboarding.' },
      { status: 500 },
    );
  }
}
