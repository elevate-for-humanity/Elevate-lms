    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <Breadcrumbs
            items={[{ label: 'Workforce Board', href: '/workforce-board' }, { label: 'Dashboard' }]}
          />
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex gap-3">
              <Link
                ...
                className="inline-flex items-center px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 bg-white hover:bg-white"
              >
              ...
              <Link
                ...
                className="inline-flex items-center px-4 py-2 bg-brand-blue-600 text-white rounded-lg text-sm font-medium hover:bg-brand-blue-700"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
          ...
          <p className="text-xs text-black mt-4">
            Employment rate and median wage gain require post-exit follow-up data. Connect
            employment outcomes to enable these indicators.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-900 flex items-center">
                <AlertTriangle className="w-5 h-5 mr-2 text-brand-orange-500" />
                At-Risk Participants
              </h2>
              <span className="bg-brand-orange-100 text-brand-orange-800 text-xs font-medium px-2.5 py-0.5 rounded-full">
                {atRiskCount || 0} flagged
              </span>
            </div>
            {atRiskParticipants && atRiskParticipants.length > 0 ? (
              <div className="space-y-3">
                {atRiskParticipants.map((p: any) => (
                  <div
                    key={p.id}
                    className="flex items-center justify-between p-3 bg-brand-orange-50 rounded-lg"
                  >
                    <div>
                    ...
                    <Link
                      href={`/workforce-board/participants/${p.id}`}
            ...
            <Link
              href="/workforce-board/participants?filter=at-risk"
              className="block mt-4 text-center text-sm text-brand-blue-600 hover:text-brand-blue-700 font-medium"
            >
              View All At-Risk Participants →
            </Link>
          </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-black">
                        {e.programs?.name || e.programs?.title || 'Unknown Program'}
                      </p>
                    </div>
                    <div className="text-right">
                      <span
                        className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${e.status === 'active' ? 'bg-brand-green-100 text-brand-green-800' : e.status === 'completed' ? 'bg-brand-blue-100 text-brand-blue-800' : e.status === 'pending' ? 'bg-slate-100 text-slate-700' : 'bg-white text-slate-800'}`}
                      >
                        {e.status}
                      </span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-slate-900 flex items-center">
              <Building2 className="w-5 h-5 mr-2 text-brand-blue-600" />
              Program Overview
            </h2>
            <Link
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link
            href="/workforce-board/dashboard"
            className="flex flex-col items-center p-6 bg-white rounded-xl shadow-sm border border-slate-200 hover:border-brand-blue-300 hover:shadow-md transition"
          >
