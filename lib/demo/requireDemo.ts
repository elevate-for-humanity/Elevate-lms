...
export async function requireDemo(): Promise<DemoAuthResult> {
  ...
  if (!isDemoTenant(tenantSlug)) {
    redirect(
      `/demo?message=${encodeURIComponent('Demo features are only available in the demo environment')}`,
    );
  }
  ...
  if (!ALLOWED_DEMO_ROLES.includes(userRole)) {
    redirect(
      `/demo?message=${encodeURIComponent('Your role does not have access to demo features')}`,
    );
  }
