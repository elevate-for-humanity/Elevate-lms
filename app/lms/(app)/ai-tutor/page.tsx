...
export default function AITutorPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Header */}
      <div className="bg-brand-red-600 text-white py-6">
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-brand-red-100 text-sm">Your 24/7 learning assistant</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="h-[500px] overflow-y-auto p-6 space-y-4">
            ...
            {isLoading && (
              <div className="flex gap-3">
                <div className="w-8 h-8 rounded-full bg-brand-red-100 text-brand-red-600 flex items-center justify-center">
                  <Bot className="w-4 h-4" />
                </div>
                <div className="bg-slate-100 rounded-2xl px-4 py-3">
                  <Loader2 className="w-5 h-5 animate-spin text-brand-red-600" />
                </div>
              </div>
            )}
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {QUICK_PROMPTS.map((prompt, index) => (
                <button
                  ...
                  disabled={isLoading}
                  className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm text-slate-600 hover:border-brand-red-300 hover:text-brand-red-600 transition whitespace-nowrap disabled:opacity-50"
                >
          ...
          <div className="border-t border-slate-200 p-4">
            <div className="flex gap-3">
              <textarea
                ...
                className="flex-1 resize-none border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-brand-red-500 focus:border-transparent"
                rows={2}
                disabled={isLoading}
              />
              <button
                ...
                className="px-6 bg-brand-red-600 text-white rounded-xl font-semibold hover:bg-brand-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        {/* Tips */}
        <div className="mt-6 bg-brand-red-50 rounded-xl p-4 border border-brand-red-100">
          <h3 className="font-semibold text-brand-red-900 mb-2">Tips for better results:</h3>
          <ul className="text-sm text-brand-red-800 space-y-1">
