...
export default function CapabilitySubscriptionDemo({
  ...
  const recommendedAddOns = /workforce|compliance|government/i.test(categoryLabel)
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-300">
              {categoryLabel} guided product tour
            </p>
            <h1 className="mt-2 text-3xl font-black text-white sm:text-5xl">
              Understand {name} before you activate it.
            </h1>
            <p className="mt-4 max-w-3xl text-base font-semibold leading-7 text-slate-100">
              {description}
            </p>
          </div>
          <button
            ...
            <aside className="border-t border-white/10 bg-white p-6 text-slate-950 lg:border-l lg:border-t-0 sm:p-8">
              ...
              <div className="mt-7 rounded-xl border border-cyan-200 bg-cyan-50 p-4">
                <div className="flex items-center gap-2 font-black text-slate-950">
                  <Sparkles className="h-5 w-5 text-cyan-700" /> Trial first
                </div>
                <p className="mt-2 text-sm font-semibold leading-6 text-slate-800">
                  Customers can see the workflow before they commit, then upgrade only for the
                  capacity and automation they want to keep.
                </p>
              </div>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          <article className="rounded-2xl border border-emerald-400/30 bg-emerald-950/40 p-6">
            ...
            <h2 className="mt-2 text-xl font-black">
              Turn a disconnected task into a tracked workflow
            </h2>
            <p className="mt-2 text-sm font-semibold leading-6 text-slate-200">
              Reduce manual handoffs, preserve one accountable record, and give staff a repeatable
              process they can measure instead of rebuilding work in email and spreadsheets.
            </p>
          </article>
          <article className="rounded-2xl border border-cyan-400/30 bg-cyan-950/40 p-6">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-slate-200">
              A serious buyer should be able to verify ownership, status, history, outcomes, and
              exceptions. Elevate keeps those controls in the operating workflow instead of treating
              compliance as an after-the-fact report.
            </p>
          </article>
          <article className="rounded-2xl border border-orange-400/30 bg-orange-950/40 p-6">
            ...
            <ul className="mt-3 space-y-2">
              {recommendedAddOns.map((item) => (
                <li key={item} className="flex gap-2 text-sm font-bold text-slate-100">
        ...
        <div className="mt-8 rounded-2xl border border-white/15 bg-white/5 p-6 sm:flex sm:items-center sm:justify-between sm:gap-6">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black">
              Test it, select the right capacity, then pay securely.
            </h2>
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-200">
              Start a no-card trial or choose a base plan and add-ons. Workspace setup happens
              before payment so the subscription and entitlements attach to the correct
              organization.
            </p>
          </div>
          <div className="mt-5 grid shrink-0 gap-2 sm:mt-0">
            <Link
              href="/store/plans"
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-brand-red-600 px-6 font-black text-white hover:bg-brand-red-700"
            >
              Choose plan & payment <CreditCard className="h-4 w-4" />
            </Link>
            <Link
              href="/store/trial"
              className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/30 px-6 font-bold text-white hover:bg-white/10"
            >
              Start 14-day trial
            </Link>
          </div>
