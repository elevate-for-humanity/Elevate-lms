...
async function _POST(req: Request) {
  try {
    ...
    if (!imageBase64) {
      return NextResponse.json({ verified: false, reason: 'Missing image' }, { status: 400 });
    }
    ...
    if (!meetingId) {
      return NextResponse.json({ verified: false, reason: 'Missing meetingId' }, { status: 400 });
    }
    ...
    if (meetingError || !meeting) {
      return NextResponse.json({ verified: false, reason: 'Meeting not found' }, { status: 404 });
    }
