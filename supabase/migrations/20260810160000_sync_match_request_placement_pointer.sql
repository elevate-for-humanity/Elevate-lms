-- Keep the direct match-request -> placement pointer synchronized with the
-- canonical apprentice_placements.match_request_id relationship.

update public.host_shop_match_requests r
   set placement_id = (
     select ap.id
       from public.apprentice_placements ap
      where ap.match_request_id = r.id
      order by ap.updated_at desc nulls last, ap.created_at desc
      limit 1
   ),
       updated_at = now()
 where r.placement_id is null
   and exists (
     select 1 from public.apprentice_placements ap where ap.match_request_id = r.id
   );
...
begin
  ...
  update public.host_shop_match_requests
     set status = p_status,
         ...
         placement_id = case when p_status = 'approved' then v_placement_id else placement_id end,
         updated_at = now()
