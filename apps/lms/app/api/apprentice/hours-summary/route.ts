...
async function _GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      summary: {
        completion_basis: 'competency',
        program_slug: barber.slug,
        ...
        required_hours: null,
        required_ojl_hours: null,
        required_rti_hours: barber.relatedInstructionHours,
        remaining_hours: null,
        remaining_ojl_hours: null,
        remaining_rti_hours: remainingRtiHours,

        competency_count_required: barber.competencyCount,
        competencies_completed: completedCompetencies,
        competencies_remaining: Math.max(0, barber.competencyCount - completedCompetencies),
        ...
        rapids_occupation_code: barber.rapidsCode,
        rapids_sponsor_id: RAPIDS_CONFIG.registrationId,
        ...
        apprentice_to_mentor_ratio: barber.apprenticeToMentorRatio,
        probationary_hours: barber.probationaryHours,

        note: 'Barber 0030CB is competency-based. OJL hours remain auditable work records and are not used as a fixed completion denominator.',
      },
    });
  } catch (error) {
    logger.error(
