...
export async function getProgramHolderWorkspace(): Promise<ProgramHolderWorkspace> {
  ...
  if (ctx.mode === 'admin') {
    return { mode: 'admin', holder: null, programs: [], enrollments: [], applicants: [], hours: [], documents: [], reports: [], courseAssignments: [], payoutProfile: null, payoutSchedules: [] };
  }

  const { db, holderId, programIds, profile } = ctx;
  const [holderRes, programsRes, enrollmentsRes, applicantsRes, hoursRes, documentsRes, reportsRes, coursesRes, payoutRes, schedulesRes] = await Promise.all([
    ...
    db.from('program_holder_payouts').select('stripe_account_id,external_account_last4,bank_name,account_type,payouts_enabled,charges_enabled,transfers_enabled,verification_status,quickbooks_sync_status,last_stripe_sync_at').eq('user_id', profile.id).maybeSingle(),
    db.from('payout_schedules').select('id,enrollment_id,program_id,total_payout_cents,increment_1_cents,increment_2_cents,increment_1_status,increment_2_status,increment_1_release_date,increment_2_release_date,increment_1_paid_at,increment_2_paid_at').eq('program_holder_id', holderId).order('created_at', { ascending: false }),
  ]);
