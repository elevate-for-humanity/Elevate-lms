...
const EMPTY_ELIGIBILITY: EligibilityAnswers = {
  ...
  educationLevel: '',
  hasDiplomaOrGed: null,
  enrolledInGed: null,
  workAuthorized: null,
  activeWarrant: null,
  pendingCharges: null,
  onProbationParole: null,
  legalNotes: '',
  canAttendSchedule: null,
  hasTransportationPlan: null,
  canMeetPhysical: null,
  willingToFollowRules: null,
  willingJobReadiness: null,
  unavailableTimes: '',
  motivation: '',
  agreesVerification: false,
...
export default function StudentApplicationForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (applicationType === 'enrollment') {
      ...
      if (decision.status === 'ineligible') {
        setError(
          'Your application cannot be submitted because one or more eligibility requirements are not met. Please review the eligibility section above.',
        );
        setLoading(false);
        return;
      }
    ...
    try {
      trackEvent(
        'form_submit',
        applicationType === 'inquiry' ? 'inquiry' : 'application',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {currentStep === 1 && (<div className="bg-white border border-slate-200 rounded-lg p-6">
        ...
        {applicationType === 'inquiry' && (
          <div className="mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4">
            <p className="text-sm text-blue-800 leading-relaxed">
              <strong>Inquiry:</strong> We'll create your account and an advisor will follow up with
              program details, costs, and next steps. No payment is required at this stage.
            </p>
          </div>
        )}
        ...
        {applicationType === 'enrollment' &&
          selectedProgram &&
          WAITLIST_PROGRAMS.has(selectedProgram) && (
            <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
              <p className="text-sm font-semibold text-amber-800 mb-2">
                This program currently has a waitlist.
              </p>
              <p className="text-sm text-amber-700 leading-relaxed mb-3">
                New enrollment spots are limited. Join the waitlist and we'll contact you as soon as
                a seat opens.
              </p>
              <Link
        ...
        {applicationType === 'enrollment' && (
          <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
            ...
            <p className="text-sm text-amber-700 leading-relaxed">
              ...
              plan to use{' '}
              <strong>
                WIOA, WorkOne, EmployIndy, Workforce Ready Grant, or any state or federal funding
              </strong>
              , you must have written approval from your funding agency before enrollment can be
              completed.{' '}
              <a
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-6 space-y-8">
          <div>
            ...
            <p className="text-sm text-slate-500">
              All fields are required. Your answers determine your eligibility status before your
            ...
            <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex gap-3 items-start">
              ...
              <div>
                <p className="font-bold text-red-800 text-sm mb-1">
                  You do not currently meet the baseline requirements for this program.
                </p>
                <p className="text-red-700 text-sm">
                  If you believe this is incorrect or want to discuss alternate options, contact our
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl flex gap-3 items-start">
              ...
              <div>
                <p className="font-bold text-amber-800 text-sm mb-1">
                  Your application requires additional review before a decision can be made.
                </p>
                <p className="text-amber-700 text-sm">
                  A team member may contact you for documentation or clarification. You may still
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Are you currently receiving SNAP (food assistance)?
              </label>
              <YesNo
                value={eligibility.hasSnap}
                onChange={(v) => setElig('hasSnap', v)}
                name="hasSnap"
              />
            </div>
            <div>
              ...
              <YesNo
                value={eligibility.hasTanf}
                onChange={(v) => setElig('hasTanf', v)}
                name="hasTanf"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Were you referred by WorkOne, IMPACT, or another workforce partner?
              </label>
              <YesNo
                value={eligibility.hasReferral}
                onChange={(v) => setElig('hasReferral', v)}
                name="hasReferral"
              />
            </div>
            {eligibility.hasReferral && (
              <div className="grid sm:grid-cols-3 gap-3 pl-4 border-l-2 border-slate-200">
                <div>
                  ...
                  <input
                    type="text"
                    value={eligibility.referralSource}
                    onChange={(e) => setElig('referralSource', e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                    placeholder="e.g. WorkOne Indianapolis"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">
                  ...
                  <input
                    type="text"
                    value={eligibility.caseManagerName}
                    onChange={(e) => setElig('caseManagerName', e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="email"
                    value={eligibility.caseManagerEmail}
                    onChange={(e) => setElig('caseManagerEmail', e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                  />
                </div>
              </div>
            ...
            {!eligibility.hasSnap && !eligibility.hasTanf && !eligibility.hasReferral && (
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Other funding source (if any)
                </label>
                <input
                  type="text"
                  value={eligibility.otherFundingSource}
                  onChange={(e) => setElig('otherFundingSource', e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                  placeholder="e.g. Workforce Ready Grant, employer sponsor"
                />
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <YesNo
                value={eligibility.isAdult}
                onChange={(v) => setElig('isAdult', v)}
                name="isAdult"
              />
              {eligibility.isAdult === false && (
            ...
            <div>
              ...
              <YesNo
                value={eligibility.isIndianaResident}
                onChange={(v) => setElig('isIndianaResident', v)}
                name="isIndianaResident"
              />
              {eligibility.isIndianaResident === false && (
                <p className="text-red-600 text-xs mt-1 font-medium">
                  Indiana residency is required for this program.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                value={eligibility.educationLevel}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Do you have a diploma, GED, or HiSET?
              </label>
              <YesNo
                value={eligibility.hasDiplomaOrGed}
                onChange={(v) => setElig('hasDiplomaOrGed', v)}
                name="hasDiplomaOrGed"
              />
            </div>
            {eligibility.hasDiplomaOrGed === false && (
              <div className="pl-4 border-l-2 border-amber-300">
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Are you actively enrolled in a GED program?
                </label>
                <YesNo
                  value={eligibility.enrolledInGed}
                  onChange={(v) => setElig('enrolledInGed', v)}
                  name="enrolledInGed"
                />
                {eligibility.enrolledInGed === false && (
                  <p className="text-amber-700 text-xs mt-1">
                    A diploma, GED, or active GED enrollment is required. Contact us to discuss
                    options.
                  </p>
                )}
              </div>
          ...
          <div className="space-y-4">
            <h3 className="font-bold text-slate-900 text-base border-b border-slate-100 pb-2">
              4. Legal &amp; Employment Eligibility
            </h3>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
              ...
              <YesNo
                value={eligibility.workAuthorized}
                onChange={(v) => setElig('workAuthorized', v)}
                name="workAuthorized"
              />
              {eligibility.workAuthorized === false && (
                <p className="text-red-600 text-xs mt-1 font-medium">
                  Work authorization is required for this program.
                </p>
              )}
            </div>
            <div>
              ...
              <YesNo
                value={eligibility.activeWarrant}
                onChange={(v) => setElig('activeWarrant', v)}
                name="activeWarrant"
              />
              {eligibility.activeWarrant === true && (
                <p className="text-red-600 text-xs mt-1 font-medium">
                  Active warrants must be resolved before applying.
                </p>
              )}
            </div>
            <div>
              ...
              <YesNo
                value={eligibility.pendingCharges}
                onChange={(v) => setElig('pendingCharges', v)}
                name="pendingCharges"
              />
            </div>
            <div>
              ...
              <YesNo
                value={eligibility.onProbationParole}
                onChange={(v) => setElig('onProbationParole', v)}
                name="onProbationParole"
              />
            </div>
            {(eligibility.pendingCharges || eligibility.onProbationParole) && (
              <div className="pl-4 border-l-2 border-amber-300">
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Please briefly explain (optional but helpful for review)
                </label>
                <textarea
                  value={eligibility.legalNotes}
                  onChange={(e) => setElig('legalNotes', e.target.value)}
                  rows={2}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                  placeholder="Your application will be reviewed on a case-by-case basis."
                />
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Can you attend all required training hours each week?
              </label>
              <YesNo
                value={eligibility.canAttendSchedule}
                onChange={(v) => setElig('canAttendSchedule', v)}
                name="canAttendSchedule"
              />
              {eligibility.canAttendSchedule === false && (
                <p className="text-red-600 text-xs mt-1 font-medium">
                  Consistent attendance is required. Contact us if you have scheduling concerns.
                </p>
              )}
            </div>
            <div>
              ...
              <input
                type="text"
                value={eligibility.unavailableTimes}
                onChange={(e) => setElig('unavailableTimes', e.target.value)}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Can you stand, move, and safely participate in hands-on training activities?
              </label>
              <YesNo
                value={eligibility.canMeetPhysical}
                onChange={(v) => setElig('canMeetPhysical', v)}
                name="canMeetPhysical"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Are you willing to follow attendance, conduct, dress, and safety requirements?
              </label>
              <YesNo
                value={eligibility.willingToFollowRules}
                onChange={(v) => setElig('willingToFollowRules', v)}
                name="willingToFollowRules"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Are you willing to participate in job readiness and placement activities?
              </label>
              <YesNo
                value={eligibility.willingJobReadiness}
                onChange={(v) => setElig('willingJobReadiness', v)}
          ...
          <div className="space-y-4">
            ...
            <p className="text-slate-500 text-xs">
              Disclosing support needs does not disqualify you. It helps us coordinate services that
              may be available through workforce partners.
            </p>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Do you currently have a reliable way to get to training?
              </label>
              <YesNo
                value={eligibility.hasTransportationPlan}
                onChange={(v) => setElig('hasTransportationPlan', v)}
                name="hasTransportationPlan"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Do you need help with childcare, work supplies, or other barriers that could affect
                attendance?
              </label>
              <YesNo
                value={eligibility.willingJobReadiness}
          ...
          <div className="space-y-3">
            ...
            <label className="flex gap-3 items-start cursor-pointer">
              <input
                type="checkbox"
                checked={eligibility.agreesAttendance}
                onChange={(e) => setElig('agreesAttendance', e.target.checked)}
                className="mt-0.5 w-4 h-4 rounded border-slate-300 text-brand-red-600 flex-shrink-0"
              />
              <span className="text-sm text-slate-700">
                I understand that this program requires consistent attendance, participation, and
                completion of all training and employment readiness requirements.
              </span>
            </label>
            <label className="flex gap-3 items-start cursor-pointer">
              <input
                type="checkbox"
                checked={eligibility.agreesVerification}
                onChange={(e) => setElig('agreesVerification', e.target.checked)}
                className="mt-0.5 w-4 h-4 rounded border-slate-300 text-brand-red-600 flex-shrink-0"
              />
              <span className="text-sm text-slate-700">
                I understand that all information provided will be verified and that submitting this
