...
async function resolveEnrollment(supabase: Awaited<ReturnType<typeof createClient>>, userId: string, programSlug: string) {
...
async function _GET(request: NextRequest) {
  try {
    ...
    const enrollment = await resolveEnrollment(supabase, user.id, programSlug);
    if (!enrollment) return NextResponse.json({ error: 'No enrollment found for this program' }, { status: 403 });

    const [{ data: documentTypes, error: typesError }, { data: uploadedDocuments, error: docsError }] = await Promise.all([
      supabase.from('apprentice_document_types').select('*').eq('program_slug', programSlug).order('display_order', { ascending: true }),
      supabase.from('documents')
        .select('id, document_type, file_name, file_url, file_size_bytes, mime_type, status, verification_status, created_at, metadata')
        .eq('user_id', user.id)
    ...
    return NextResponse.json({ enrollmentId: enrollment.id, documentTypes: documentTypes || [], uploadedDocuments: uploadedDocuments || [] });
  } catch (error) {
...
async function _POST(request: NextRequest) {
  try {
    ...
    if (!(file instanceof File) || !documentTypeId || !programSlug) {
      return NextResponse.json({ error: 'File, document type, and program are required' }, { status: 400 });
    }
    ...
    const { data: docType, error: docTypeError } = await supabase
      ...
      .eq('program_slug', programSlug)
      .maybeSingle();
    if (docTypeError || !docType) return NextResponse.json({ error: 'Invalid document type for this program' }, { status: 400 });

    const maxBytes = Number(docType.max_file_size_mb || 10) * 1024 * 1024;
    if (file.size <= 0 || file.size > maxBytes) {
      return NextResponse.json({ error: `File too large. Maximum size is ${docType.max_file_size_mb || 10}MB` }, { status: 400 });
    }

    const ext = file.name.split('.').pop()?.toLowerCase() || '';
    const acceptedFormats = Array.isArray(docType.accepted_formats)
      ? docType.accepted_formats.map((value: unknown) => String(value).toLowerCase().replace(/^\./, ''))
      : [];
    if (!acceptedFormats.includes(ext)) return NextResponse.json({ error: `Invalid file type. Accepted: ${acceptedFormats.join(', ')}` }, { status: 400 });
    ...
    const immutableExisting = (existingDocs || []).filter((doc) =>
      ['approved', 'accepted', 'verified'].includes(String(doc.verification_status || doc.status || '').toLowerCase()),
    );
    if (immutableExisting.length) {
      return NextResponse.json({ error: 'An approved document is already on file. Approved evidence cannot be replaced or deleted; contact program staff if a new version is required.' }, { status: 409 });
    }
    ...
    const { data: docRecord, error: recordError } = await supabase.from('documents').insert({
      user_id: user.id,
    ...
    if (existingDocs?.length) {
      ...
      if (oldIds.length) {
        const { error: deleteRecordError } = await supabase.from('documents').delete().in('id', oldIds).eq('user_id', user.id);
        if (!deleteRecordError && oldPaths.length) await supabase.storage.from('documents').remove(oldPaths);
      }
    }
    ...
    await auditLog({
      ...
      metadata: { enrollment_id: enrollment.id, program_slug: programSlug, document_type: docType.document_type, storage_path: storagePath },
    });

    try {
      const [{ data: studentProfile }, { data: admins }] = await Promise.all([
        supabase.from('profiles').select('full_name,email').eq('id', user.id).maybeSingle(),
        supabase.from('profiles').select('email').in('role', ['admin', 'super_admin']),
      ]);
      if (admins?.length) {
        ...
        await Promise.all(admins.filter((admin) => admin.email).map((admin) =>
          emailService.sendDocumentUploadNotification(admin.email!, studentName, docType.name || docType.document_type, programSlug).catch(() => undefined),
        ));
      }
    } catch (notifyError) {
...
async function _DELETE(request: NextRequest) {
  try {
    ...
    const { data: doc } = await supabase.from('documents').select('*').eq('id', docId).eq('user_id', user.id).maybeSingle();
    if (!doc) return NextResponse.json({ error: 'Document not found' }, { status: 404 });
    if (['approved', 'accepted', 'verified'].includes(String(doc.verification_status || doc.status || '').toLowerCase())) {
    ...
    if (storagePath) {
      ...
      const { error: storageError } = await supabase.storage.from('documents').remove([pathInBucket]);
      if (storageError) return NextResponse.json({ error: 'Failed to delete stored file' }, { status: 500 });
    }
    ...
    await auditLog({ actorId: user.id, actorRole: 'apprentice', action: AuditAction.DOCUMENT_DELETED, entity: AuditEntity.DOCUMENT, entityId: docId, metadata: { document_type: doc.document_type, program_slug: doc.metadata?.program_slug, enrollment_id: doc.metadata?.enrollment_id, reason: 'user_initiated' } });
