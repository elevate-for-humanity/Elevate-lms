...
function DegradedBanner({ data }: { data: AdminDashboardData }) {
  ...
  return (
    <div
      className={`mb-6 flex items-start gap-3 rounded-xl border px-5 py-4 text-sm ${dashboardUnavailable ? 'border-rose-200 bg-rose-50 text-rose-900' : 'border-amber-200 bg-amber-50 text-amber-900'}`}
    >
      ...
      <div>
        ...
        <p className="mt-1 text-xs opacity-80">
          Open System Health for the current dependency status before acting on missing data.
        </p>
      </div>
    </div>
  );
...
function OperationalShortcuts() {
  const shortcuts = [
    {
      title: 'Applications',
      description: 'Review submitted applications and resolve intake decisions.',
      href: '/applications',
    },
    {
      title: 'Students',
      description: 'Open learner records, enrollments, documents, and progress.',
      href: '/students',
    },
    {
      title: 'Programs',
      description: 'Manage published programs, courses, credentials, and curriculum.',
      href: '/programs',
    },
    {
      title: 'Funding & Payments',
      description: 'Review funding, vouchers, payout holds, and releases.',
      href: '/funding',
    },
    {
      title: 'Documents',
      description: 'Review uploaded learner and partner documents.',
      href: '/documents/review',
    },
    {
      title: 'Program Holders',
      description: 'Manage program holders, onboarding, agreements, and students.',
    ...
    },
    {
      ...
      description: 'Open every role dashboard from the canonical portal registry.',
      href: '/portals',
    },
    {
      title: 'Reports',
    ...
    {
      title: 'Compliance',
      description: 'Resolve alerts, missing records, and required approvals.',
      href: '/compliance',
    },
    {
      title: 'Website',
      description: 'Edit, preview, and publish the public website.',
      href: '/website-editor',
    },
    {
      title: 'System Health',
      description: 'Check integrations, deployments, jobs, and platform health.',
      href: '/system-health',
    },
    {
  ...
  return (
    <section className="mb-8">
      <div className="mb-4">
        <h2 className="text-xl font-black text-slate-950">Run Elevate</h2>
        <p className="mt-1 text-sm font-medium text-slate-700">
          Open every core administrative workspace directly.
        </p>
      </div>
...
function ReviewQueues({ data }: { data: AdminDashboardData }) {
  ...
  const queues = [
    ...
    },
    {
      ...
      count: data.pendingWioaDocs ?? 0,
      href: '/wioa/documents',
    },
    {
      label: 'Lab submissions awaiting sign-off',
      count: data.pendingSubmissions?.length ?? 0,
      href: '/submissions',
    },
    {
      label: 'Program holders awaiting approval',
      count: counts?.pendingProgramHolders ?? 0,
      href: '/program-holders',
    },
    {
      label: 'Program holder documents pending',
      count: counts?.pendingDocuments ?? 0,
      ...
      <div className="divide-y divide-slate-100">
        {queues.map((queue) => (
          <Link
            key={queue.label}
            href={queue.href}
            className="flex min-h-12 items-center justify-between gap-4 px-5 py-4 hover:bg-slate-50"
          >
            <span className="text-sm font-semibold text-slate-800">{queue.label}</span>
            <span className="shrink-0 text-lg font-black tabular-nums text-slate-950">
              {queue.count}
            </span>
          </Link>
        ))}
...
export function AdminDashboardContent({
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] min-w-0 px-3 pb-16 pt-4 sm:px-5 lg:px-6">
      <section className="mb-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="p-5 sm:p-8">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-blue-100 px-3 py-1.5 text-xs font-black text-blue-900">
              Live Supabase data
            </span>
          </div>
          ...
          <p className="mt-4 max-w-3xl text-base font-medium leading-7 text-slate-700">
            This dashboard is an operational view of current application, enrollment, program,
      ...
      {canAccessDevStudio && (
        <section className="mb-8 overflow-hidden rounded-3xl border border-blue-200 bg-gradient-to-br from-white via-blue-50/40 to-indigo-50/70 shadow-sm">
          <div className="p-5 sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-3xl">
                <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white px-3 py-1.5 text-xs font-black uppercase tracking-wider text-blue-800">
                  <Sparkles className="h-4 w-4" /> Admin AI
                </div>
                <h2 className="mt-3 text-xl font-black tracking-tight text-slate-950 sm:text-2xl">
                  Work with Elevate’s operations assistant
                </h2>
                <p className="mt-2 text-sm font-medium leading-6 text-slate-700">
                  Ask it to inspect records, run approved workflows, build courses, diagnose
                  deployments, or open the affected page beside the conversation.
                </p>
              </div>
              <Link
                href="/studio"
                className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800"
              >
          ...
          {(data.recentApplications?.length ?? 0) > 0 && (
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                ...
                <Link href="/applications" className="text-xs font-semibold text-blue-700">
                  View all
                </Link>
              </div>
        ...
        {(data.topPrograms?.length ?? 0) > 0 && (
          <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
              ...
              <Link href="/programs" className="shrink-0 text-xs font-semibold text-blue-700">
                All programs
              </Link>
            </div>
            <div className="divide-y divide-slate-100">
              {data.topPrograms.slice(0, 6).map((program) => (
                <Link
                  ...
                  className="flex min-h-12 items-center justify-between gap-4 px-5 py-3 hover:bg-slate-50"
                >
      ...
      <div className="mt-8 grid min-w-0 grid-cols-1 gap-6 xl:grid-cols-2">
      ...
      <section className="mt-8" aria-labelledby="portal-access-title">
        <div className="mb-4">
          <h2 id="portal-access-title" className="text-xl font-black text-slate-950">
            Portal apps and secure portal access
          </h2>
          <p className="mt-1 text-sm font-medium text-slate-700">
            Select a real account before previewing its dashboard, or install the appropriate PWA
            shell on this device.
          </p>
        </div>
        <PwaInstallCenter />
        <HostShopPortalLauncher />
      </section>
    </div>
  );
}
