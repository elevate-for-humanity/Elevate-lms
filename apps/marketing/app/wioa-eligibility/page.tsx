...
export default function WIOAEligibilityPage() {
  return (
    <div className="min-h-screen bg-white">
            ...
            {[
              ...
              <div key={item.step} className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm">
                ...
                <div className="p-4">
                  ...
                  {item.external ? (
                    <a href={item.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-brand-blue-600 text-white font-semibold px-4 py-2 rounded-lg text-sm hover:bg-brand-blue-700 transition-colors">
                      {item.cta} <ArrowRight className="w-4 h-4" />
                    </a>
                  ) : (
                    <Link href={item.href} className="inline-flex items-center gap-2 bg-brand-blue-600 text-white font-semibold px-4 py-2 rounded-lg text-sm hover:bg-brand-blue-700 transition-colors">
                      {item.cta} <ArrowRight className="w-4 h-4" />
                    </Link>
                  )}
                </div>
      ...
      <section className="py-10 sm:py-14">
        <div className="max-w-5xl mx-auto px-4">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 text-center mb-8">What WIOA Covers</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { title: 'Tuition', image: '/images/pages/writing-center.jpg' },
              { title: 'Books & Supplies', image: '/images/pages/writing-center-page-1.jpg' },
              { title: 'Certification Exams', image: '/images/pages/certificates-page-1.webp' },
              { title: 'Support Services', image: '/images/pages/about-supportive-services.webp' },
            ].map((item) => (
              <div key={item.title} className="rounded-xl overflow-hidden border border-slate-100 bg-white">
                <div className="relative h-24 sm:h-32 overflow-hidden">
                  <Image src={item.image} alt={item.title} fill sizes="100vw" className="object-cover" />
                </div>
                <div className="p-3 text-center"><h3 className="font-semibold text-slate-900 text-sm">{item.title}</h3></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-10 sm:py-14">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 text-center mb-2">Who Qualifies</h2>
          <p className="text-slate-500 text-sm text-center mb-8">WorkOne determines eligibility at your appointment</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              'Adults 18+ authorized to work in the US', 'Indiana residents', 'Unemployed or underemployed',
              'Receiving public assistance (SNAP, TANF, SSI)', 'Veterans and eligible spouses',
              'Individuals with disabilities', 'Justice-involved individuals', 'Low-income households',
            ].map((item) => (
          ...
          <div className="mt-6 flex flex-wrap gap-3 justify-center">
            <Link href="/wioa-eligibility/low-income" className="px-4 py-2 bg-brand-blue-50 text-brand-blue-700 rounded-lg text-sm font-semibold hover:bg-brand-blue-100 transition-colors">Low-Income Guidelines →</Link>
            <Link href="/wioa-eligibility/public-assistance" className="px-4 py-2 bg-brand-blue-50 text-brand-blue-700 rounded-lg text-sm font-semibold hover:bg-brand-blue-100 transition-colors">Public Assistance Recipients →</Link>
            <Link href="/wioa-eligibility/veterans" className="px-4 py-2 bg-brand-blue-50 text-brand-blue-700 rounded-lg text-sm font-semibold hover:bg-brand-blue-100 transition-colors">Veterans Eligibility →</Link>
          </div>
        </div>
      </section>
      <section className="py-10 sm:py-14">
        <div className="max-w-5xl mx-auto px-4">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 text-center mb-8">Other Funding Options</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {[
              ...
              <Link key={item.title} href={item.href} className="block rounded-2xl overflow-hidden border border-slate-200 bg-white hover:shadow-lg transition-shadow group">
                <div className="relative h-[140px] sm:h-[160px] overflow-hidden"><Image src={item.image} alt={item.title} fill sizes="100vw" className="object-cover" /></div>
                <div className="p-4"><h3 className="font-bold text-slate-900 mb-1">{item.title}</h3><p className="text-slate-600 text-sm mb-2">{item.desc}</p><span className="text-brand-blue-600 font-semibold text-sm group-hover:underline">Learn More →</span></div>
              </Link>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <details key={faq.q} className="bg-white rounded-lg border border-slate-200 group">
                <summary className="p-4 cursor-pointer font-semibold text-slate-900 text-sm flex justify-between items-center">{faq.q}<svg className="w-4 h-4 text-slate-400 group-open:rotate-180 transition-transform flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></summary>
      ...
      <section className="py-10 sm:py-14">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-slate-900 mb-6 text-sm sm:text-base">Register, schedule your WorkOne appointment, and start training in weeks.</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="https://www.indianacareerconnect.com" target="_blank" rel="noopener noreferrer" className="bg-white text-brand-blue-600 font-bold px-6 py-3 rounded-lg text-base hover:bg-brand-blue-50 transition-colors">Register at Indiana Career Connect</a>
            <Link href="/start-trial" className="border-2 border-white text-white font-bold px-6 py-3 rounded-lg text-base hover:bg-white/10 transition-colors">Apply for Training</Link>
          </div>
        </div>
      </section>
    </div>
