...
export default async function PartnerBoardPage() {
  ...
  const summaryCards: PortalCard[] = [
    ...
    { href: '/host-shop/dashboard/hours/pending', title: 'Hours pending review', value: board.pendingHoursCount, detail: 'Review OJT hours that require Host Shop verification.', image: '/images/pages/barber-shop-interior.webp' },
    { href: '/host-shop/dashboard/competencies', title: 'Competency sign-offs', value: `${board.tradeInfo.hours.toLocaleString()}h`, detail: 'Review required skills and complete supervisor verification.', image: '/images/pages/shop-hero.webp' },
    { href: '/host-shop/dashboard/documents', title: 'Required documents', value: board.requiredDocumentCount ? `${board.acceptedDocumentCount}/${board.requiredDocumentCount}` : board.partner?.documents_verified ? 'Complete' : 'Review', detail: 'Maintain the MOU, licenses, insurance, and required host-site files.', image: '/images/pages/comp-home-highlight-success.webp' },
  ];
  const toolCards: PortalCard[] = [
    { href: '/host-shop/dashboard/orientation', title: 'Required orientation', detail: 'Learn recruiting, shop economics, payroll, geofencing, OJL approval, competencies, transfer-credit rules, and compliance.', image: '/images/pages/training-classroom.webp' },
    ...
    { href: '/host-shop/dashboard/reports', title: 'Reporting center', detail: 'Review documented hours, attendance, and progress records.', image: '/images/heroes/lms-analytics.webp' },
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
    ...
    {!board.partner?.onboarding_completed ? <section className="mt-6 rounded-2xl border-2 border-amber-300 bg-amber-50 p-5"><h2 className="font-black text-amber-950">Required Host Shop orientation</h2><p className="mt-1 text-sm font-medium text-amber-950">Complete the operating orientation before independently managing apprentice hours, competencies, payroll documentation, and compliance.</p><Link href="/host-shop/dashboard/orientation" className="mt-4 inline-flex rounded-lg bg-amber-800 px-4 py-2 text-sm font-black text-white">Start required orientation</Link></section> : null}
