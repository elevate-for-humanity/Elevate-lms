...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description,
    keywords: [publicName, `${publicName} ${shop.city}`, `barber shop ${shop.city} Indiana`, `barber apprenticeship ${shop.city}`, 'Indiana barber apprenticeship host shop', 'Elevate for Humanity host shop'],
    alternates: { canonical },
    openGraph: {
      title: `${publicName} | Apprenticeship Host Shop`, description, url: canonical, type: 'website',
...
export default async function HostShopProfilePage({ params }: PageProps) {
  ...
  const canonical = `${SITE_URL}/host-shops/${shop.slug}`;
  const jsonLd = {
    '@context': 'https://schema.org', '@type': shop.businessType ?? 'BarberShop', name: publicName,
    legalName: shop.name, url: canonical, description: shop.marketingBlurb ?? shop.note, telephone: shop.phone,
    ...
    sameAs: sameAs.length ? sameAs : undefined, areaServed: { '@type': 'City', name: `${shop.city}, Indiana` },
  };
    ...
    <main className="bg-white text-slate-950">
      ...
      <section className="border-y border-slate-200 bg-slate-50 px-4 py-12 sm:px-6">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <p className="mx-auto mt-4 max-w-3xl leading-7 text-slate-700">Start with the Elevate barber apprenticeship application. Host-shop placement, available positions, funding, and transfer-hour documentation are reviewed as part of enrollment.</p>
          <div className="mt-7 flex flex-wrap justify-center gap-3">
            <Link href="/programs/barber-apprenticeship/apply" className="rounded-xl bg-brand-red-600 px-6 py-3 font-extrabold text-white hover:bg-brand-red-700">Apply for Barber Apprenticeship</Link>
            <Link href="/programs/barber-apprenticeship#host-shops" className="rounded-xl border border-slate-300 bg-white px-6 py-3 font-extrabold text-slate-900 hover:bg-slate-50">View all host shops</Link>
          </div>
        </div>
      </section>
    </main>
