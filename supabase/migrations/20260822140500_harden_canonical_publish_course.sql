-- Harden the canonical publish RPC. Application publication already crosses the
-- Course Builder procurement service; this function is the database backstop.
...
begin
  if coalesce(auth.role(), '') <> 'service_role' then
    ...
    if v_role is null or v_role not in ('admin','super_admin','program_holder') then
      raise exception 'Publishing role required' using errcode='42501';
    end if;
  end if;

  select * into v_course from public.courses where id = p_course_id for update;
  if not found then
    raise exception 'PUBLISH_BLOCKED: course % not found', p_course_id;
  end if;

  if coalesce(v_course.review_status, '') <> 'approved'
     or v_course.reviewed_by is null
     or v_course.reviewed_at is null then
    raise exception 'PUBLISH_BLOCKED: course has not completed attributable human review/approval';
  end if;

  select count(*)::integer,
         count(*) filter (where approved = true)::integer
  ...
  if v_required_count = 0 then
    raise exception 'PUBLISH_BLOCKED: course has no required lessons';
  end if;
  if v_approved_count <> v_required_count then
    raise exception 'PUBLISH_BLOCKED: required lesson approval incomplete (%/%)', v_approved_count, v_required_count;
  end if;
  ...
  set status = 'published',
      ...
      generation_status = case when generation_status in ('generated','completed','verification_ready','certificate_ready') then 'published' else generation_status end,
      updated_at = now()
  where course_id = p_course_id;
  ...
  set status = 'published',
      ...
      total_lessons = v_required_count,
      updated_at = now()
