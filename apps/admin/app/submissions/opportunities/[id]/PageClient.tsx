          ...
          <section className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  value={opp.opportunity_type}
                  onChange={field('opportunity_type')}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500"
                >
                  {OPP_TYPES.map((t) => (
              ...
              <div>
                ...
                <input
                  value={opp.submission_method ?? ''}
                  onChange={field('submission_method')}
                  placeholder="e.g. Grants.gov, email, portal"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500"
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    value={opp.issuer_name ?? ''}
                    onChange={field('issuer_name')}
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-blue-500"
                  />
                </div>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="url"
                    value={opp.portal_url ?? ''}
                    onChange={field('portal_url')}
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500"
                  />
                </div>
                <div>
          ...
          <section className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  rows={4}
                  value={opp.scope_summary ?? ''}
                  onChange={field('scope_summary')}
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  value={opp.eligibility_text ?? ''}
                  onChange={field('eligibility_text')}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500 resize-none"
                />
              </div>
              <div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Map each submission requirement to a review classification.
                </p>
              </div>
              <button
              ...
              <div className="mb-4 p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      value={newReq.requirement_category}
                      onChange={(e) =>
                        setNewReq((p) => ({ ...p, requirement_category: e.target.value }))
                      }
                      placeholder="e.g. Narrative, Budget, Attachment"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewReq((p) => ({ ...p, requirement_name: e.target.value }))
                      }
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      value={newReq.review_class}
                      onChange={(e) => setNewReq((p) => ({ ...p, review_class: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue-500"
                    >
                      {REVIEW_CLASSES.map((c) => (
                  ...
                  <div className="flex items-center gap-2 pt-5">
                    <input
                      ...
                      className="rounded"
                    />
                    <label htmlFor="req-required" className="text-sm text-slate-700">
                      Required
                    </label>
                  </div>
                </div>
            ...
            {reqs.length === 0 ? (
              ...
              <div className="space-y-2">
                {reqs.map((req) => (
                  <div
                    key={req.id}
                    className="flex items-start gap-3 p-3 border border-slate-100 rounded-lg hover:bg-slate-50 transition"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-0.5">
                        ...
                        {req.review_class && (
                          <span
                            className={`text-xs px-1.5 py-0.5 rounded font-medium ${REVIEW_CLASS_COLORS[req.review_class] ?? 'bg-slate-100 text-slate-600'}`}
                          >
