...
export default async function WorkforceDashboardPage() {
  ...
  // External workforce partners must query through their authenticated client
  // so database RLS remains in force. Only internal privileged roles may use
  // the service-role client for platform-wide oversight.
  ...
  const recentUserIds = [
    ...new Set((rawRecent ?? []).map((enrollment: any) => enrollment.user_id).filter(Boolean)),
  ];
  ...
  const profileMap = Object.fromEntries(
    (recentProfiles ?? []).map((profile: any) => [profile.id, profile]),
  );
  ...
  const quickActions = [
    ...
    { label: 'Reports', href: '/workforce/reports', icon: TrendingUp },
  ];
