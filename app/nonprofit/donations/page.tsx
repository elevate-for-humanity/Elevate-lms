...
export default async function DonationsPage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Hero */}
      <section className="py-20 px-4 bg-brand-red-50 text-center">
        <div className="max-w-4xl mx-auto">
          <Heart className="w-16 h-16 text-brand-red-600 mx-auto mb-6" />
          ...
          <Link
            ...
            target="_blank"
            className="inline-block bg-brand-red-600 text-white px-12 py-4 rounded-lg text-lg font-bold hover:bg-brand-red-700 transition-colors shadow-lg"
          >
            Donate Now
          </Link>
        </div>
          ...
          <div className="grid grid-cols-3 gap-8">
            {displayStats.map((stat: any, index: number) => (
              <div key={index} className="text-center">
                <div className="text-3xl md:text-4xl font-bold text-brand-red-600 mb-1">
                  {stat.value}
                </div>
              ...
              return (
                <div key={index} className="bg-white rounded-xl p-6 text-center shadow-sm hover:shadow-md transition">
                  <div className="w-16 h-16 bg-brand-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <IconComponent className="w-8 h-8 text-brand-red-600" />
                  </div>
                  <h3 className="text-2xl font-bold text-black mb-2">${tier.amount}</h3>
                  <p className="text-brand-red-600 font-medium mb-2">{tier.title}</p>
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-brand-red-50 rounded-xl p-8">
            ...
            <div className="space-y-4">
              {recentDonors.map((donor: any, index: number) => (
                <div key={index} className="bg-white rounded-lg p-4 flex items-center justify-between">
                  ...
                  <span className="text-brand-red-600 font-semibold">${donor.amount}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* CTA */}
      <section className="py-16 px-4 bg-brand-red-600 text-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <Link
            href="https://donate.stripe.com/5kA5kn7EsfrD08w4gg"
            target="_blank"
            className="inline-block bg-white text-brand-red-600 px-12 py-4 rounded-lg text-lg font-bold hover:bg-gray-100 transition"
          >
            Donate Now
          </Link>
        </div>
