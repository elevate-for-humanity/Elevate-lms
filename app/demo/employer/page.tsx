...
export default function EmployerDemo() {
  ...
  const statusColor = (s: string) => s === 'Interview' ? 'bg-green-100 text-green-700' : s === 'Offer' ? 'bg-brand-red-100 text-brand-red-700' : s === 'Review' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700';
...
<div className="bg-blue-600 text-white text-center py-3 px-2 text-xs"><Info className="w-3 h-3 inline mr-1" />Live Demo - Employer Portal</div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t z-40 sm:hidden">
        <div className="flex justify-around py-2">
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)} className={`flex flex-col items-center px-4 py-3 ${tab === t.id ? 'text-brand-red-600' : 'text-gray-500'}`}>
      ...
      <div className="hidden sm:flex justify-center gap-1 py-2 bg-white border-b">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${tab === t.id ? 'bg-brand-red-100 text-brand-red-700' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      <main className="p-3 pb-20 sm:pb-6 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            { label: 'Apprentices', value: 3, color: 'text-brand-red-600' },
          ].map((s, i) => (
        ...
        {tab === 'roles' && (
          <div className="space-y-3">
            <div className="flex justify-end">
              <button className="bg-brand-red-600 text-white px-3 py-3 rounded-lg text-sm font-medium flex items-center gap-1"><Plus className="w-4 h-4" />Post Role</button>
            </div>
            {roles.map(r => (
        ...
        {tab === 'apprentice' && (
          <div className="space-y-4">
            <div className="bg-brand-red-50 rounded-xl p-4 border border-brand-red-200">
              <div className="flex items-center gap-2 mb-2">
                <Award className="w-5 h-5 text-brand-red-600" />
                <span className="font-bold text-brand-red-900">Registered Apprenticeship</span>
              </div>
              <p className="text-sm text-brand-red-700">Indiana Dept of Labor • Healthcare Program</p>
            </div>
              ...
              {apprentices.map(a => (
                <div key={a.id} className="flex items-center gap-3 p-3 border-b last:border-0">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-brand-red-600">{a.level}</div>
                  </div>
                  <div className="text-right">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {['Screen', 'Review', 'Interview', 'Offer'].map(s => (
                        <button key={s} onClick={() => updateStatus(c.id, s)} className={`py-2 rounded-lg text-sm font-medium ${c.status === s ? 'bg-brand-red-600 text-white' : 'bg-gray-100 text-gray-700'}`}>{s}</button>
                      ))}
                    </div>
                  </div>
                  <button onClick={() => setModal(null)} className="w-full bg-brand-red-600 text-white py-3 rounded-lg font-medium">Schedule Interview</button>
                </>
              );
            })()}
          </div>
