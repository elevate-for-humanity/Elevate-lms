...
function parseSubdomain(value: unknown): { valid: boolean; value: string | null } {
  ...
  // Draft websites are allowed to remain unnamed on the Elevate subdomain.
...
async function snapshotRevision(
  ...
  // Revision history is protective, not a reason to block a customer save if a
  // migration is still rolling through an environment.
...
export async function PATCH(
  ...
  const existing = site.site_config && typeof site.site_config === 'object'
    ...
    : base;
  const configPatch = body.siteConfig && typeof body.siteConfig === 'object'
    ? body.siteConfig as Partial<TenantSiteConfig>
    : {};
  ...
  if (body.subdomain !== undefined) {
    ...
    if (!parsed.valid) {
      return NextResponse.json(
        { error: 'Subdomain can contain only letters, numbers, and hyphens' },
        { status: 400 },
      );
    }
  ...
  if (body.publish === true) {
    ...
    if (!target) {
      return NextResponse.json(
        { error: 'Choose a valid subdomain before publishing' },
        { status: 400 },
      );
    }
  ...
  const reason = body.publish === true
    ? 'publish'
    : body.publish === false
      ? 'unpublish'
      : 'save';
