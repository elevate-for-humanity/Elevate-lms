...
export async function regenerateLessonAction(formData: FormData) {
  ...
  // Mark regenerating
  await db.from('course_lessons').update({ generation_status: 'generating' }).eq('id', lessonId);
  ...
  if (!res.ok) {
    await db.from('course_lessons').update({ generation_status: 'generated' }).eq('id', lessonId);
