...
function OperationalShortcuts() {
  const shortcuts = [
    ...
    {
      title: 'Operations',
      description: 'Review system health, workflows, alerts, and release conditions.',
      href: '/operations',
    },
  ] as const;
  ...
  return (
    <section className="mb-8">
      <div className="mb-4">
        <h2 className="text-xl font-black text-slate-950">Operational workspaces</h2>
        <p className="mt-1 text-sm font-medium text-slate-700">
          Use these when you want a manual record view. The Admin AI remains the primary control surface.
        </p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {shortcuts.map((item) => (
          ...
          >
            ...
            <p className="mt-2 text-sm font-medium leading-5 text-slate-700">{item.description}</p>
          </Link>
        ))}
      </div>
    </section>
      ...
      <div className="divide-y divide-slate-100">
        {queues.map((queue) => (
          ...
          >
            ...
            <span className="shrink-0 text-lg font-black tabular-nums text-slate-950">{queue.count}</span>
          </Link>
        ))}
      </div>
    </div>
      ...
      <section className="mb-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="p-6 sm:p-8">
          ...
          <p className="mt-4 max-w-3xl text-base font-medium leading-7 text-slate-700">
            This dashboard is an operational view of current application, enrollment, program, payment, compliance, and system-health data.
          </p>
        </div>
      </section>
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-3xl">
                ...
                <p className="mt-3 text-sm font-medium leading-6 text-slate-700 sm:text-base">
                  Build courses, review applications, inspect records, publish website changes, run workflows, verify compliance, diagnose deployments, and use advanced platform tools from one governed conversation.
                </p>
              </div>
              <Link
                href="/studio"
        ...
        {(data.topPrograms?.length ?? 0) > 0 && (
          <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              ...
              <Link href="/programs" className="text-xs font-semibold text-blue-700">
                All programs
              </Link>
            </div>
            <div className="divide-y divide-slate-100">
              {data.topPrograms.slice(0, 6).map((program) => (
                ...
                >
                  <p className="min-w-0 truncate text-sm font-semibold text-slate-950">{program.title}</p>
                  <span className="shrink-0 text-xs text-slate-600">{program.learners} enrolled</span>
                </Link>
              ))}
            </div>
          </div>
