      ...
      <div className="max-w-4xl mx-auto px-6 py-10 print:py-4 print:px-0">
        <div className="flex items-start justify-between mb-8 print:mb-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 border rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 print:hidden"
          >
        ...
        <div className="grid grid-cols-2 gap-6 border rounded-xl p-6 mb-8 bg-slate-50 print:mb-4">
          ...
          <div className="space-y-3">
            <div>
              <p className="text-xs text-slate-400 uppercase tracking-wide">Enrolled</p>
            ...
            <div>
              <p className="text-xs text-slate-400 uppercase tracking-wide">Completion Date</p>
              <p className="text-sm text-slate-700">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8 print:mb-4">
          {[
            ['Theory Hours', `${totalTheoryHours}h`],
            ['OJT Hours', `${totalOjtHours}h`],
            ['Lessons Complete', `${completedLessons}/${totalLessons}`],
            ['Checkpoints Passed', `${checkpointsPassed}/${totalCheckpoints}`],
          ].map(([label, value]) => (
            ...
            <div key={domain.code} className="border rounded-xl overflow-hidden">
              <div className="bg-slate-900 text-white px-5 py-3 flex items-center justify-between">
                ...
                <div className="text-xs text-slate-400">
                  {domain.completedCount}/{domain.totalCount} · {domain.theoryHours}h theory ·{' '}
                  {domain.ojtHours}h OJT
                </div>
              </div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-slate-50">
                    <th className="text-left px-4 py-2 text-xs font-semibold text-slate-500 w-16">
                      Code
                    </th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-slate-500">
                      Competency
                    </th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-slate-500 w-24">
                      Type
                    </th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-slate-500 w-28">
                      Status
                    </th>
                    <th className="text-right px-4 py-2 text-xs font-semibold text-slate-500 w-16">
                      Score
                    </th>
                  </tr>
                </thead>
                  ...
                  {domain.competencies.map((comp) => (
                    <tr key={comp.code} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2.5">
                        <span
                          className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusColor[comp.status]}`}
                        >
