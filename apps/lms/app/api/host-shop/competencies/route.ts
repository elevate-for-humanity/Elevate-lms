...
async function getAuthContext() {
  ...
  return { user, db, profile, profileRole, membershipRole, membershipPartnerId: membership?.partner_id || null, isPlatformAdmin };
}
...
async function getAuthorizedShopIds(db: any, userId: string, isPlatformAdmin: boolean, membershipPartnerId: string | null) {
  ...
  const { data: partner } = await db.from('partners').select('id, status, approval_status, is_active').eq('id', partnerId).maybeSingle();
  if (!partner || partner.status !== 'active' || partner.approval_status !== 'approved' || partner.is_active === false) return [];
...
export async function GET() {
  ...
  if (shopIds.length === 0) return NextResponse.json({ error: 'No active Host Shop assignment found' }, { status: 403 });
  ...
  const [{ data: profiles }, { data: enrollments }] = await Promise.all([
    ...
    db.from('program_enrollments').select('id, user_id, student_id, program_id, program_slug, status, created_at')
  ...
  for (const enrollment of enrollments || []) {
    ...
    const placement = (placements || []).find((item: any) => item.student_id === enrollmentStudentId && (!item.program_slug || !enrollment.program_slug || item.program_slug === enrollment.program_slug));
    if (placement && !enrollmentByStudent.has(enrollmentStudentId)) enrollmentByStudent.set(enrollmentStudentId, enrollment);
  }

  const enrollmentIds = Array.from(enrollmentByStudent.values()).map((row: any) => row.id);
  const { data: records } = enrollmentIds.length
    ? await db.from('apprentice_competency_records')
  ...
  const apprentices = (placements || []).map((placement: any) => {
    ...
    if (!enrollment || !contract) return null;
    ...
    return {
      ...
      completedCompetencies: competencyRecords.filter((row: any) => row.completed).length,
      competencyRecords,
    };
  }).filter(Boolean);
...
export async function PATCH(req: NextRequest) {
  ...
  const { data: enrollment } = await db.from('program_enrollments').select('id, user_id, student_id, program_slug').eq('id', enrollmentId).maybeSingle();
  if (!enrollment) return NextResponse.json({ error: 'Enrollment not found' }, { status: 404 });
  const contract = getRegisteredProgramStandard(enrollment.program_slug);
  if (!contract || !contract.standard.competencies.some((item) => item.id === competencyId)) {
    return NextResponse.json({ error: 'Competency is not part of the approved registered-program standard' }, { status: 400 });
  }
  ...
  const { data: placement } = await db.from('apprentice_placements').select('id, supervisor_user_id')
    .eq('student_id', studentId).eq('status', 'active').in('shop_id', shopIds).limit(1).maybeSingle();
