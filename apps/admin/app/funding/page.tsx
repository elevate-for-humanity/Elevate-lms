...
const WIOA_TYPE_STYLES: Record<string, string> = {
  'Dislocated Worker': 'bg-purple-100 text-purple-800', Adult: 'bg-blue-100 text-blue-800', Youth: 'bg-teal-100 text-teal-800',
};
...
export default async function FundingPage() {
  ...
  const [vouchersRes, approvedRes, pendingRes, expiringRes, enrollmentsRes] = await Promise.all([
    db.from('ita_vouchers').select(
      'id, voucher_id, participant_name, wioa_type, fund_stream, service_name, voucher_date, voucher_expire_date, total_voucher_amount, payments_to_date, status, is_final, remittance_email',
      { count: 'exact' },
  ...
  const missingNameIds = enrollments.filter((e: any) => !e.full_name).map((e: any) => e.user_id).filter(Boolean);
  ...
  const studentPayments = enrollments.map((e: any) => ({ ...e, display_name: e.full_name || profileMap[e.user_id] || e.email || 'Unknown' }));
    ...
    <div className="min-h-screen bg-white">
      <div className="bg-white border-b border-slate-200 px-6 py-5">
        <nav className="flex items-center gap-1.5 text-xs text-slate-500 mb-3">
          <Link href="/dashboard" className="hover:text-slate-700">Admin</Link><ChevronRight className="w-3 h-3" /><span className="text-slate-900 font-medium">Funding</span>
        </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            const Icon = s.icon;
            return <div key={s.label} className={`bg-white rounded-2xl border shadow-sm p-5 ${s.urgent ? 'border-rose-300 ring-1 ring-rose-200' : 'border-slate-200'}`}><div className={`w-9 h-9 rounded-xl ${s.bg} flex items-center justify-center mb-3`}><Icon className={`w-4 h-4 ${s.color}`} /></div><p className="text-2xl font-bold text-slate-900 tabular-nums">{s.value}</p><p className="text-xs text-slate-500 mt-1 font-medium">{s.label}</p></div>;
          })}
        </div>
        ...
        {total > 0 && <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          ].map((s) => <div key={s.label} className="bg-slate-50 rounded-xl border border-slate-200 p-4"><p className="text-xs text-slate-500 font-medium mb-1">{s.label}</p><p className={`text-xl font-black tabular-nums ${s.color}`}>${s.value.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p></div>)}
        </div>}

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {QUICK_LINKS.map((l) => <Link key={l.href} href={l.href} className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all flex items-center justify-between">{l.label}<ArrowRight className="w-3.5 h-3.5 text-slate-400" /></Link>)}
        </div>

        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between"><div><h2 className="font-semibold text-slate-900 text-sm">ITA Vouchers</h2><p className="text-xs text-slate-400 mt-0.5">WorkOne / EmployIndy WIOA training vouchers</p></div><span className="text-xs text-slate-400">{total} total</span></div>
          {!vouchers.length ? <div className="py-16 text-center"><DollarSign className="w-8 h-8 text-slate-300 mx-auto mb-3" /><p className="text-sm text-slate-500 font-medium">No ITA vouchers</p><p className="text-xs text-slate-400 mt-1">Vouchers appear here when WIOA participants are approved for training funding</p></div> : (
            <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-slate-100 bg-slate-50">{['Participant', 'Voucher ID', 'WIOA Type', 'Program', 'Dates', 'Amount', 'Paid', 'Status', ''].map((h) => <th key={h} className="text-left py-3 px-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 whitespace-nowrap">{h}</th>)}</tr></thead><tbody className="divide-y divide-slate-50">
              {vouchers.map((v) => {
                ...
                return <tr key={v.id} className="hover:bg-slate-50 transition-colors">
                  <td className="py-3.5 px-4"><p className="font-semibold text-slate-900 whitespace-nowrap">{v.participant_name}</p>{v.remittance_email && <a href={`mailto:${v.remittance_email}`} className="flex items-center gap-1 text-xs text-slate-400 hover:text-brand-blue-600 mt-0.5"><Mail className="w-3 h-3" />{v.remittance_email}</a>}</td>
                  <td className="py-3.5 px-4"><span className="font-mono text-xs font-semibold text-slate-700">{v.voucher_id}</span>{v.is_final && <span className="ml-1.5 text-[10px] font-bold text-rose-600 uppercase">Final</span>}</td>
                  <td className="py-3.5 px-4"><span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold ${WIOA_TYPE_STYLES[v.wioa_type || ''] ?? 'bg-slate-100 text-slate-600'}`}>{v.wioa_type ?? '—'}</span></td>
                  <td className="py-3.5 px-4 max-w-[200px]"><p className="text-xs text-slate-600 truncate">{v.service_name ?? '—'}</p><p className="text-[10px] text-slate-400 truncate">{v.fund_stream ?? ''}</p></td>
                  <td className="py-3.5 px-4 whitespace-nowrap"><p className="text-xs text-slate-600">{v.voucher_date ? new Date(v.voucher_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</p><p className={`text-[10px] mt-0.5 ${isExpiringSoon ? 'text-rose-600 font-semibold' : 'text-slate-400'}`}>Exp: {v.voucher_expire_date ? new Date(v.voucher_expire_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</p></td>
        ...
        <div className="sr-only" aria-hidden="true">Self-pay payments captured: ${(totalSelfPay / 100).toFixed(2)}</div>
      </div>
    </div>
  );
}
