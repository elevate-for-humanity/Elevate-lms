-- A completed render is persisted before canonical audiovisual QA runs. Do not
-- reclaim that durable candidate while the renderer still owns a live lease;
-- doing so would clear the lease and strand the job at pending_review without
-- quality evidence.
...
begin
  ...
  -- markCandidate has already stored video_url.
...
comment on function public.claim_video_jobs(integer, uuid, integer) is
  'Reconciles abandoned durable outputs, preserves live QA leases, bounds expired retries, and atomically claims canonical video jobs.';
