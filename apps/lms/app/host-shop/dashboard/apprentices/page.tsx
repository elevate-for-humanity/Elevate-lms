  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5"><ShieldCheck className="h-5 w-5 text-brand-green-700"/><p className="mt-3 text-3xl font-black text-slate-950">{competencyBased ? `${board.tradeInfo.competencyCount ?? 14} skills` : `${Number(board.tradeInfo.hours ?? 0).toLocaleString()}h`}</p><p className="text-sm text-slate-600">{competencyBased ? `${board.tradeInfo.rtiHours ?? 260} RTI hours · competency-based` : `OJT target · ${board.tradeInfo.label}`}</p></div>
      </div>
                ...
                <article key={apprentice.id} className="px-5 py-5 sm:px-6">
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="md:w-80">
                      {competency ? <><div className="flex items-center justify-between text-sm"><span className="font-bold text-slate-800">Appendix A competency progress</span><span className="font-black text-slate-950">{competency.completed} / {competency.required}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-brand-blue-700" style={{ width: `${competencyPct}%` }}/></div><p className="mt-1 text-right text-xs text-slate-500">{competencyPct}% verified · {completedHours.toLocaleString()} approved work hours recorded</p></> : <><div className="flex items-center justify-between text-sm"><span className="font-bold text-slate-800">OJT progress</span><span className="font-black text-slate-950">{completedHours.toLocaleString()} / {Number(requiredHours ?? 0).toLocaleString()}h</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-brand-blue-700" style={{ width: `${hourPct}%` }}/></div><p className="mt-1 text-right text-xs text-slate-500">{hourPct}% complete</p></>}
                    </div>
                  </div>
                </article>
              );
