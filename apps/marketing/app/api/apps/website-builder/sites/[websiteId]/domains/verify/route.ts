/**
 * POST /api/apps/website-builder/sites/[websiteId]/domains/verify
 * Force a status refresh + verification check for a domain (manual retry).
 */
...
export async function POST(
  ...
  if (!isDomaineeConfigured() || !row.domainee_domain_id) {
    return NextResponse.json({ domain: row, message: 'Domain service not configured' });
  }
  try {
      ...
      message:
        domain.status === 'verified' || domain.status === 'active'
          ...
          : domain.status === 'pending'
            ? 'DNS not yet resolved. Add the CNAME record and try again in a minute.'
