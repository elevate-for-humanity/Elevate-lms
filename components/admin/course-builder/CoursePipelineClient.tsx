...
function saveDraft(config: DraftConfig) { try { localStorage.setItem(DRAFT_KEY, JSON.stringify(config)); } catch {} }
...
export default function CoursePipelineClient({ programs, onCourseCreated }: { programs: Program[]; onCourseCreated?: (courseId: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (!draft) return;
    setTitle(draft.title); setTopic(draft.topic); setDifficulty(draft.difficulty); setProgramId(draft.programId);
    setModuleCount(draft.moduleCount); setLessonsPerModule(draft.lessonsPerModule); setIncludeVideos(draft.includeVideos); setDryRun(draft.dryRun); setDraftRestored(true);
  }, []);

  const currentConfig = useCallback((): DraftConfig => ({ title, topic, difficulty, programId, moduleCount, lessonsPerModule, includeVideos, dryRun }), [title, topic, difficulty, programId, moduleCount, lessonsPerModule, includeVideos, dryRun]);
  useEffect(() => {
    if (!title && !topic) return;
    if (autosaveTimer.current) clearTimeout(autosaveTimer.current);
    autosaveTimer.current = setTimeout(() => { saveDraft(currentConfig()); setLastSaved(new Date()); }, 800);
  ...
  async function run() {
    if (!title.trim() || !topic.trim() || !programId) return setError('Title, topic, and program are required.');
    setRunning(true); setEvents([]); setResult(null); setError(''); setCurrentStage('blueprint'); setDraftRestored(false);
    try {
  ...
  function reset() { setEvents([]); setResult(null); setError(''); setCurrentStage(null); }
  function discardDraft() {
    clearDraft(); setTitle(''); setTopic(''); setDifficulty('intermediate'); setProgramId(''); setModuleCount(6); setLessonsPerModule(5); setIncludeVideos(false); setDryRun(true); setDraftRestored(false); setLastSaved(null);
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-2 text-white">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        <div><h2 className="text-xl font-bold">Course Factory Pipeline</h2><p className="mt-1 text-sm text-slate-400">Blueprint → lessons → assessments → validation → persistence → media queue.</p></div>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <section className="space-y-4 rounded-xl border border-slate-800 bg-slate-900 p-5 lg:col-span-2">
          {draftRestored && <div className="flex items-center justify-between gap-2 rounded-lg border border-brand-blue-800 bg-brand-blue-950/40 px-3 py-2 text-xs text-brand-blue-200"><span>Restored your unfinished pipeline configuration.</span><button onClick={discardDraft} className="underline">Discard</button></div>}
          ...
          <div><label className="mb-1.5 block text-xs font-medium text-slate-400">Difficulty</label><div className="flex gap-2">{(['beginner','intermediate','advanced'] as const).map((level) => <button key={level} onClick={() => setDifficulty(level)} disabled={running} className={`flex-1 rounded-lg border px-2 py-2 text-xs capitalize ${difficulty === level ? 'border-brand-blue-500 bg-brand-blue-600 text-white' : 'border-slate-700 bg-slate-800 text-slate-300'}`}>{level}</button>)}</div></div>
          ...
          <div className="flex gap-2"><button onClick={run} disabled={running || !title.trim() || !topic.trim() || !programId} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-brand-blue-600 py-2.5 text-sm font-semibold hover:bg-brand-blue-700 disabled:opacity-40">{running ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}{running ? 'Running…' : dryRun ? 'Run Validation' : 'Generate Draft'}</button>{(events.length > 0 || result) && !running && <button onClick={reset} className="rounded-lg border border-slate-700 bg-slate-800 px-3"><RotateCcw className="h-4 w-4" /></button>}</div>
        </section>

        <section className="space-y-4 lg:col-span-3">
          <div className="rounded-xl border border-slate-800 bg-slate-900 p-5"><h3 className="mb-4 text-sm font-semibold text-slate-300">Pipeline Stages</h3><div className="space-y-2">{STAGES.map((stage, index) => { const Icon = stage.icon; const active = currentStage === stage.id; const done = currentIndex > index || (result && result.success && currentIndex === index); return <div key={stage.id} className="flex items-center gap-3"><div className={`flex h-7 w-7 items-center justify-center rounded-full ${done ? 'bg-green-900/50 text-green-400' : active ? 'bg-brand-blue-900/50 text-brand-blue-400' : 'bg-slate-800 text-slate-600'}`}>{active && running ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Icon className="h-3.5 w-3.5" />}</div><span className={`text-sm ${done ? 'text-green-400' : active ? 'font-medium text-white' : 'text-slate-500'}`}>{stage.label}</span></div>; })}</div></div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <div><label className="mb-1.5 block text-xs font-medium text-slate-400">{label}</label>{children}</div>; }
function Metric({ label, value }: { label: string; value: unknown }) { return <div><p className="text-xs text-slate-400">{label}</p><p className="font-medium text-white">{String(value ?? 0)}</p></div>; }
