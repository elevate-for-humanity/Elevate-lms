...
export default async function HostShopProfilePage() {
  ...
  return (
    <main className="mx-auto w-full min-w-0 max-w-6xl overflow-x-clip px-4 py-8 sm:px-6" style={{ contain: 'inline-size', maxWidth: '100vw' }}>
      <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.14em] text-brand-blue-700">Host Shop Profile</p><h1 className="mt-2 break-words text-3xl font-black text-slate-950">{board.partner?.name || board.shops[0]?.name || 'Host Shop'}</h1><p className="mt-2 text-slate-600">Live partner, compliance, location, and public-profile information from Supabase.</p></div>
        <div className="flex min-w-0 flex-wrap gap-2">{!isPlatformAdmin ? <Link href="/host-shop/onboarding/profile" className="inline-flex min-h-11 items-center justify-center rounded-xl bg-brand-blue-700 px-4 py-2 text-sm font-bold text-white">Update business profile</Link> : null}<Link href="/host-shop/dashboard" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-800">Dashboard</Link></div>
      </div>

      <div className="mt-6 grid min-w-0 gap-4 md:grid-cols-3">
        <section className="min-w-0 max-w-full rounded-2xl border border-slate-200 bg-white p-5 md:col-span-2"><div className="flex items-center gap-2"><Building2 className="h-5 w-5 text-brand-blue-700"/><h2 className="break-words font-black">Business record</h2></div><dl className="mt-5 grid min-w-0 gap-5 sm:grid-cols-2"><div><dt className="text-xs font-bold uppercase text-slate-500">Partner name</dt><dd className="mt-1 break-words font-semibold">{board.partner?.name || 'Not provided'}</dd></div><div><dt className="text-xs font-bold uppercase text-slate-500">Program</dt><dd className="mt-1 break-words font-semibold">{board.tradeInfo.label}</dd></div><div><dt className="text-xs font-bold uppercase text-slate-500">Approval</dt><dd className="mt-1 font-semibold capitalize">{board.partner?.approval_status || 'pending'}</dd></div><div><dt className="text-xs font-bold uppercase text-slate-500">Verification</dt><dd className="mt-1 font-semibold capitalize">{publicProfile?.verification_status || 'pending'}</dd></div></dl></section>
        <section className="min-w-0 max-w-full rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-brand-green-700"/><h2 className="break-words font-black">Compliance</h2></div><div className="mt-5 space-y-3 text-sm"><div className="flex min-w-0 flex-wrap justify-between gap-2"><span>MOU</span><b>{board.partner?.mou_signed ? 'Signed' : 'Pending'}</b></div><div className="flex min-w-0 flex-wrap justify-between gap-2"><span>Orientation</span><b>{board.partner?.onboarding_completed ? 'Complete' : 'Incomplete'}</b></div><div className="flex min-w-0 flex-wrap justify-between gap-2"><span>Documents</span><b>{board.requiredDocumentCount ? `${board.acceptedDocumentCount}/${board.requiredDocumentCount} accepted` : board.partner?.documents_verified ? 'Verified' : 'Pending'}</b></div></div><Link href="/host-shop/dashboard/documents" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-brand-blue-700"><FileText className="h-4 w-4"/> Review documents</Link></section>
      </div>
