...
const AVAILABLE_PROGRAMS = [
  {
    ...
    description: 'Host barber apprentices for hands-on training',
  },
  {
    id: 'COSMETOLOGY',
    name: 'Cosmetology',
    description: 'Beauty and cosmetology training placement',
  },
  {
    id: 'CNA',
    name: 'CNA/Healthcare',
    description: 'Certified Nursing Assistant clinical placement',
  },
  {
    id: 'HVAC',
    name: 'HVAC',
    description: 'Heating, ventilation, and air conditioning apprenticeship',
  },
];

const US_STATES = ['Indiana', 'Illinois', 'Ohio', 'Michigan', 'Kentucky'];
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <Breadcrumbs
            items={[{ label: 'Partner', href: '/partner/attendance' }, { label: 'Onboarding' }]}
          />
        </div>
      </div>
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {[1, 2, 3, 4, 5].map((s) => (
            <div key={s} className="flex items-center">
              ...
              {s < 5 && (
                <div className={`w-8 h-1 ${s < step ? 'bg-brand-green-500' : 'bg-slate-700'}`} />
              )}
            </div>
          ))}
        </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <p className="text-slate-600 text-sm">
                Select the programs you want to host apprentices for.
              </p>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                <p className="text-amber-800 text-sm">
                  <strong>All documents are required.</strong> Your account will be activated
                  automatically once documents are verified.
                </p>
              </div>
              <div className="space-y-4">
                {requiredDocs.map((docType) => {
                  ...
                  return (
                    <div
                      key={docType}
                      className={`p-4 border-2 rounded-lg ${uploaded ? 'border-brand-green-500 bg-brand-green-50' : 'border-slate-200'}`}
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          {uploaded && (
                            <p className="text-sm text-brand-green-600">{uploaded.name}</p>
                          )}
                        </div>
                        {uploaded ? (
                          <button
                          ...
                          <label className="cursor-pointer bg-brand-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-blue-700">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                e.target.files?.[0] && handleFileUpload(docType, e.target.files[0])
                              }
                            />
                          </label>
                        )}
              ...
              <div className="bg-white rounded-lg p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {formData.programsRequested.map((p) => (
                      <span
                        key={p}
                        className="bg-brand-blue-100 text-brand-blue-700 px-3 py-1 rounded-full text-sm"
                      >
                ...
                <div>
                  ...
                  <p className="font-medium text-brand-green-600">
                    {uploadedFiles.length} of {requiredDocs.length}
                  </p>
                </div>
              </div>
              <div className="bg-brand-green-50 border border-brand-green-200 rounded-lg p-4">
                <p className="text-brand-green-800 text-sm">
                  <strong>No admin approval required.</strong> Your account will be activated
                  automatically once documents are verified (typically within 24-48 hours).
                </p>
              </div>
            ...
            {step < 5 ? (
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    Submit Application <span className="text-slate-500 flex-shrink-0">•</span>
                  </>
                )}
              </button>
            )}
