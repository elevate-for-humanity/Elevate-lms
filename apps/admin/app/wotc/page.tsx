...
export default async function WOTCAdminPage() {
  ...
  const activeCount = allApps.filter((a) =>
    ['submitted', 'pending_review'].includes(a.status),
  ).length;
  const approvedThisMonth = allApps.filter((a) => {
    ...
    const now = new Date();
    return (
      approvedDate.getMonth() === now.getMonth() && approvedDate.getFullYear() === now.getFullYear()
    );
  }).length;
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {(app.target_groups || []).slice(0, 2).map((group: string, i: number) => (
                          <span
                            key={i}
                            className="px-2 py-1 bg-brand-blue-100 text-brand-blue-700 text-xs rounded-full"
                          >
                            {group}
                          </span>
                        ))}
