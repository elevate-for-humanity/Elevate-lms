    ...
    <section className="bg-white px-4 py-16 sm:py-20" aria-labelledby="featured-pathways-heading">
      <div className="mx-auto max-w-6xl">
        <div className="mb-10 max-w-3xl">
          <p className="mb-3 text-sm font-extrabold uppercase tracking-[0.14em] text-brand-red-700">Career Pathways</p>
          <h2 id="featured-pathways-heading" className="text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">See yourself in the next step.</h2>
          <p className="mt-4 text-lg leading-8 text-slate-700">Explore hands-on training, apprenticeships, and career pathways designed around real work and industry credentials.</p>
        </div>

        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {PATHWAYS.map((p, index) => (
            <article key={p.slug} className={`group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${index < 2 ? 'lg:col-span-1' : ''}`}>
              <Link href={p.ctaHref} className="block">
                <div className="relative h-56 overflow-hidden bg-slate-100 sm:h-64">
                  <Image src={p.image} alt={p.imageAlt} fill className="object-cover transition-transform duration-500 group-hover:scale-[1.04]" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" loading="lazy" />
                  <div className="absolute left-4 top-4 rounded-full bg-slate-950/85 px-3 py-1.5 text-xs font-extrabold uppercase tracking-wide text-white backdrop-blur-sm">{p.badge}</div>
                </div>
                <div className="p-6 sm:p-7">
                  ...
                  <span className="mt-5 inline-flex items-center gap-2 text-base font-extrabold text-brand-red-700">Explore pathway <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
                </div>
              </Link>
            </article>
          ))}
        ...
        <div className="mt-10 flex justify-center">
          <Link href="/programs" className="inline-flex items-center gap-2 rounded-xl bg-brand-red-600 px-8 py-4 text-base font-extrabold text-white transition hover:bg-brand-red-700">Explore All Programs <ArrowRight className="h-5 w-5" /></Link>
        </div>
      </div>
    </section>
  );
