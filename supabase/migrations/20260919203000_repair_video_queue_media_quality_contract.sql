-- Keep the queue claim transaction aligned with the canonical lesson media
-- quality vocabulary. `video_jobs.review_status` uses `pending_review`, while
-- `course_lessons.media_quality_status` deliberately uses `pending`.
...
begin
  ...
  -- Reconcile jobs whose durable output was written before a previous worker
  -- lost its lease. Never render those assets twice.
  ...
  set status = 'complete',
      ...
      heartbeat_at = null,
      updated_at = now()
  where v.status in ('queued', 'rendering')
  ...
  with candidates as (
    ...
    from public.video_jobs v
    where nullif(btrim(v.video_url), '') is null
      and (
        (v.status = 'queued' and coalesce(v.retry_count, 0) < 3 and (v.next_retry_at is null or v.next_retry_at <= now()))
        or (v.status = 'rendering' and v.lease_expires_at is not null and v.lease_expires_at <= now() and coalesce(v.retry_count, 0) < 3)
      )
      and v.dead_lettered_at is null
      and (p_course_id is null or v.course_id = p_course_id)
  ...
  set status = 'rendering',
      retry_count = case when c.status = 'rendering' then coalesce(v.retry_count, 0) + 1 else coalesce(v.retry_count, 0) end,
...
comment on function public.claim_video_jobs(integer, uuid, integer) is
  'Reconciles durable outputs, bounds expired retries, and atomically claims canonical video jobs.';
