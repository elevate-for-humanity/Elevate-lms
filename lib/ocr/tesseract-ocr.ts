...
export async function extractW2Data(buffer: Buffer): Promise<W2Data | null> {
  const result = await autoExtract(buffer);
  // Placeholder - actual OCR parsing would extract W2 fields
  return null;
}

export async function extract1099Data(buffer: Buffer): Promise<TaxFormData | null> {
  const result = await autoExtract(buffer);
  // Placeholder - actual OCR parsing would extract 1099 fields
  return null;
}

export async function extractIDData(buffer: Buffer): Promise<IDData | null> {
  const result = await autoExtract(buffer);
  // Placeholder - actual OCR parsing would extract ID fields
  return null;
}
