...
export async function GET(request: NextRequest) {
  return capabilityHealthResponse(request, async () => {
    const tokenPresent = Boolean(process.env.NORTHFLANK_API_TOKEN || process.env.NORTHFLANK_API_KEY || process.env.NF_API_TOKEN);
    ...
    return buildCapabilityHealth('deployments', [
      ...
      { name: 'northflank-runtime', passed: runtimeReady, required: true, message: runtimeReady ? 'Northflank runtime reports ready.' : 'Northflank runtime is not ready.' },
    ]);
  });
}
