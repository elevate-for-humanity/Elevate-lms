...
function MOUBuilder() {
  ...
  const update = (field: keyof MOUTemplate, value: any) =>
    setMou((prev) => ({ ...prev, [field]: value }));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        ...
        <EditableList
          label="Elevate Responsibilities"
          items={mou.elevateResponsibilities}
        ...
        <EditableList
          label="Partner Responsibilities"
          items={mou.partnerResponsibilities}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            {savedVersion && <p className="text-xs text-slate-700 mt-0.5">Saved v{savedVersion}</p>}
          </div>
...
function ReportBuilder() {
  const [report, setReport] = useState<ReportTemplate>({
    ...
    programOverview:
      'The HVAC Technician Training Program is a 12-week workforce development program preparing participants for entry-level HVAC technician positions. The program includes classroom instruction, hands-on lab exercises, and industry credential preparation.',
    keyActivities: [
      'Completed Module 1-4 classroom instruction',
      'Administered EPA 608 practice exams',
      'Conducted employer site visits',
    ],
    operationalUpdates: ['Updated curriculum to include new refrigerant handling procedures'],
    challengesOrRisks: [],
    nextSteps: [
      'Begin Module 5-8 instruction',
      'Schedule EPA 608 certification testing',
      'Coordinate OJT placements',
    ],
    summary:
      'The program is on track with participant engagement and credential preparation proceeding as planned.',
  });
  const [copied, setCopied] = useState(false);
  const [saveStatus, setSaveStatus] = useState<SaveStatus>('idle');
  const [savedVersion, setSavedVersion] = useState<string | null>(null);
  ...
  const update = (field: keyof ReportTemplate, value: any) =>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="space-y-4">
        ...
        <EditableList
          ...
          items={report.operationalUpdates}
        ...
        <EditableList
          ...
          items={report.challengesOrRisks}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <Icon
                  className={`w-5 h-5 ${selected.id === t.id ? 'text-brand-blue-600' : 'text-slate-700'}`}
                />
                <div>
      ...
      <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            onClick={() =>
              copyBody(selected.id, `Subject: ${selected.subject}\n\n${selected.body}`)
            }
            className="flex items-center gap-2 px-3 py-1.5 bg-brand-blue-600 text-white rounded-lg text-sm hover:bg-brand-blue-700"
          >
          ...
          <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <div className="flex flex-wrap gap-1.5">
              {selected.placeholders.map((p) => (
                <span
                  key={p}
                  className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-mono"
                >
                  {p}
                </span>
              ))}
...
function EditableList({
  ...
  return (
    <div>
      ...
      <button
        onClick={onAdd}
        className="mt-1 text-sm text-brand-blue-600 hover:text-brand-blue-700 font-medium"
      >
        + Add item
      </button>
    </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-700 mt-2">
            Standardized templates for MOUs, reports, and professional emails. Fill in the fields,
            copy the output.
          </p>
        </div>
          ...
          {tabs.map((t) => {
            ...
            return (
              ...
              >
                ...
                {t.label}
                <span
                  className={`text-xs ${tab === t.id ? 'text-brand-blue-200' : 'text-slate-700'}`}
                >
                  {t.desc}
                </span>
              </button>
