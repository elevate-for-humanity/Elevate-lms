...
export const metadata: Metadata = {
  robots: { index: false }, title: 'Compliance Records | Submissions OS | Admin' };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm">
                Licenses, certifications, SAM registration, ETPL listing, insurance
              </p>
            </div>
          </div>
          <Link
        ...
        {expiringSoon.length > 0 && (
          <div className="mb-5 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
            ...
            <p className="text-sm text-red-800 font-semibold">
              {expiringSoon.length} record{expiringSoon.length > 1 ? 's' : ''} expiring within 60
              days: {(expiringSoon as any[]).map((r: any) => r.title).join(', ')}
            </p>
          </div>
        )}
        ...
        {!records || records.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-10 text-center">
            ...
            <p className="text-sm text-slate-500 mb-4">
              Add SAM registration, state licenses, certifications, and insurance records.
            </p>
            <Link
              href="/submissions/compliance"
              className="inline-flex items-center gap-2 px-4 py-2 bg-brand-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-brand-blue-700 transition"
