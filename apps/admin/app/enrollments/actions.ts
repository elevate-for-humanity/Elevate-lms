...
export async function updateEnrollment(
  ...
  const { error } = await db
    ...
    .eq('id', enrollmentId);
  if (error) {
    logger.error('updateEnrollment', undefined, { reason: error?.message });
...
export async function createEnrollment(data: {
  ...
  const { data: enrollment, error } = await db
    ...
    .single();
  if (error) {
    logger.error('createEnrollment', undefined, { reason: error?.message });
  ...
  await writeAdminAuditEvent(supabase, {
    ...
    metadata: { user_id: data.user_id, course_id: data.course_id },
  });
  return enrollment;
}
...
export async function deleteEnrollment(enrollmentId: string, userId: string, courseId: string) {
  ...
  if (error) {
    logger.error('deleteEnrollment', undefined, { reason: error?.message });
...
export async function toggleAtRisk(enrollmentId: string, currentValue: boolean) {
  ...
  await writeAdminAuditEvent(supabase, {
    action: 'enrollment_at_risk_toggled',
    target_type: 'enrollment',
    target_id: enrollmentId,
    metadata: { at_risk: !currentValue },
  });
}
...
export async function markEnrollmentComplete(enrollmentId: string) {
  ...
  await writeAdminAuditEvent(supabase, {
    action: 'enrollment_marked_complete',
    target_type: 'enrollment',
    target_id: enrollmentId,
  });
}
...
export async function approveEnrollment(enrollmentId: string, userId: string) {
  ...
  await writeAdminAuditEvent(supabase, {
    ...
    target_type: 'enrollment',
    target_id: enrollmentId,
    metadata: { approved_by: user.id, user_id: userId },
  });
}
