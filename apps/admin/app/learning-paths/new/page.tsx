...
export default async function NewLearningPathPage() {
  ...
  const { data: programs } = await db
    .from('programs')
    .select('id,title,name,duration_weeks,estimated_weeks')
    .eq('is_active', true)
    .order('title');
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      <nav className="flex items-center gap-1.5 text-xs text-slate-500 mb-6">
        ...
        <Link href="/learning-paths" className="hover:text-slate-700">
          Learning Paths
        </Link>
        <ChevronRight className="w-3 h-3" />
      ...
      <NewLearningPathClient
        programs={(programs ?? []).map((program) => ({
          ...
          name: program.title || program.name || 'Untitled program',
          durationWeeks: Number(program.duration_weeks || program.estimated_weeks || 0),
        }))}
      />
    </div>
  );
