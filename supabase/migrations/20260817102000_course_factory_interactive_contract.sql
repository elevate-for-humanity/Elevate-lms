-- Make the Course Factory handoff complete and refreshable in place.
-- `refresh` updates module/lesson rows by canonical slug so learner progress,
-- external references and stable lesson URLs survive a Course Builder rerun.
    ...
    LOOP
      ...
      IF p_mode = 'refresh' AND v_lesson_id IS NOT NULL THEN
        ...
        SET module_id = v_module_id,
            ...
            lesson_type = coalesce(nullif(v_lesson->>'lesson_type', ''), 'lesson')::lesson_type,
...
IS 'Atomic Course Factory publisher with stable-ID refresh and complete interactive/media lesson contract.';
