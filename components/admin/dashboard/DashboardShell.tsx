...
export function AdminDashboardContent({ data, canAccessDevStudio = false }: { data: AdminDashboardData; canAccessDevStudio?: boolean }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] min-w-0 px-3 pb-16 pt-4 sm:px-5 lg:px-6">
      ...
      <section className="mt-8" aria-labelledby="portal-access-title"><div className="mb-4"><h2 id="portal-access-title" className="text-xl font-black text-slate-950">Portal apps and secure portal access</h2><p className="mt-1 text-sm font-medium text-slate-700">Select a real account before previewing its dashboard, or install the appropriate PWA shell on this device.</p></div><PwaInstallCenter /><HostShopPortalLauncher /></section>
    </div>
  );
}
