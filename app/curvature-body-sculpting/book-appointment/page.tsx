      ...
      <div className="min-h-screen bg-pink-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center shadow-xl">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 text-brand-red-600 font-medium hover:text-brand-red-700"
          >
            ...
            <div key={s} className="flex items-center gap-2">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                  step >= s ? 'bg-brand-red-600 text-white' : 'bg-gray-200 text-gray-500'
                }`}
              >
                {step > s ? <Circle className="w-5 h-5" /> : s}
              </div>
              <span className={`text-sm ${step >= s ? 'text-brand-red-600 font-medium' : 'text-gray-500'}`}>
                {s === 1 ? 'Service' : s === 2 ? 'Date & Time' : 'Your Info'}
              </span>
              {s < 3 && <div className={`w-12 h-0.5 ${step > s ? 'bg-brand-red-600' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <span className={`font-bold ${price === 'FREE' ? 'text-green-600' : 'text-brand-red-600'}`}>
                            {price}
                          </span>
                        </div>
                      </button>
              ...
              <div className="mt-8 flex justify-end">
                <button
                  ...
                  className="px-6 py-3 bg-brand-red-600 text-white font-bold rounded-lg hover:bg-brand-red-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition flex items-center gap-2"
                >
          ...
          {step === 3 && (
            <form onSubmit={handleSubmit}>
              ...
              {/* Summary */}
              <div className="bg-brand-red-50 rounded-lg p-4 mb-6">
