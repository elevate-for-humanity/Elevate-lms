      ...
      >
        ...
        {!collapsed && (
          <>
            ...
            {badge !== undefined && (
              <span className="rounded-full bg-white/15 px-1.5 py-0.5 text-[10px] font-black">
                {badge}
              </span>
            )}
          </>
    ...
    <div className="flex h-full flex-col">
      <div className="flex h-16 flex-shrink-0 items-center justify-between border-b border-slate-800 px-4">
        <Link href="/lms/dashboard" className="flex min-w-0 items-center gap-2.5">
          ...
          {!collapsed && (
            <span className="truncate text-sm font-black tracking-tight text-white">Elevate</span>
          )}
        </Link>
        <button
      ...
      <div
        className={`flex-shrink-0 border-b border-slate-800 px-4 py-4 ${collapsed ? 'px-2' : ''}`}
      >
        <div className="flex items-center gap-3">
          {profile?.avatar_url ? (
            <ProfileImage
              ...
              alt={userName}
              className="h-9 w-9 flex-shrink-0 rounded-full bg-white object-cover ring-2 ring-slate-700"
            />
          ) : (
        ...
        {profile?.portal_type && PORTAL_TYPE_TO_PATH[profile.portal_type] && (
          <div className="mb-4">
            ...
            <Link
              ...
              onClick={() => setMobileOpen(false)}
              className={`mb-0.5 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium ${pathname?.startsWith('/apprentice') ? 'bg-brand-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
              <Scissors className="h-4 w-4" />
              {!collapsed && <span>My Apprenticeship</span>}
            </Link>
          </div>
        )}
      ...
      <div className="flex-shrink-0 space-y-0.5 border-t border-slate-800 px-2 py-3">
        ...
        <button
          ...
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-500 transition hover:bg-slate-800 hover:text-red-400"
        >
    ...
    <>
      <div className="fixed left-0 right-0 top-0 z-40 flex h-14 items-center justify-between border-b border-slate-800 bg-slate-900 px-4 lg:hidden">
        <Link href="/lms/dashboard" className="flex items-center gap-2">
          ...
          <span className="text-sm font-black text-white">Elevate</span>
        </Link>
      ...
      <div
        className={`fixed bottom-0 left-0 top-14 z-40 w-72 transform bg-slate-900 transition-transform duration-300 lg:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {sidebarContent}
      </div>
      <div
        className={`fixed bottom-0 left-0 top-0 z-30 hidden flex-col border-r border-slate-800 bg-slate-900 transition-all duration-300 lg:flex ${collapsed ? 'w-16' : 'w-64'}`}
      >
        {sidebarContent}
      </div>
    </>
