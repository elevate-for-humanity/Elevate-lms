    ...
    <section className="bg-white py-16" id="guided-setup">
      <div className="mx-auto max-w-6xl px-5">
        <div className="mx-auto max-w-3xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full bg-brand-red-50 px-4 py-2 text-sm font-black text-brand-red-700"><Sparkles className="h-4 w-4" /> Zero-code guided setup</span>
          <h2 className="mt-4 text-3xl font-black text-slate-950 sm:text-5xl">Tell Elevate what you want to accomplish.</h2>
          <p className="mt-4 text-lg leading-8 text-slate-600">You do not need to understand product names or technical settings. Answer two questions and Elevate will recommend the simplest starting stack.</p>
        </div>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 md:p-8">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">{GOALS.map((item) => <button key={item.id} type="button" onClick={() => setGoal(item.id)} className={`rounded-xl border px-4 py-3 text-left text-sm font-bold transition ${goal === item.id ? 'border-brand-red-600 bg-brand-red-50 text-brand-red-800' : 'border-slate-200 bg-white text-slate-800 hover:border-slate-300'}`}>{item.label}</button>)}</div>
            <p className="mt-7 text-sm font-black uppercase tracking-wider text-slate-500">2. Who is this for?</p>
            <div className="mt-4 grid gap-2 sm:grid-cols-2">{ORGS.map((item) => <button key={item.id} type="button" onClick={() => setOrg(item.id)} className={`rounded-xl border px-4 py-3 text-left text-sm font-bold transition ${org === item.id ? 'border-brand-red-600 bg-brand-red-50 text-brand-red-800' : 'border-slate-200 bg-white text-slate-800 hover:border-slate-300'}`}>{item.label}</button>)}</div>
          </div>
          ...
          <div className="rounded-3xl bg-slate-950 p-6 text-white md:p-8">
            ...
            {!goal || !org ? <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-6 text-slate-300">Choose a goal and organization type. Elevate will explain what to start with and why.</div> : <div className="mt-6 space-y-4">{recommendations.map((item) => <article key={item.name} className="rounded-2xl border border-white/10 bg-white/5 p-5"><div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-black">{item.name}</h3>{item.badge ? <span className="rounded-full bg-brand-red-600 px-2 py-1 text-[11px] font-black uppercase">{item.badge}</span> : null}</div><p className="mt-2 text-sm leading-6 text-slate-300">{item.reason}</p></div><CheckCircle2 className="h-5 w-5 shrink-0 text-emerald-400" /></div><Link href={item.href} className="mt-4 inline-flex items-center gap-2 text-sm font-black text-white">See this product <ArrowRight className="h-4 w-4" /></Link></article>)}</div>}
