  ...
  useEffect(() => {
    ...
    async function initialize() {
      ...
      try {
        ...
        if (!cancelled) {
          setMessage(error instanceof Error ? error.message : 'Unable to prepare MFA.');
        }
      } finally {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Privileged Elevate access requires an AAL2 session when MFA enforcement is enabled.
            </p>
          </div>
        </div>

        {loading ? (
          <div className="mt-8 flex items-center justify-center gap-2 py-10 text-sm text-slate-400">
          ...
          <>
            {mode === 'enroll' && qrCode && (
              <section className="mt-6 rounded-xl border border-slate-700 bg-white p-5 text-slate-900">
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Scan this QR code with a TOTP authenticator such as Google Authenticator,
                  Microsoft Authenticator, 1Password, or another compatible app.
                </p>
            ...
            <form onSubmit={verify} className="mt-6 space-y-4">
              <div>
                <label htmlFor="mfa-code" className="text-sm font-semibold text-slate-200">
                  {mode === 'enroll' ? '2. Enter the verification code' : 'Authenticator code'}
                </label>
                <div className="relative mt-2">
                  ...
                  <input
                    ...
                    placeholder="000000"
                    className="w-full rounded-xl border border-slate-700 bg-slate-950 py-3 pl-10 pr-3 text-lg tracking-[0.25em] outline-none focus:border-cyan-500"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-500 px-4 py-3 font-black text-slate-950 hover:bg-cyan-400 disabled:opacity-50"
              >
