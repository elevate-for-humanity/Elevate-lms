...
export default function AIChatPage() {
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Hero */}
      <section className="bg-brand-red-600 text-white py-20">
        <div className="max-w-5xl mx-auto px-4 text-center">
          ...
          <Link
            href="/ai-tutor"
            className="inline-flex items-center gap-2 bg-white text-brand-red-600 px-8 py-4 rounded-full font-bold text-lg hover:bg-brand-red-50 transition-colors"
          >
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-white rounded-2xl shadow-xl border overflow-hidden">
            {/* Chat Header */}
            <div className="bg-brand-red-600 p-4 flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                <Bot className="w-6 h-6 text-white" />
              </div>
              <div>
                <h3 className="font-semibold text-white">Elevate AI Assistant</h3>
                <p className="text-brand-red-200 text-sm">Always here to help</p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4 bg-gray-50">
              ...
              <div className="flex gap-3">
                <div className="w-8 h-8 bg-brand-red-100 rounded-full flex items-center justify-center flex-shrink-0">
                  <Bot className="w-5 h-5 text-brand-red-600" />
                </div>
              ...
              <div className="flex gap-3 justify-end">
                <div className="bg-brand-red-600 text-white rounded-2xl rounded-tr-none p-4 max-w-md">
            ...
            <div className="p-4 border-t bg-white">
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-brand-red-500"
                  disabled
                />
                <Link
                  href="/ai-tutor"
                  className="px-6 py-3 bg-brand-red-600 text-white rounded-full font-semibold hover:bg-brand-red-700 transition-colors"
                >
                  Start Chat
                </Link>
              </div>
            ...
            {features.map((feature, index) => (
              <div key={index} className="text-center">
                <div className="w-16 h-16 bg-brand-red-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <feature.icon className="w-8 h-8 text-brand-red-600" />
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {sampleQuestions.map((question, index) => (
              <Link
                key={index}
                href="/ai-tutor"
                className="flex items-center gap-3 p-4 bg-white rounded-xl border hover:border-brand-red-300 hover:shadow-md transition-all"
              >
                <MessageSquare className="w-5 h-5 text-brand-red-600 flex-shrink-0" />
