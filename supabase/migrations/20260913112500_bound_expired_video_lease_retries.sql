-- Prevent long-running or crashed media work from being reclaimed forever.
-- A queued job must still have retry budget, and an expired rendering lease
-- consumes one retry before it can be reclaimed.
...
comment on function public.claim_video_jobs(integer, uuid, integer) is
  'Atomically claims bounded queued jobs or expired rendering leases without exceeding retry limits.';
