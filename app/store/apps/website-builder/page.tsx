          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="flex items-center gap-2 mb-4">
                <span className="bg-brand-red-500 text-white text-xs font-bold px-3 py-1 rounded-full">WEBSITE</span>
            ...
            {features.map((f, i) => (
              <div key={i} className="p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-shadow">
                <div className="w-14 h-14 bg-brand-red-100 rounded-xl flex items-center justify-center mb-4">
                  <f.icon className="w-7 h-7 text-brand-red-600" />
                </div>
      ...
      <section className="py-16 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {pricing.map((plan, i) => (
              <div key={i} className={`rounded-2xl p-8 ${plan.popular ? 'bg-brand-red-600 text-white ring-4 ring-brand-red-300' : 'bg-white border border-gray-200'}`}>
                ...
                <div className="mt-4 mb-6">
                  ...
                  <span className={plan.popular ? 'text-brand-red-100' : 'text-gray-500'}>{plan.period}</span>
                </div>
                <ul className="space-y-3 mb-8">
                  {plan.features.map((f, j) => (
                    <li key={j} className="flex items-center gap-2">
                      <Check className={`w-5 h-5 ${plan.popular ? 'text-brand-red-200' : 'text-green-500'}`} />
                      <span className={plan.popular ? 'text-brand-red-100' : 'text-gray-600'}>{f}</span>
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  className={`block w-full text-center py-3 rounded-lg font-bold transition-colors ${
                    plan.popular 
                      ? 'bg-white text-brand-red-600 hover:bg-gray-100' 
