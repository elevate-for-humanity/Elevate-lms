...
import { BookOpen, Bot, Boxes, CheckCircle2, FileCheck2, FlaskConical, Image, Loader2, RefreshCw, Sparkles, Video } from 'lucide-react';
...
type CourseRow = { id: string; title: string; slug: string; program_id?: string | null; status?: string };
...
const TABS: Array<{ id: Tab; label: string; icon: any; description: string }> = [
  { id: 'build', label: 'Build', icon: BookOpen, description: 'Course, modules, lessons and live learner preview' },
  { id: 'ai', label: 'AI Generate', icon: Sparkles, description: 'Generate complete course structures and content' },
  { id: 'blueprints', label: 'Blueprints', icon: Boxes, description: 'Credential and state-aligned course blueprints' },
  { id: 'media', label: 'Video + Audio', icon: Video, description: 'Lesson video, narration and media production' },
  { id: 'interactions', label: 'Interactive', icon: FlaskConical, description: 'Scenarios, practicals, flashcards, hotspots and activities' },
  { id: 'assessments', label: 'Assessments', icon: FileCheck2, description: 'Knowledge checks, checkpoints and final exams' },
  { id: 'compliance', label: 'Compliance', icon: CheckCircle2, description: 'Hours, competencies, sign-off and publish readiness' },
];
...
export default function UnifiedCourseBuilder() {
  ...
  const loadWorkspace = async (id = courseId) => {
    if (!id) return;
    setLoading(true);
    setMessage('');
    try {
  ...
  useEffect(() => { if (courseId) loadWorkspace(courseId); }, [courseId]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="border-b border-slate-800 bg-slate-900 px-5 py-4">
        <div className="mx-auto flex max-w-[1600px] flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">One authoring surface for curriculum, AI, blueprint, media, interaction, assessment and compliance.</p>
          </div>
          <div className="flex min-w-0 items-center gap-2">
            ...
            <button onClick={() => loadWorkspace()} disabled={!courseId || loading} className="rounded-lg border border-slate-700 p-2 hover:bg-slate-800 disabled:opacity-50" title="Refresh course"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button>
          </div>
        </div>
      </div>

      <div className="border-b border-slate-800 bg-slate-900/70 px-4 py-2 overflow-x-auto">
      ...
      <main className="mx-auto max-w-[1600px] p-4">
        ...
        {tab === 'interactions' && <InteractionPanel course={selectedCourse} />}
        {tab === 'assessments' && <AssessmentPanel workspace={workspace} onChanged={() => loadWorkspace()} />}
...
function CreateCoursePanel({ onCreated }: { onCreated: (id: string) => void | Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to create course'); } finally { setSaving(false); }
  }
...
function BlueprintPanel({ blueprints, selectedCourse, onGenerated }: { blueprints: BlueprintRow[]; selectedCourse: CourseRow | null; onGenerated: (id: string) => void | Promise<void> }) {
  ...
  async function generate() {
    ...
    try {
      ...
      setResult(`Generated ${data.moduleCount ?? 0} modules and ${data.lessonCount ?? 0} lessons. Video jobs queued when configured.`);
      if (data.courseId) await onGenerated(data.courseId);
    } catch (e) { setResult(e instanceof Error ? e.message : 'Generation failed'); } finally { setRunning(false); }
  }
  return <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
    ...
    <aside className="rounded-2xl border border-slate-800 bg-slate-900 p-5"><h3 className="font-bold">Build from blueprint</h3><input value={programId} onChange={(e) => setProgramId(e.target.value)} placeholder="Program UUID" className="mt-4 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" /><button onClick={generate} disabled={running || !blueprintId} className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-cyan-500 px-4 py-2 font-bold text-slate-950 disabled:opacity-50">{running ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}Generate + enrich + queue media</button>{result && <p className="mt-3 text-sm text-slate-300">{result}</p>}</aside>
  </div>;
}

function InteractionPanel({ course }: { course: CourseRow | null }) {
  const capabilities = [
    ['Knowledge checks', 'Multiple choice, true/false, fill blank and image-based checks'],
    ['Scenarios', 'Branching workplace decisions with retry and feedback'],
    ['Flashcards', 'Text/image/audio/video recall cards and spaced practice'],
    ['Interactive video', 'Questions inside lesson videos with answer tracking'],
    ['Hands-on practical', 'Observable tasks, repetitions, evidence and instructor sign-off'],
    ['Hotspots', 'Click-to-identify tools, anatomy, equipment and safety areas'],
    ['Drag + drop', 'Matching, categorization, sequencing and diagram labeling'],
  ];
  return <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6"><div className="flex items-center gap-2"><Image className="h-5 w-5 text-cyan-400" /><h2 className="text-xl font-bold">Interactive lesson design</h2></div><p className="mt-2 text-sm text-slate-400">{course ? `Authoring interactions for ${course.title}.` : 'Select a course first.'} These interaction types are part of the unified lesson-experience contract; lesson AI generation now returns structured interaction and practical-task data rather than text alone.</p><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{capabilities.map(([name, desc]) => <div key={name} className="rounded-xl border border-slate-700 bg-slate-950 p-4"><div className="font-bold text-white">{name}</div><p className="mt-1 text-sm text-slate-400">{desc}</p></div>)}</div></div>;
}
...
function AssessmentPanel({ workspace, onChanged }: { workspace: WorkspacePayload | null; onChanged: () => void }) {
  ...
  return <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6"><h2 className="text-xl font-bold">Assessment studio</h2><div className="mt-4 space-y-2">{assessments.length ? assessments.map((lesson: any) => <div key={lesson.id} className="flex items-center justify-between rounded-xl border border-slate-700 bg-slate-950 p-3"><div><div className="font-semibold">{lesson.title}</div><div className="text-xs text-slate-500">{lesson.step_type} · pass {lesson.passing_score ?? 70}%</div></div><button onClick={() => hydrate(lesson)} disabled={runningId === lesson.id} className="rounded-lg bg-cyan-500 px-3 py-2 text-xs font-bold text-slate-950">{runningId === lesson.id ? 'Generating…' : 'Generate / refresh questions'}</button></div>) : <p className="text-sm text-slate-400">No quiz, checkpoint or exam lessons found for the selected course.</p>}</div></div>;
}
...
function CompliancePanel({ workspace }: { workspace: WorkspacePayload | null }) {
  ...
  const cards = [
    ['Course declared hours', String((workspace?.course as any)?.duration_hours ?? '—')],
