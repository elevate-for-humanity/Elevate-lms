...
export default function BarberChatAssistant() {
    ...
    try {
      ...
      if (data.error) {
        setMessages(prev => [...prev, {
          role: 'assistant',
          content: "I'm not sure about that. For specific questions, please call us at Contact Us or email elevate4humanityedu@gmail.com.",
        }]);
      } else {
        setMessages(prev => [...prev, {
          role: 'assistant',
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: "I'm having trouble connecting. Please try again or call us at Contact Us.",
      }]);
    } finally {
      setIsLoading(false);
    }
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-brand-red-600 hover:bg-brand-red-700 text-white px-5 py-3 rounded-full shadow-lg transition-all hover:scale-105"
      >
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-brand-red-600 text-white p-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-brand-red-200">Ask me anything</div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button
            onClick={() => setIsMinimized(!isMinimized)}
            className="p-1.5 hover:bg-brand-red-500 rounded-full transition-colors"
          >
              ...
              <div className="flex flex-wrap gap-2">
                {quickActions.map((action, i) => (
                  <button
                    ...
                    className="text-xs px-3 py-1.5 bg-brand-red-50 text-brand-red-700 rounded-full hover:bg-brand-red-100 transition"
                  >
                    {action.label}
                  </button>
                ))}
          ...
          <div className="p-3 border-t border-gray-200 bg-white">
            ...
            >
              <input
                ...
                className="flex-1 px-3 py-2 border border-gray-300 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-brand-red-500 focus:border-transparent"
                disabled={isLoading}
              />
              <button
                type="submit"
                disabled={isLoading || !input.trim()}
                className="p-2 bg-brand-red-600 text-white rounded-full hover:bg-brand-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
