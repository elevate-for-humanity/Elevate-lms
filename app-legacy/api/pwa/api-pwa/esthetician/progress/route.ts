...
/**
 * GET /api/pwa/esthetician/progress
 *
 * Returns a strictly-typed ApprenticeProgressResponse for the authenticated
 * esthetician apprentice. Validates the payload before returning — if any
 * field is missing or wrong type, returns 500 so the failure is loud.
 *
 * Required hours: 700 (Indiana IPLA esthetician standard)
 */
...
const REQUIRED_HOURS = 700;
const PROGRAM_LABEL  = 'Esthetician Apprenticeship';
const DISCIPLINE     = 'esthetician';
const PROGRAM_SLUG   = 'esthetician-apprenticeship';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (hoursErr) {
      logger.error('[esthetician/progress] hours query failed', hoursErr);
    ...
    if (contractViolation) {
      logger.error('[esthetician/progress] contract violation', undefined, { violation: contractViolation, payload });
