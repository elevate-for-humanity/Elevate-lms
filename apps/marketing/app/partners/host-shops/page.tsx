      ...
      <section className="border-b border-slate-200 bg-slate-50 px-4 py-12">
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 leading-7 text-slate-700">Profiles are tied to the approved operational record. Photos and videos are shown only when sourced from the partner’s own public media or otherwise approved for promotion. Placement is never implied by a public profile alone.</p>
          </div>
                  ...
                  <article key={shop.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                    <div className="grid lg:grid-cols-[1.05fr_.95fr]">
                      <div className="p-6 sm:p-8">
                        ...
                        {shop.description ? <p className="mt-4 max-w-2xl text-base leading-7 text-slate-700">{shop.description}</p> : <p className="mt-4 max-w-2xl text-base leading-7 text-slate-700">Approved worksite participating in supervised apprenticeship training through Elevate.</p>}
                        <p className="mt-5 flex items-start gap-2 text-sm font-bold text-slate-700"><MapPin className="mt-0.5 h-5 w-5 shrink-0 text-brand-red-700" /> {shopAddress}</p>
                        {shop.supervisor ? <p className="mt-2 text-sm font-semibold text-slate-600">Approved supervisor: {shop.supervisor}</p> : null}
                      ...
                      <div className="min-h-[320px] bg-slate-100 p-4 sm:p-6">
                        {media.length || shop.videoUrl || shop.logoUrl || shop.flyerUrl ? (
                          <HostShopMediaCarousel shopName={shop.name} items={[...media, ...(shop.logoUrl ? [{ url: shop.logoUrl, alt: `${shop.name} logo` }] : []), ...(shop.flyerUrl ? [{ url: shop.flyerUrl, alt: `${shop.name} flyer` }] : [])]} videoUrl={shop.videoUrl} />
                        ) : shopAddress ? (
                          <div className="h-full min-h-[300px] overflow-hidden rounded-2xl border border-slate-200 bg-white"><iframe title={`Map — ${shop.name}`} src={embedUrl(shopAddress)} className="h-full min-h-[300px] w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div>
                        ) : null}
                      </div>
                    </div>
                  </article>
