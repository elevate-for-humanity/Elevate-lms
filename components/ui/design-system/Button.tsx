...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'default' | 'primary' | 'secondary' | 'tertiary' | 'danger' | 'destructive' | 'outline' | 'ghost' | 'link';
  size?: 'sm' | 'md' | 'lg' | 'icon';
  loading?: boolean;
  fullWidth?: boolean;
  asChild?: boolean;
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variantStyles = {
      ...
      secondary: 'bg-white text-black border-2 border-slate-300 hover:bg-slate-50 hover:border-slate-400 focus:ring-slate-500',
      tertiary: 'bg-transparent text-brand-orange-600 hover:bg-slate-50 focus:ring-brand-blue-500',
      danger: 'bg-brand-red-600 text-white hover:bg-brand-red-700 focus:ring-brand-red-500',
      destructive: 'bg-brand-red-600 text-white hover:bg-brand-red-700 focus:ring-brand-red-500',
      outline: 'bg-white text-black border-2 border-slate-300 hover:bg-slate-50 hover:border-slate-400 focus:ring-slate-500',
