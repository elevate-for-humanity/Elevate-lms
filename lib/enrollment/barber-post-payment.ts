...
export async function runBarberPostPayment(
  ...
  if (!enrollmentId) {
    ...
    if (existingProfile?.id) {
      ...
      const provision = await provisionAccount({
        ...
        programSlug: BARBER_PROGRAM_SLUG,
  ...
  if (steps['provision_account'] === 'skipped') {
    try {
      ...
      logger.warn('[barber-post-payment] Enrollment confirmation email failed (non-fatal)', err);
      steps['student_email'] = 'failed';
    }
  } else {
    // provisionAccount handled it during account creation
