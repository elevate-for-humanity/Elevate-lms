...
function getTotalHours(program: ProgramSchema) {
...
export default function BeautyProgramSearchSection({ program }: { program: ProgramSchema }) {
  ...
  const hours = getTotalHours(program);
  ...
  const faqs = [
    {
      ...
      answer:
        hours > 0
          ...
          : `The required hours and completion schedule are listed in the program details above. Admissions can confirm the current registered standard before enrollment.`,
    },
    {
