...
export default function TemplateGallery() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/course-builder?action=blueprints', { cache: 'no-store' })
      ...
      .catch((err) => {
        if (!cancelled) setError(err instanceof Error ? err.message : 'Unable to load course blueprints');
      })
      .finally(() => {
  ...
  function choose(template: BlueprintTemplate) {
    ...
    setProgramCode((template.credentialCode || template.slug).toUpperCase().replace(/[^A-Z0-9]+/g, '_'));
    setDurationWeeks('');
  ...
  async function buildSelected() {
    ...
    try {
      ...
      if (!courseResponse.ok || !courseData.ok) {
        throw new Error((courseData.errors ?? [courseData.error ?? 'Course build failed']).join('; '));
      }
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white px-6 py-8">
        <div className="mx-auto max-w-6xl">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-brand-red-600">Course Builder</p>
          <h1 className="mt-1 text-3xl font-black text-slate-950">Choose a canonical blueprint</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Every blueprint is loaded from the canonical registry and built through the same Course Factory,
            including complete lessons, assessments, interactions, narration, media, validation, and persistence.
          </p>
              ...
              {filtered.map((template) => (
                <button
                  ...
                  className={`rounded-2xl border bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
                    selected?.id === template.id ? 'border-brand-red-500 ring-2 ring-brand-red-100' : 'border-slate-200'
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="rounded-xl bg-slate-950 p-2 text-white"><BookOpen className="h-5 w-5" /></div>
          ...
          {selected ? (
            <div className="mt-4 space-y-4">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">{selected.modules} modules · {selected.lessons} lessons</p>
              </div>
              <label className="block text-xs font-bold text-slate-700">
                Program name
                <input value={programName} onChange={(event) => setProgramName(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal" />
              </label>
              <label className="block text-xs font-bold text-slate-700">
                Program code
                <input value={programCode} onChange={(event) => setProgramCode(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-mono font-normal" />
              </label>
              <label className="block text-xs font-bold text-slate-700">
                Duration in weeks
                <input type="number" min={1} value={durationWeeks} onChange={(event) => setDurationWeeks(event.target.value)} placeholder="Optional" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal" />
              </label>
              <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                <input type="checkbox" checked={fundingEligible} onChange={(event) => setFundingEligible(event.target.checked)} />
                Funding eligible
              </label>
              <button
                type="button"
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">Select a blueprint to create the program and send the complete build through the unified Course Factory.</p>
          )}
