...
- **pnpm in Docker:** Store dir is **`/app/.pnpm-store`** (same filesystem as `/app/node_modules` for hardlinks). Never use `--package-import-method=copy` in Northflank Dockerfiles — it duplicates the full store into `node_modules` and triggers ENOSPC even when the registry cache mount works. Flow: `pnpm fetch` after copying workspace manifests, `COPY . .`, then `pnpm install --offline`.
...
- **Admin (Northflank):** service `elevate-admin`, Dockerfile `Dockerfile.northflank-admin`, branch `main`, readiness path `/api/ping`. Same pnpm fetch/offline flow as LMS (no `--package-import-method=copy`). After `next build`, **`node scripts/prune-admin-standalone.mjs`** drops playwright/puppeteer/three/etc. from `apps/admin/.next/standalone` but **keeps Remotion** for `/api/admin/generate-lesson-videos`. LMS uses `scripts/prune-standalone.mjs` via root `postbuild` and excludes Remotion via `outputFileTracingExcludes` (`scripts/next-standalone-trace-excludes.mjs`). Audit sizes: `node scripts/audit-admin-standalone-size.mjs` (fails if playwright/puppeteer remain).
