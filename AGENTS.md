...
- **Production hosting is Northflank only** (`elevate-lms`, `elevate-admin`) — not AWS ECS. Sync secrets from Supabase `platform_secrets` (column `value_enc`): `NORTHFLANK_PROJECT_ID=elevate-platform pnpm tsx scripts/northflank/sync-supabase-secrets.ts --execute`, then set runtime `SUPABASE_ANON_KEY` via Northflank API or `sync-admin-from-db.ts --execute`.
- Northflank Dockerfiles use `build-placeholder` for `next build` only. At **runtime**, server auth reads **`SUPABASE_ANON_KEY` / `SUPABASE_URL`** (not inlined `NEXT_PUBLIC_*`). URL must be `https://<ref>.supabase.co`, not `db.<ref>.supabase.co`.
