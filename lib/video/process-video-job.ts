...
import { generateGpuVideo, gpuVideoAvailable } from './gpu-video-client';
...
async function hydrateMediaRuntimeSecrets(): Promise<void> {
  const missing = ['ELEVENLABS_API_KEY', 'GEMINI_API_KEY', 'OPENAI_API_KEY', 'PEXELS_API_KEY'].filter((key) => !process.env[key]?.trim());
  ...
  for (const key of missing) {
    try {
      ...
      logger.warn('[video-worker] Unable to hydrate optional media secret', { key, error: error instanceof Error ? error.message : String(error) });
    }
  }
}

function safeAssetKey(value: string): string { return value.replace(/[^a-zA-Z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80) || 'clip'; }
...
export async function processClaimedVideoJob(job: VideoJob): Promise<void> {
  ...
  try {
    ...
    // never make learner-critical media dependent on GPU capacity.
    if (isMicroclip && (await gpuVideoAvailable())) {
      ...
      try {
        const generated = await generateGpuVideo({ prompt: visualPrompt, durationSeconds: Math.min(15, Math.max(3, Number(sceneData.duration_seconds) || 5)) });
        if (generated?.videoUrl) {
          await markComplete(job.id, { video_url: generated.videoUrl, duration_seconds: generated.durationSeconds });
          return;
        }
      } catch (gpuError) {
        logger.warn('[video-worker] GPU scene failed; falling back to Remotion', { jobId: job.id, error: gpuError instanceof Error ? gpuError.message : String(gpuError) });
      }
    }
    ...
    const result = await renderLessonVideo({
      ...
      keyPoints: bulletPoints.length ? bulletPoints.slice(0, 5) : script.split(/\.\s+/).filter(Boolean).slice(0, 5),
      ...
      quizTeaser: isMicroclip ? 'Return to the lesson and apply this concept.' : 'Complete the knowledge check and review any missed objectives to continue.',
      domainKey: inferDomainKey(courseTitle, job.lesson_title), instructorId: instructor.id, courseName: courseTitle,
      visualPrompt: typeof sceneData.visual_prompt === 'string' ? sceneData.visual_prompt : undefined,
    });
    if (!result.success || !result.videoUrl) { await markFailed(job.id, result.error ?? 'Render returned no playable video URL'); return; }
    await markComplete(job.id, { video_url: result.videoUrl, audio_url: result.audioUrl ?? undefined, duration_seconds: result.duration });
  } catch (error) {
