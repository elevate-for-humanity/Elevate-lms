...
const SLA_HOURS: Record<string, number> = { submitted: 48, in_review: 72, approved: 24 };
...
export async function GET(request: NextRequest) {
  ...
  for (const row of rows ?? []) {
    ...
    const hoursInStatus = (now.getTime() - since) / 3_600_000;
    if (SLA_HOURS[status] !== undefined) {
      ...
      if (hoursInStatus > SLA_HOURS[status]) {
        stuck.push({ id: row.id, status, program_slug: row.program_slug, hours_in_status: Math.round(hoursInStatus), sla_hours: SLA_HOURS[status] });
      }
    }
  }
  ...
  if (lastRun?.ran_at) {
    ...
    if (hoursSinceLast > DEADMAN_INTERVAL_HOURS) {
      ...
      if (alertTo) await trySendEmail({ to: alertTo, subject: `[ALERT] Enrollment health check gap — ${hoursSinceLast.toFixed(1)}h since last run`, html: `<p>${msg}</p><p>If this was intentional downtime, no action needed. Otherwise investigate why the health route stopped running.</p>` });
      if (slackWebhook) await fetch(slackWebhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: `⚠️ *HEALTH_CHECK_GAP* — ${msg}` }) }).catch(() => {});
    }
  }
  ...
  if (integrity.failures.includes('PRIVILEGED_BYPASS_DETECTED')) {
    ...
    if (alertTo) {
      await trySendEmail({
        ...
        html: `<p><strong>PRIVILEGED_BYPASS_DETECTED</strong></p><p>${bypassCount} enrollment row(s) were written to <code>program_enrollments</code> outside the <code>enroll_application</code> RPC.</p><p>Check <code>enrollment_insert_audit</code> where <code>via_rpc = false</code>. Treat this as a security incident unless it was an approved maintenance write.</p>`,
      });
    }
    if (slackWebhook) await fetch(slackWebhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: `🚨 *PRIVILEGED_BYPASS_DETECTED* — ${bypassCount} enrollment row(s) written outside the RPC.` }) }).catch(() => {});
  }
  ...
  return NextResponse.json({
    ...
    top_bottlenecks,
    stuck,
    enrollment_integrity: { clean: integrity.clean, failures: integrity.failures, counts: integrity.counts },
    deadman: { last_run_at: lastRun?.ran_at ?? null, interval_threshold_hours: DEADMAN_INTERVAL_HOURS },
  });
}
