...
export const metadata: Metadata = {
  ...
  description:
    'DOL Registered Apprenticeship Sponsor. Indiana ETPL Certified. WIOA compliant. WorkOne referrals accepted. RAPIDS-tracked outcomes. Built for workforce boards, DWD, and case managers.',
...
const complianceFeatures = [
  ...
  },
  {
    ...
    href: '/funding/how-it-works#wioa',
    cta: 'WIOA Details',
  },
  {
    ...
    <div className="bg-white">
      <div className="bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <Breadcrumbs
            items={[{ label: 'Funding', href: '/funding' }, { label: 'Partner Agencies' }]}
          />
        </div>
      </div>
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            href="/contact"
            className="border-2 border-slate-300 text-slate-800 font-bold px-6 py-3 rounded-lg text-sm hover:bg-slate-50 transition-colors"
          >
            ...
            {[
              {
                ...
                desc: 'Case manager determines WIOA eligibility and selects an ETPL-listed Elevate program.',
              },
              {
                step: '2',
                title: 'Funding Authorization',
                desc: 'ITA or grant authorization issued. Training cost is covered before the participant begins.',
              },
              {
                step: '3',
                title: 'Training & Tracking',
                desc: 'Participant completes checkpoint-gated training. Attendance, RTI hours, and progress are logged in real time.',
              },
              {
                step: '4',
                title: 'Credential & Placement',
                desc: 'Exam funding authorized on completion. Credential issued on-site. Placement outcome reported to DWD.',
              },
            ].map((s, i) => (
              <div key={s.step} className="relative">
                <div className="w-10 h-10 rounded-full bg-blue-600 text-white font-black text-base flex items-center justify-center mb-3">
                  {s.step}
                </div>
                {i < 3 && (
                  <div className="hidden sm:block absolute top-5 left-10 right-[-1rem] h-px bg-blue-200" />
                )}
        ...
        {/* IMAGE-CONTRACT: placeholder-review required (blurDataURL or approved fallback) */}
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="mt-4">
                    <Link
                      href={feature.href}
                      className="inline-block bg-brand-red-600 hover:bg-brand-red-700 text-white text-sm font-bold px-5 py-2.5 rounded-lg transition-colors"
                    >
                      {feature.cta}
                    </Link>
                  </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-slate-600 mb-8">
                Designed for workforce boards, state agencies, and regional partnerships. Deploy
                across multiple jurisdictions with centralized oversight and reporting.
              </p>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {governanceFeatures.map((feature, index) => (
              <div
                key={index}
                className="flex items-start gap-3 bg-slate-50 rounded-lg p-5 border border-slate-100"
              >
      ...
      <section className="py-16 bg-slate-900">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/store/demos"
              className="inline-flex items-center gap-2 bg-brand-red-600 hover:bg-brand-red-700 text-white px-8 py-4 rounded-lg font-bold transition"
            >
