...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const suggestedFixes = failed
      ...
      .map((r) => ({ step: r.id, playbook: AUTOFIX_MAP[r.id], endpoint: '/api/admin/dev-studio/autofix' }));
      ...
      nextActions:
        failed.length === 0
          ...
          : [
              ...
              'Re-run /api/admin/dev-studio/stabilize after fixes',
            ],
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    endpoint: '/api/admin/dev-studio/stabilize',
    method: 'POST',
    body: {
      quick: 'boolean (default true)',
      includeTests: 'boolean (default true)',
