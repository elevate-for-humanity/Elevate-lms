...
import { MessageSquare, Send, ThumbsUp, Clock, User, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function GroupDiscussions({
  ...
  async function fetchPosts() {
    ...
    let query = supabase
      ...
      .select(`id, content, created_at, likes_count, comments_count, tags, profiles:user_id ( full_name, avatar_url )`)
  ...
  async function handleLike(post: Post) {
    ...
    if (post.liked_by_me) {
      const { error: err } = await supabase
        ...
        .eq('post_id', post.id)
        .eq('user_id', userId);
      if (err) return setError('Could not update your reaction.');
  ...
  return (
    <div className="space-y-4">
      {userId ? (
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Sign in to participate in the community.</div>
      )}
              ...
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    ...
                    <span className="flex items-center gap-1 text-xs text-slate-400"><Clock className="h-3 w-3" />{timeAgo(post.created_at)}</span>
                  </div>
                  <p className="whitespace-pre-wrap break-words text-sm text-slate-700">{post.content}</p>
                  {post.tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {post.tags.map((tag) => <span key={tag} className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">#{tag}</span>)}
                    </div>
                  )}
                  <div className="mt-3 flex items-center gap-4 border-t border-slate-100 pt-2">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 text-xs font-medium transition ${post.liked_by_me ? 'text-brand-blue-600' : 'text-slate-500 hover:text-brand-blue-600'}`}
                    >
                      <ThumbsUp className="h-3.5 w-3.5" /> {post.likes_count} {post.likes_count === 1 ? 'Like' : 'Likes'}
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <MessageSquare className="h-3.5 w-3.5" /> {post.comments_count} {post.comments_count === 1 ? 'Comment' : 'Comments'}
                      {openComments === post.id ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                    </button>
                  </div>
                    ...
                    <div className="mt-3 space-y-3 rounded-xl bg-slate-50 p-3">
                      {(comments[post.id] ?? []).map((comment) => (
                        <div key={comment.id} className="flex gap-2">
                          <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-200">
                            {comment.author_avatar ? <img src={comment.author_avatar} alt="" className="h-full w-full object-cover" /> : <User className="h-3.5 w-3.5 text-slate-400" />}
                          </div>
                          <div className="min-w-0 rounded-lg bg-white px-3 py-2">
                            <div className="flex items-center gap-2"><span className="text-xs font-bold text-slate-900">{comment.author_name}</span><span className="text-[11px] text-slate-400">{timeAgo(comment.created_at)}</span></div>
                      ...
                      {userId && (
                        <div className="flex gap-2">
                          ...
                          />
                          <button type="button" onClick={() => submitComment(post.id)} disabled={!(commentDrafts[post.id] ?? '').trim()} className="rounded-lg bg-brand-blue-600 px-3 py-2 text-white disabled:opacity-40" aria-label="Post comment"><Send className="h-4 w-4" /></button>
                        </div>
                      )}
                    </div>
                  )}
