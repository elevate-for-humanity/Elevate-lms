    ...
    <div className="max-w-7xl mx-auto py-8 space-y-8 px-4 sm:px-6 lg:px-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Broadcast push notifications and view delivery history
        </p>
      </div>
              ...
              {logs.map((log) => {
                ...
                return (
                  <div key={log.id} className="px-5 py-3.5 flex items-start gap-3">
                    <span
                      className={`mt-0.5 inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold flex-shrink-0 ${STATUS_STYLES[log.status] ?? 'bg-slate-100 text-slate-600'}`}
                    >
