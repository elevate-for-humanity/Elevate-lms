...
export default function WebsiteEditorPage() {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      secondaryCtaLabel: form.get("secondaryCtaLabel"),
      secondaryCtaHref: form.get("secondaryCtaHref"),
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-5 py-8">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-6">
          <h1 className="text-4xl font-black text-slate-950">Website Editor</h1>
          <p className="mt-3 text-lg text-slate-600">Update page titles, hero banners, descriptions, images, and calls to action.</p>
        </div>
        ...
        <form onSubmit={save} className="mt-8 space-y-6 rounded-3xl bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <Field name="primaryCtaLabel" label="Primary CTA label" placeholder="Apply Now" />
            <Field name="primaryCtaHref" label="Primary CTA link" placeholder="/apply?program=medical-assistant" />
            <Field name="secondaryCtaLabel" label="Secondary CTA label" placeholder="Request Information" />
            <Field name="secondaryCtaHref" label="Secondary CTA link" placeholder="/contact" />
          </div>

          <Field name="heroImage" label="Hero image path" placeholder="/images/programs/medical-assistant-hero.webp" />

          <label className="block">
            <span className="mb-2 block font-bold text-slate-800">Status</span>
            <select name="status" defaultValue="draft" className="min-h-12 w-full rounded-xl border border-slate-300 px-4">
          ...
          {message && (
            <div className={status === "error" ? "rounded-xl bg-red-100 p-4 text-red-900" : "rounded-xl bg-emerald-100 p-4 text-emerald-900"}>
              {message}
            </div>
          )}

          <button type="submit" disabled={status === "saving"} className="min-h-12 rounded-xl bg-blue-600 px-6 py-3 font-bold text-white disabled:opacity-60">
...
function Field({ name, label, placeholder, required = false }: { name: string; label: string; placeholder: string; required?: boolean }) {
  return (
    <label className="block">
      ...
      <input name={name} required={required} placeholder={placeholder} className="min-h-12 w-full rounded-xl border border-slate-300 px-4" />
    </label>
  );
}
...
function TextArea({ name, label, placeholder, required = false }: { name: string; label: string; placeholder: string; required?: boolean }) {
  return (
    <label className="block">
      <span className="mb-2 block font-bold text-slate-800">{label}</span>
      <textarea name={name} required={required} placeholder={placeholder} rows={5} className="w-full rounded-xl border border-slate-300 px-4 py-3" />
    </label>
  );
}
