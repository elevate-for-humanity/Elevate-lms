...
const ExperienceSchema = z.object({
  ...
  caseStudy: z.any().optional(),
  simulation: z.any().optional(),
  decisionTree: z.any().optional(),
  practicalTask: z.any().optional(),
  interactiveVideo: z.any().optional(),
});
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await db.from('course_lessons').select('id,title,slug,content_json,practical_required,requires_instructor_signoff,competency_checks').eq('id', lessonId).maybeSingle();
    if (error) throw error;
    if (!data) return safeError('Lesson not found', 404);
    const contentJson = data.content_json && typeof data.content_json === 'object' ? data.content_json as Record<string, unknown> : {};
    return NextResponse.json({ ok: true, lesson: data, experience: contentJson.experience ?? {} });
  } catch (error) {
...
export async function PATCH(req: NextRequest) {
  ...
  const parsed = ExperienceSchema.safeParse(body.experience ?? {});
  if (!parsed.success) return safeError('Invalid lesson experience', 400);
  try {
    const db = await requireAdminClient();
    const { data: existing, error: loadError } = await db.from('course_lessons').select('content_json').eq('id', body.lessonId).maybeSingle();
    if (loadError) throw loadError;
    if (!existing) return safeError('Lesson not found', 404);
    const current = existing.content_json && typeof existing.content_json === 'object' ? existing.content_json as Record<string, unknown> : {};
