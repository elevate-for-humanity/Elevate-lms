...
export default async function HostShopDashboardView() {
  ...
  const partnerApproved = board.partner?.approval_status === 'approved' && board.partner?.status === 'active';
  ...
  const summaryCards: PortalCard[] = [
    ...
    { href: '/host-shop/dashboard/documents', title: 'Required documents', value: `${board.acceptedDocumentCount}/${board.requiredDocumentCount}`, detail: board.documentsComplete ? 'All required compliance documents are accepted.' : `${board.missingDocuments.length} missing and ${board.pendingDocuments.length} awaiting review.`, image: '/images/pages/comp-home-highlight-success.webp' },
  ];
  ...
  const operatingSteps = [
    { href: '/host-shop/orientation', title: '1. Complete Host Shop orientation', detail: 'Review responsibilities, payroll, safety, geofencing, work approvals, competencies, and required records.' },
    { href: '/host-shop/dashboard/apprentices', title: '2. Confirm each apprentice', detail: 'Verify the apprentice, active placement, supervisor, occupation, start date, and contact information.' },
    { href: '/host-shop/dashboard/schedule', title: '3. Set the training schedule', detail: 'Keep the apprentice schedule and supervised training sessions current.' },
    { href: '/host-shop/dashboard/attendance/record', title: '4. Record attendance and work', detail: 'Make sure apprentices clock at the approved site and that supervised work is documented.' },
    { href: '/host-shop/dashboard/hours/pending', title: '5. Review hours every week', detail: 'Approve accurate OJL entries promptly; return incorrect entries with a clear correction note.' },
    { href: '/host-shop/dashboard/competencies', title: '6. Verify competencies', detail: 'Record observed skills only after the apprentice demonstrates them under qualified supervision.' },
    { href: '/host-shop/dashboard/reports', title: '7. Check compliance monthly', detail: 'Review wages, RTI, OJL, attendance, documents, and exceptions before reporting deadlines.' },
  ] as const;
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
    <section className="mb-5 flex flex-col gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-4 sm:flex-row sm:items-center sm:justify-between">
      <div><p className="font-black text-slate-950">Host Shop app</p><p className="mt-1 text-sm font-medium text-slate-700">Install this exact Host Shop dashboard for direct access to apprentices, hours, attendance, competencies, and reports.</p></div>
      <PwaInstallButton label="Install Host Shop App" installedLabel="Host Shop App Installed" className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-blue-800 px-5 py-3 text-sm font-black text-white hover:bg-blue-900" />
    </section>
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="grid lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch"><div className="p-6 sm:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-brand-blue-700">Host Shop</p><h1 className="mt-2 text-3xl font-black text-slate-950">{board.partner?.name || board.shops[0]?.name || 'Host Shop'}</h1><p className="mt-2 font-medium text-slate-700">Shared registered-apprenticeship workspace</p>{board.registeredPrograms.length ? <div className="mt-3 flex flex-wrap gap-2">{board.registeredPrograms.map((program) => <span key={program.rapidsCode} className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-900">{program.label} · RAPIDS {program.rapidsCode}</span>)}</div> : null}{board.shops[0]?.city ? <p className="mt-2 text-sm font-medium text-slate-700">{board.shops[0].city}, {board.shops[0].state}</p> : null}</div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs font-bold ${partnerVerified ? 'bg-green-100 text-green-900' : 'bg-amber-100 text-amber-950'}`}>{partnerVerified ? 'Verified Partner' : 'Verification Pending'}</span><span className={`rounded-full px-3 py-1 text-xs font-bold ${partnerMouSigned ? 'bg-green-100 text-green-900' : 'bg-amber-100 text-amber-950'}`}>{partnerMouSigned ? 'MOU Signed' : 'MOU Pending'}</span>{isPlatformAdmin ? <form action={clearAdminPartner}><button className="rounded-full border border-slate-300 bg-white px-3 py-1 text-xs font-bold">Change Host Shop</button></form> : null}</div></div>{isPlatformAdmin && (!partnerVerified || !partnerMouSigned || !partnerOnboardingComplete) ? <div role="alert" className="mt-5 rounded-xl border border-blue-300 bg-blue-50 p-4 text-sm font-semibold leading-6 text-blue-950">Admin review mode: this Host Shop record is accessible for audit and setup, but owner access remains gated until verification, MOU, and onboarding requirements are complete.</div> : null}{board.unconfiguredPrograms.length ? <div role="alert" className="mt-5 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm font-semibold leading-6 text-amber-950">Regulated progress is blocked for: {board.unconfiguredPrograms.map((program) => program.programSlug || 'unknown occupation').join(', ')}. An active approved registered-program standard must be configured before competencies, RTI, or regulated OJL are credited.</div> : null}</div><div className="relative min-h-[240px]"><Image src={heroImage} alt={`${partnerName || 'Host Shop'} supervised apprenticeship workspace`} fill priority className="object-cover" sizes="(max-width: 1024px) 100vw, 42vw" /></div></div></section>
    {!board.documentsComplete ? <section role="alert" className="mt-6 rounded-2xl border border-red-300 bg-red-50 p-5 text-red-950"><h2 className="text-lg font-black">Required compliance documents are incomplete</h2><p className="mt-1 text-sm font-semibold">Upload or replace every item below before the Host Shop is considered fully ready.</p><ul className="mt-3 grid gap-2 sm:grid-cols-2">{board.missingDocuments.map((item: any) => <li key={item.document_type} className="rounded-xl border border-red-200 bg-white px-4 py-3 text-sm font-bold">Missing: {item.document_name}</li>)}{board.pendingDocuments.map((item: any) => <li key={item.document_type} className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-950">In review: {item.document_name}</li>)}</ul><Link href="/host-shop/onboarding/documents" className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-red-700 px-5 py-3 font-black text-white hover:bg-red-800">Complete required documents</Link></section> : null}
    ...
    <section className="mt-8 rounded-3xl border border-cyan-200 bg-cyan-50 p-6 sm:p-8" aria-labelledby="host-shop-operating-guide">
      <p className="text-xs font-black uppercase tracking-[0.14em] text-cyan-800">Start here and repeat weekly</p>
      <h2 id="host-shop-operating-guide" className="mt-2 text-2xl font-black text-slate-950">Run your apprenticeship step by step</h2>
      <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-slate-700">Use this checklist for every apprentice. Each step opens the same verified shop, placement, geofence, hours, competency, and compliance records used by the program.</p>
      <ol className="mt-5 grid gap-3 md:grid-cols-2">
        {operatingSteps.map((step) => <li key={step.title}><Link href={step.href} className="block h-full rounded-2xl border border-cyan-200 bg-white p-4 transition hover:border-brand-blue-400 hover:shadow-sm"><h3 className="font-black text-slate-950">{step.title}</h3><p className="mt-1 text-sm font-medium leading-6 text-slate-700">{step.detail}</p></Link></li>)}
      </ol>
    </section>
