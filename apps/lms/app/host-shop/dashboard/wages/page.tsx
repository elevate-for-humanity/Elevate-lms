...
export default async function HostShopWagesPage({ searchParams }: { searchParams?: Promise<{ error?: string; verified?: string }> }) {
  ...
  const programSlug = board.apprentices.find((a) => a.program_slug)?.program_slug || 'barber-apprenticeship';
  const contract = await resolveRegisteredProgramContract(db, { programSlug, partnerId: board.partner.id });
  const standard = contract?.standard || null;
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-black uppercase tracking-[0.14em] text-brand-blue-700">{board.partner?.name || 'Host Shop'}</p><h1 className="mt-2 text-3xl font-black text-slate-950">Progressive Wage Compliance</h1><p className="mt-2 max-w-3xl text-slate-600">Verify actual apprentice pay against the registered occupation baseline and this employer&apos;s RAPIDS wage schedule when one is recorded. The higher applicable legal or registered floor controls.</p></div>
      ...
      <section className="mt-6 rounded-2xl border border-cyan-200 bg-cyan-50 p-5 text-cyan-950">
        <div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0"/><div><h2 className="font-black">Registered wage contract</h2>{standard ? <><p className="mt-1 text-sm font-semibold">Occupation {standard.occupationTitle} ({standard.rapidsCode}) · baseline entry ${standard.startingHourlyRate.toFixed(2)}/hr. Competency milestones are read from the approved standard.</p>{employerSchedule ? <p className="mt-1 text-sm font-semibold">Employer RAPIDS schedule: start ${Number(employerSchedule.startingHourlyRate || 0).toFixed(2)}/hr · end ${Number(employerSchedule.endingHourlyRate || 0).toFixed(2)}/hr · journeyworker ${Number(employerSchedule.journeyworkerHourlyRate || 0).toFixed(2)}/hr.</p> : <p className="mt-1 text-sm font-semibold">No employer-specific RAPIDS wage schedule is stored yet; occupation baseline and other applicable wage law remain in force.</p>}</> : <p className="mt-1 text-sm font-semibold">No registered-program contract was found for the current placement. Wage verification is limited to existing recorded obligations.</p>}</div></div>
      </section>

      <div className="mt-6 grid gap-4 sm:grid-cols-3"><Metric label="Pending wage actions" value={pending.length} /><Metric label="Verified wage actions" value={verified.length} /><Metric label="Required competencies" value={standard?.competencyCount || 0} /></div>
      ...
      <section className="mt-6 space-y-4"><h2 className="text-xl font-black text-slate-950">Actions requiring verification</h2>{pending.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white p-6 text-slate-600">No pending wage milestones for apprentices assigned to this Host Shop.</div> : pending.map((row: any) => {
        const apprentice = apprenticeByPlacement.get(row.placement_id); const floor = Number(row.required_hourly_rate || row.appendix_hourly_rate || 0);
