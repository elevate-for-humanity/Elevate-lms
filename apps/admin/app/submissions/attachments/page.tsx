...
const TYPE_LABELS: Record<string, string> = {
  ...
  general_liability_cert: 'GL Certificate',
  workers_comp_cert: "Workers' Comp",
  audit_report: 'Audit Report',
  board_list: 'Board List',
  staff_resume: 'Staff Resume',
  uei_letter: 'UEI Letter',
  sam_proof: 'SAM Proof',
  capability_statement: 'Capability Statement',
  past_performance_summary: 'Past Performance',
  financial_statement: 'Financial Statement',
  tax_return: 'Tax Return',
  nonprofit_determination: '501(c)(3) Letter',
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm">
                Pre-approved files ready to include in any submission packet
              </p>
            </div>
          </div>
          <Link
        ...
        {error && (
          <div className="mb-5 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
            ...
            <p className="text-sm text-amber-800">
              Attachment Library tables not yet applied. Run migrations in Supabase Dashboard.
            </p>
          </div>
        )}
        ...
        {expiringSoon > 0 && (
          <div className="mb-5 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
            ...
            <p className="text-sm text-red-800 font-semibold">
              {expiringSoon} attachment{expiringSoon > 1 ? 's' : ''} expiring within 30 days — renew
              before including in packets.
            </p>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-black text-slate-900">
                {(attachments ?? []).length - approved}
              </p>
              <p className="text-xs text-slate-500">Pending / Other</p>
            </div>
          </div>
        </div>
                  ...
                  return (
                    <tr key={att.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-4 py-3 text-slate-500 text-xs">
                        {TYPE_LABELS[att.document_type] ?? att.document_type}
                      </td>
                      ...
                      <td className="px-4 py-3 text-xs">
                        {att.expiration_date ? (
                          <span
                            className={expiring ? 'text-red-600 font-semibold' : 'text-slate-500'}
                          >
                            {expiring && '⚠️ '}
                            {new Date(att.expiration_date).toLocaleDateString()}
                          </span>
                        ) : (
