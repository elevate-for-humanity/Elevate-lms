...
export default async function ParentDashboardPage() {
  ...
  const [{ data: profiles }, { data: enrollments }] = await Promise.all([
    studentIds.length
      ? access.supabase.from('profiles').select('id, full_name, email').in('id', studentIds)
      : Promise.resolve({ data: [] }),
    studentIds.length
      ? access.supabase
          .from('program_enrollments')
          .select(
            'id, user_id, status, enrollment_state, progress_percent, enrolled_at, programs(title, slug)',
          )
    ...
    <main className="min-h-screen w-full bg-slate-50 px-4 py-8">
      <div className="w-full max-w-none space-y-7">
        <section className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
          ...
          <h1 className="mt-2 text-4xl font-black text-slate-950">Welcome, {firstName}</h1>
          <p className="mt-2 text-slate-600">
            View verified student relationships, program enrollment, and training progress.
          </p>
        </section>
        ...
        {students.length ? (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 p-5">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 rounded-lg bg-blue-700 px-4 py-2.5 text-sm font-bold text-white"
                  >
                ...
                <div className="divide-y divide-slate-100">
                  {student.enrollments.length ? (
                          ...
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              ...
                              <p className="text-xs text-slate-600">
                                {enrollment.enrollment_state || enrollment.status || 'unknown'}
                              </p>
                            </div>
                    ...
                    <p className="p-5 text-sm text-slate-600">No program enrollments found.</p>
                  )}
                </div>
              </article>
            ))}
          ...
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center">
            ...
            <h2 className="mt-3 text-xl font-black text-amber-950">No verified student links</h2>
            <p className="mt-2 text-sm text-amber-900">
              A coordinator must verify the parent/student relationship before student records are
              visible.
            </p>
          </section>
        )}

        <section className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5">
          <div>
            <p className="font-black">Need help?</p>
            <p className="text-sm text-slate-600">Contact the program coordinator.</p>
          </div>
...
async function ParentAdminOversight() {
  ...
  const [{ count: verifiedLinks }, { count: pendingLinks }, { count: linkedStudents }] =
    await Promise.all([
      db
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="mx-auto max-w-5xl">
        <p className="text-sm font-bold uppercase tracking-[0.14em] text-blue-700">
          Parent Portal · Admin Oversight
        </p>
        <h1 className="mt-2 text-4xl font-black">Guardian access oversight</h1>
        <p className="mt-2 max-w-3xl text-slate-600">
          Admin may enter the portal without being assigned a fake parent relationship. Student
          detail still requires an explicit student selection.
        </p>
        <div className="mt-7 grid gap-4 sm:grid-cols-3">
          <Metric label="Verified relationships" value={verifiedLinks ?? 0} />
          <Metric label="Pending verification" value={pendingLinks ?? 0} />
          <Metric label="Relationship records" value={linkedStudents ?? 0} />
        </div>
        <div className="mt-7 rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-black">Support workflow</h2>
          <p className="mt-2 text-sm text-slate-600">
            Use Admin student search to select a student, then open that student through the Parent
            Portal detail route for support verification.
          </p>
          <a
