  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="border-b border-slate-800 bg-slate-900 px-5 py-4">
        <div className="mx-auto flex max-w-[1600px] flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            <button onClick={() => void loadCourses()} className="rounded-lg border border-slate-700 p-2 hover:bg-slate-800" title="Refresh courses"><RefreshCw className="h-4 w-4" /></button>
            {courseId && (
      ...
      <main className="mx-auto max-w-[1600px] p-4">
        {tab === 'courses' && <CourseCatalog courses={courses} onCreated={async (id) => { await loadCourses(); setCourseId(id); }} />}
...
function CourseCatalog({ courses, onCreated }: { courses: CourseRow[]; onCreated: (id: string) => void | Promise<void> }) {
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        <h2 className="text-lg font-bold">Course applications</h2>
        <p className="mt-1 text-sm text-slate-400">Every course opens the same session, state provider, mutation layer and feature workspace.</p>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {courses.map((course) => (
            <Link key={course.id} href={`/studio/courses/${course.id}`} className="rounded-xl border border-slate-700 bg-slate-950 p-4 hover:border-cyan-500">
              <div className="font-bold text-white">{course.title}</div>
              <div className="mt-1 text-xs text-slate-400">{course.status ?? 'draft'} · {course.duration_hours ?? '—'} hours</div>
