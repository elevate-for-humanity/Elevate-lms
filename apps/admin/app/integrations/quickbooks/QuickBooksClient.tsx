...
export default function QuickBooksClient() {
  ...
  async function checkStatus() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResult(data.error || 'QuickBooks status could not be checked.');
      }
    } catch {
      setResult('QuickBooks status could not be checked.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function getAuthUrl() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResult(d.error || 'QuickBooks authorization could not be started.');
      }
    } catch {
      setResult('QuickBooks authorization could not be started.');
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-3 mt-6 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Verify accounting data and employee records with QuickBooks Online
          </p>
        </div>
      </div>
      ...
      {/* Status card */}
      <div
        className={`rounded-2xl border p-6 mb-6 ${status.connected ? 'border-green-200 bg-green-50' : 'border-slate-200 bg-white'}`}
      >
          ...
          <div className="space-y-3">
            <p className="text-sm text-slate-600">
              {status.credentials_configured
                ? 'The Intuit application is configured. Authorize the Elevate for Humanity company to finish connecting accounting data.'
                : 'Add the Intuit application credentials before authorizing a company.'}{' '}
              Payroll-run sync requires separate Intuit Payroll API access.
            </p>
            {!authUrl ? (
              <button
                onClick={getAuthUrl}
            ...
            <p className="text-xs text-slate-400">
              {status.credentials_configured
                ? 'Application credentials detected. Company authorization is still required.'
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500">
                Pull employee records through QuickBooks Online Accounting
              </p>
            </div>
            {syncing === 'sync_payroll' && (
              <Loader2 className="w-4 h-4 animate-spin text-slate-400" />
            )}
          </button>

          <button
            onClick={() => sync('sync_expenses')}
            disabled={!!syncing}
            className="w-full flex items-center gap-3 p-4 border border-slate-200 rounded-xl hover:bg-slate-50 disabled:opacity-50 transition-colors text-left"
          >
      ...
      <div className="mt-8 p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-500 space-y-1">
        ...
        <p>
          <code className="font-mono">QB_CLIENT_ID</code> — Intuit app client ID
        </p>
        <p>
          <code className="font-mono">QB_CLIENT_SECRET</code> — Intuit app client secret
        </p>
        <p>
          <code className="font-mono">QB_REDIRECT_URI</code> — OAuth callback URL
        </p>
        <p>
          <code className="font-mono">QB_ACCESS_TOKEN</code> — set after OAuth (auto)
        </p>
        <p>
          <code className="font-mono">QB_REFRESH_TOKEN</code> — set after OAuth (auto)
        </p>
        <p>
          <code className="font-mono">QB_REALM_ID</code> — company ID (set after OAuth)
        </p>
      </div>
    </div>
  );
