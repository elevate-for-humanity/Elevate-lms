      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          <nav className="text-sm mb-4">
            <ol className="flex items-center space-x-2 text-black">
              <li>
                <Link href="/workforce-board" className="hover:text-primary">
                  Workforce Board
                </Link>
              </li>
              <li>/</li>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            <p className="text-3xl font-bold text-black mt-2">{count ?? 0}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <h3 className="text-sm font-medium text-black">Active</h3>
            <p className="text-3xl font-bold text-black mt-2">{activeCount ?? 0}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <h3 className="text-sm font-medium text-black">Participants</h3>
            <p className="text-3xl font-bold text-black mt-2">{participantCount ?? 0}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border p-6">
                ...
                <div key={prog.id} className="p-4 flex items-center justify-between hover:bg-white">
                  <div>
                    ...
                    <p className="text-sm text-black">
                      {prog.provider || 'Provider'} • {prog.duration || 'Duration N/A'}
                    </p>
                  </div>
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${prog.status === 'active' ? 'bg-brand-green-100 text-brand-green-800' : 'bg-white text-black'}`}
                  >
                    {prog.status || 'draft'}
                  </span>
                </div>
