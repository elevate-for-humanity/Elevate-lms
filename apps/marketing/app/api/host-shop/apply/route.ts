...
export async function POST(request: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            'Business name, contact name, email, phone, city, and at least one program are required.',
        },
        { status: 400 },
      );
    }
    ...
    if (!licenseNumber || !supervisorName || !supervisorLicenseNumber) {
      return NextResponse.json(
        {
          ok: false,
          error:
            'Business license number, supervising professional name, and supervisor license number are required for host-shop review.',
        },
        { status: 400 },
      );
    }
    if (hasInsurance === 'yes' && (!body.insuranceFileData || !body.insuranceFileName)) {
      return NextResponse.json(
        { ok: false, error: 'Upload the current commercial liability insurance COI.' },
        { status: 400 },
      );
    }
    ...
    const licensePath = await uploadApplicationDocument(
      ...
      email,
      String(body.shopLicenseFileData),
      String(body.shopLicenseFileName),
    );
    if (!licensePath) {
      return NextResponse.json(
        { ok: false, error: 'The shop license file could not be uploaded. Please try again.' },
        { status: 500 },
      );
    }

    const insurancePath = body.insuranceFileData
      ? await uploadApplicationDocument(
          admin,
          'host-shop-insurance-coi-documents',
          email,
          String(body.insuranceFileData),
          String(body.insuranceFileName || 'insurance-coi.pdf'),
        )
      : null;

    if (hasInsurance === 'yes' && !insurancePath) {
      return NextResponse.json(
        { ok: false, error: 'The insurance COI file could not be uploaded. Please try again.' },
        { status: 500 },
      );
    }
    ...
    const notes = [
      ...
      `Liability insurance status: ${hasInsurance || 'not answered'}`,
      workersCompStatus ? `Workers comp status: ${workersCompStatus}` : '',
      compensationModel ? `Apprentice compensation model: ${compensationModel}` : '',
      `Supervisor: ${supervisorName}`,
      `Supervisor license: ${supervisorLicenseNumber}`,
      supervisorYearsLicensed ? `Supervisor years licensed: ${supervisorYearsLicensed}` : '',
      canSuperviseAndVerify ? `Can supervise and verify OJL/competencies: ${canSuperviseAndVerify}` : '',
      ...
      insurancePath ? `Insurance COI document: ${insurancePath}` : 'Insurance COI: not uploaded',
    ...
    const applicantEmail = sendEmail({
      ...
      html: `<p>Hello ${contactName},</p><p>We received the host shop application for <strong>${businessName}</strong>.</p><p>Reference: <strong>${data.id}</strong></p><p>Our team will verify the business license, supervisor credentials, insurance, worksite capacity, and program fit before approval.</p>`,
    });
    const adminEmail = sendEmail({
      ...
      html: `<p>New host shop application.</p><p><strong>${businessName}</strong><br>${contactName}<br>${email}<br>${phone}</p><p>Programs: ${programs.join(', ')}</p><p>Supervisor: ${supervisorName} — ${supervisorLicenseNumber}</p><p>License document: ${licensePath}</p><p>Insurance document: ${insurancePath || 'Not uploaded'}</p><p>Reference: ${data.id}</p>`,
    });
