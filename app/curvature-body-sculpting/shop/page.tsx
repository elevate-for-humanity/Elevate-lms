...
function CheckoutButton({ cartTotal, itemCount }: { cartTotal: number; itemCount: number }) {
  ...
  return (
    <button
      ...
      className="w-full bg-brand-red-600 text-white py-2 rounded-lg font-medium hover:bg-brand-red-700 transition disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2"
    >
      {loading ? (
        <>
...
{/* Header */}
                ...
                <div className="absolute top-full right-0 mt-2 w-72 bg-white text-gray-900 rounded-lg shadow-xl p-4 z-10 hidden group-hover:block">
                  ...
                  <div className="border-t border-gray-200 pt-2 mb-3">
                    <div className="flex justify-between font-bold">
                      <span>Total</span>
                      <span className="text-brand-red-600">${cartTotal.toFixed(2)}</span>
                    </div>
                  </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-brand-red-500 focus:border-brand-red-500"
              />
            </div>
            {/* Categories */}
                  ...
                  >
                    ...
                    <div className="relative h-48 bg-pink-100 flex items-center justify-center overflow-hidden">
                      {product.image ? (
                        ...
                        <Sparkles className="w-16 h-16 text-brand-red-300 group-hover:scale-110 transition" />
                      )}
                      {product.badge && (
                        <span className="absolute top-3 left-3 px-2 py-1 bg-brand-red-600 text-white text-xs font-bold rounded-full z-10">
                          {product.badge}
                        </span>
                      )}
                    </div>
                    {/* Product Info */}
                    <div className="p-4">
                      <p className="text-xs text-brand-red-600 font-medium mb-1 capitalize">{product.category}</p>
                      <h3 className="font-bold text-gray-900 mb-2">{product.name}</h3>
                      <p className="text-gray-600 text-sm mb-3 line-clamp-2">{product.description}</p>
                      {product.ingredients && (
                        <details className="mb-3">
                          <summary className="text-xs text-brand-red-600 cursor-pointer hover:text-brand-red-800 font-medium">
                            View Ingredients
                          </summary>
      ...
      {/* Partnership Banner */}
      <section className="py-12 bg-brand-red-900 text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <Heart className="w-12 h-12 text-pink-400 mx-auto mb-4" />
          <h2 className="text-2xl font-bold mb-4">Supporting Mental Wellness</h2>
          <p className="text-brand-red-200 mb-6">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link
              ...
              className="px-6 py-3 bg-white text-brand-red-900 font-medium rounded-lg hover:bg-brand-red-50 transition"
            >
      ...
      <section className="py-12 bg-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Contact us at <a href="mailto:curvaturebodysculpting@gmail.com" className="text-brand-red-600 hover:underline">curvaturebodysculpting@gmail.com</a>
          </p>
        </div>
      </section>
    </div>
