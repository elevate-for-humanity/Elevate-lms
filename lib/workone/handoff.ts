...
export async function ensureWorkOneHandoffByReference(
  ...
  if (responseId) {
    ...
    const insertResult = await db
      ...
      .select('id')
      .single();
    if (insertResult.error || !insertResult.data) throw insertResult.error || new Error('Unable to create WorkOne progress record');
  ...
  const codeBlock = programCode
    ? `<div style="margin:20px 0;padding:18px;border:2px solid #0f172a;border-radius:10px;background:#f8fafc;text-align:center;"><div style="font-size:13px;color:#475569;">INTraining Program ID</div><div style="font-family:monospace;font-weight:800;font-size:28px;color:#0f172a;margin-top:6px;">${programCode}</div></div>`
  ...
  const emailResult = await sendEmail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#0f172a;line-height:1.55;">
      ...
      <p>Hi ${application.first_name || 'there'},</p>
      ...
      ${codeBlock}
      <p><strong>Important:</strong> If WorkOne cannot find Elevate by name, ask them to search the INTraining program ID shown above and verify the provider as <strong>${organization.legalName} dba ${organization.dbaName}</strong>.</p>
      ...
      <p style="font-size:13px;color:#475569;">Application reference: ${application.reference_number || referenceNumber}<br/>${organization.dbaName}<br/>${organization.phone} · ${organization.email}</p>
    </div>`,
    attachments: [
      {
        content: pdf.toString('base64'),
        filename: `WorkOne-Program-Sheet-${String(application.reference_number || referenceNumber).replace(/[^A-Za-z0-9_-]/g, '')}.pdf`,
