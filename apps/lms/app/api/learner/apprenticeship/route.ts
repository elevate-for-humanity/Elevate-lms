...
export async function GET(request: NextRequest) {
  try {
    ...
    const config = loaded?.blueprint.apprenticeshipConfig;
    if (!config) return NextResponse.json({ error: 'Apprenticeship not supported for this program' }, { status: 400 });
    ...
    return NextResponse.json({
      ...
      hours: {
        total: {
          ...
          remaining: Math.max(0, config.totalHours - (rtiCompleted + ojlCompleted)),
          percentComplete: config.totalHours ? Math.min(100, Math.round(((rtiCompleted + ojlCompleted) / config.totalHours) * 100)) : 0,
        },
        rti: { required: config.rtiHours, completed: rtiCompleted },
        ojl: { required: config.ojlHours, completed: ojlCompleted },
      },
      competencyTracking: { totalRequired: config.competencyCount, verified, signoffs: signoffsData.data || [] },
      employerEvaluations: evaluationsData.data || [],
      rapids: rapidsData.data || null,
    });
  } catch (error) {
