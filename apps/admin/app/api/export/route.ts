...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!type) return NextResponse.json({ error: 'Export type is required' }, { status: 400 });

    let authResult;
    if (type === 'students' || type === 'analytics') authResult = await apiRequireAdmin(request);
    else authResult = await apiRequireInstructor(request);
    ...
    switch (type) {
      case 'students': data = await exportStudents(options); break;
      case 'courses': data = await exportCourses(options); break;
      case 'enrollments': data = await exportEnrollments(options); break;
      case 'assignments': data = await exportAssignments(options); break;
      case 'grades': data = await exportGrades(options); break;
      case 'analytics': {
        const entityId = searchParams.get('entityId');
        const entityType = searchParams.get('entityType') as 'course' | 'student' | 'instructor';
        if (!entityId || !entityType) {
          return NextResponse.json({ error: 'entityId and entityType required for analytics export' }, { status: 400 });
        }
    ...
    await auditLog({
      ...
      metadata: { export_type: type, format, record_count: data.length, filters: options.filters },
    });
    ...
    if (format === 'pdf') {
      return NextResponse.json({ error: 'PDF export is not available. Use CSV format.' }, { status: 501 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const table of tables) {
      ...
      switch (table) {
        case 'students': results[table] = await exportStudents({ filters: tableFilters }); break;
        case 'courses': results[table] = await exportCourses({ filters: tableFilters }); break;
        case 'enrollments': results[table] = await exportEnrollments({ filters: tableFilters }); break;
        case 'assignments': results[table] = await exportAssignments({ filters: tableFilters }); break;
        case 'grades': results[table] = await exportGrades({ filters: tableFilters }); break;
      }
    }
