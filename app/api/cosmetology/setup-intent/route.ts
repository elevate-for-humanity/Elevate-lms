...
import { TUITION_CENTS, PAYMENT_TERM_WEEKS, weeklyPaymentCents, COSMETOLOGY_PROGRAM_ID } from '@/lib/cosmetology/pricing';
import { applyRateLimit } from '@/lib/api/withRateLimit';
import { logger } from '@/lib/logger';
import { safeError, safeInternalError } from '@/lib/api/safe-error';

// POST /api/cosmetology/setup-intent
// Creates a Stripe Customer (if needed) and a SetupIntent for saving a payment method.
// Called from the cosmetology payment-setup onboarding step.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if a cosmetology_subscriptions record already exists
    ...
    if (!customerId) {
      const customer = await stripe.customers.create({
        email,
        name,
        metadata: { user_id: user.id, program: 'cosmetology-apprenticeship' },
      });
    ...
    if (!existingSub) {
      ...
      await db.from('cosmetology_subscriptions').insert({
        user_id: user.id,
    ...
    // Pull final weekly amount from DB
    ...
    logger.error('[cosmetology/setup-intent] Error', err);
